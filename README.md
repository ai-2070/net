...
Net is a working protocol, not a paper design. ~1,573 unit tests plus ~469 integration + SDK-level tests across the Rust core (`crates/net/tests/` + `sdk/tests/`), plus 62 Node and ~190 Python SDK smoke tests, verify the implementation across every layer of the stack.
...
- **NAT traversal as an optimization (feature `nat-traversal`).** Two NATed peers reach each other unconditionally via the routed-handshake path above; NAT traversal is a shorter direct path when the NAT shape allows it, never a correctness requirement. Each node peer-probes its reflex over `SUBPROTOCOL_REFLEX` (`0x0D00`) and classifies itself `Open` / `Cone` / `Symmetric` / `Unknown`; the classification rides on capability announcements as a `nat:*` tag plus a dedicated `reflex_addr` field. `connect_direct` applies a pair-type matrix (plan §8): Open-side pairs connect directly, Symmetric × Symmetric skips punching, everything else runs a 3-message rendezvous on `SUBPROTOCOL_RENDEZVOUS` (`0x0D01`) — `PunchRequest` → coordinator fans out `PunchIntroduce` with a synchronized `fire_at` → keep-alive train primes NAT state → `PunchAck` confirms. Three monotonic counters (`punches_attempted` / `punches_succeeded` / `relay_fallbacks`) partition real wire activity after the ordering fixes landed; operators can read them as a genuine NAT-traversal-effectiveness signal without matrix-only inflation. Runtime `set_reflex_override` / `clear_reflex_override` let port-forwarded servers or successful port-mapping installs short-circuit the classifier sweep entirely; setters serialize under a publication mutex against the announce read path so peers never observe a torn `(nat_class, reflex_addr)` snapshot. Full design in [`NAT_TRAVERSAL_PLAN.md`](net/crates/net/docs/NAT_TRAVERSAL_PLAN.md).
- **Opt-in UPnP-IGD / NAT-PMP port mapping (feature `port-mapping`).** `MeshNodeConfig::with_try_port_mapping(true)` spawns a task that probes NAT-PMP first (RFC 6886, inlined wire codec with RFC-mandated kernel source-address filter via `UdpSocket::connect` — defense against response spoofing), falls back to UPnP-IGD (`igd-next`), installs a mapping, and renews every 30 minutes. On install the task calls `set_reflex_override(external)` and peers see the mapped address on the next announce; renewal propagates a rebound external if the router changes it (router reboot / WAN flap); three consecutive renewal failures or shutdown trigger a best-effort revoke (fire-and-forget UDP — no 1-second stall on a dead gateway). Gateway discovery reads the OS routing table directly on Linux (`/proc/net/route`) and macOS (`route -n get default`), skipping invalid or zero-gateway rows. Port mapping is a latency optimization on top of an already-working routed mesh — a router that doesn't cooperate leaves the node on the classifier path, which is fine. Full design in [`PORT_MAPPING_PLAN.md`](net/crates/net/docs/PORT_MAPPING_PLAN.md).
