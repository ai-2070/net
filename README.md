...
nRPC is request/response *on the bus*. There is no second transport. Same niche as gRPC; different shape. No HTTP/2, no protobuf IDL, no codegen step. The wire is **raw bytes** over the existing encrypted UDP transport — the typed SDK layers ride a codec on top (JSON by default; Rust can also use postcard or a custom one), and the schema is whatever both sides agree on (serde, TypeScript interfaces, Pydantic, Go structs). Deadlines and end-to-end cancellation come from the SDK rather than a sidecar, and work the same way across Rust, TypeScript, Python, and Go. **Retries, hedging and circuit breakers are Rust-only helpers today** — Go has deadlines and context cancellation but no retry, hedge or breaker; check `bindings/coverage.md` before promising them in another language.
...
Because nRPC rides the same channel machinery as everything else, it inherits that machinery's **backpressure**: when the channel append rate-limits, the caller sees `RpcError::Backpressure` and the retry helper handles it. No RPC-specific scaffolding.

**In-flight request state is in-memory today.** Serve state is a bounded `mpsc` plus an `RpcServerFold`; a full queue drops the request or response and the caller times out. There is no persistence, snapshot restore, or durable replay wired behind the RPC path, so do not plan on crash recovery, at-least-once handler execution, in-flight migration, or a replayable per-service audit trail — those are properties of the durable log layer, and nRPC is not currently integrated with it. Design for at-most-once with an explicit application-level retry, and put anything that must survive a crash in RedEX yourself.
...
|-----------|--------|-----------|
| Header serialize | 2.19 ns / **456M ops/sec** | 1.21 ns / **829M ops/sec** |
| Header deserialize | 2.35 ns / **426M ops/sec** | 1.61 ns / **622M ops/sec** |
| Routing header serialize | 0.62 ns / **1.60G ops/sec** | 0.51 ns / **1.97G ops/sec** |
| Routing header forward | 0.57 ns / **1.75G ops/sec** | 0.20 ns / **4.96G ops/sec** |
| Routing lookup (hit) | 37.73 ns / **26.5M ops/sec** | 38.14 ns / **26.2M ops/sec** |
| Decision pipeline | 37.48 ns / **26.7M ops/sec** | 38.47 ns / **26.0M ops/sec** |

### Multi-hop Forwarding

| Hops | M1 Max | i9-14900K |
|-----:|--------|-----------|
| 1 | 61.66 ns / **16.2M ops/sec** | 53.42 ns / **18.7M ops/sec** |
| 2 | 116.46 ns / **8.59M ops/sec** | 88.64 ns / **11.3M ops/sec** |
| 3 | 160.03 ns / **6.25M ops/sec** | 121.08 ns / **8.26M ops/sec** |
| 5 | 271.11 ns / **3.69M ops/sec** | 189.54 ns / **5.28M ops/sec** |

| Threads | M1 Max | i9-14900K |
|--------:|--------|-----------|
| 4 | 5.64 M/s | 6.89 M/s |
| 8 | 6.94 M/s | 9.91 M/s |
| 16 | 9.20 M/s | 10.5 M/s |

### Failure Detection & Recovery

| Operation | M1 Max | i9-14900K |
|-----------|--------|-----------|
| Heartbeat (existing node) | 39.76 ns / **25.2M ops/sec** | 69.27 ns / **14.4M ops/sec** |
| Status check | 15.10 ns / **66.2M ops/sec** | 15.17 ns / **65.9M ops/sec** |
| Circuit breaker check | 9.55 ns / **104.7M ops/sec** | 11.11 ns / **90.0M ops/sec** |
| Recovery (evaluate alternates) | 257.51 ns / **3.88M ops/sec** | 354.87 ns / **2.82M ops/sec** |
| Full fail + recover cycle | 290.75 ns / **3.44M ops/sec** | 279.63 ns / **3.58M ops/sec** |

### Swarm / Discovery

| Operation | M1 Max | i9-14900K |
|-----------|--------|-----------|
| Pingwave serialize | 0.78 ns / **1.28G ops/sec** | 0.52 ns / **1.93G ops/sec** |
| Pingwave roundtrip | 0.93 ns / **1.07G ops/sec** | 0.64 ns / **1.57G ops/sec** |
| New peer discovery | 39.49 ns / **25.3M ops/sec** | 37.92 ns / **26.4M ops/sec** |

| Nodes | M1 Max (all_nodes) | i9-14900K (all_nodes) |
|------:|-------------------:|----------------------:|
| 100 | 2.85 us | 13.27 us |
| 500 | 8.33 us | 26.06 us |
| 1,000 | 37.38 us | 43.11 us |
| 5,000 | 113.37 us | 316.56 us |

### Encryption (ChaCha20-Poly1305)

| Payload | M1 Max | i9-14900K |
|--------:|--------|-----------|
| 64B | 301.09 ns / 202.7 MiB/s | 212.81 ns / 286.8 MiB/s |
| 256B | 476.74 ns / 512.1 MiB/s | 277.96 ns / 878.3 MiB/s |
| 1KB | 908.20 ns / 1.05 GiB/s | 540.50 ns / 1.76 GiB/s |
| 4KB | 2.89 us / 1.32 GiB/s | 1.53 us / 2.50 GiB/s |
...
| Threads | M1 Max | i9-14900K |
|--------:|--------|-----------|
| 8 | **11.66 M/s** | **14.19 M/s** |
| 16 | **11.46 M/s** | **17.21 M/s** |
| 32 | **11.47 M/s** | **21.09 M/s** |
...
|--------:|--------|-----------|
| 8 | **67.8 M/s** | **69.7 M/s** |
| 16 | **72.7 M/s** | **91.1 M/s** |
| 32 | **70.4 M/s** | **120.9 M/s** |

### SDK Ingestion

| SDK | Method | Throughput | Latency |
|-----|--------|------------|---------|
| Go | IngestRaw (9B) | **8.69M/sec** | 115 ns |
| Go | Batch (1000) | **7.51M/sec** | 133 ns/event |
| Python | ingest_raw (9B) | **6.88M/sec** | 0.15 us |
| Python | Batch (1000) | **9.53M/sec** | 0.10 us |
| Node.js | pushBatch | **6.07M/sec** | 0.16 us |
| Node.js | push (single) | **4.34M/sec** | 0.23 us |
| Bun | pushBatch | **6.91M/sec** | 0.14 us |
| Bun | push (single) | **4.57M/sec** | 0.22 us |

SDK ingestion benchmarks were captured 2026-06-15 on M1 Max.

All SDKs exceed **4.3M events/sec** even on single-event ingestion, and **6M+ events/sec** on batch. Go leads single-event ingestion at **8.69M/sec** (zero allocations on the raw ingestion path). Python (via PyO3) is the fastest binding on batch at **9.53M/sec** — the GIL releases for the duration of the FFI call so per-event overhead is the bare PyO3 marshalling. Node.js sync methods are ~51x faster than async (`push` 4.34M vs async `ingestRaw`). Bun batch (6.91M) is ~14% faster than Node.js batch (6.07M) on the same `pushBatch` call.
