...
cargo install net-cli    # the `net-cli` crate builds the `net-mesh` binary
                         # (net-mesh wrap / mcp serve / mcp pin / forwarding)
```
...
tools. `--allow-capability` pre-approves a sensitive capability at startup.
...
It manages **policy only**: it records _that_ a secret ref may go to a provider,
...
local). Forwarding inverts that _only_ for downstream services that understand
...
Deny wins on any mismatch, and a denial names _which_ gate refused (global /
...
tampered field fails the tag). Exact replay of an _unmodified_ blob to its
...
> explicitly allowed _and_ accepted.
