...
- **Two new audit docs shipped in tree.** `docs/internal/plans/NRPC_AI_TOOL_CALLING_AND_AGENT_DX.md` covers the agent surface (eight locked decisions, phasing, per-binding status); `docs/internal/misc/PERF_AUDIT_2026_05_28_CAPABILITY.md` covers the capability perf pass (headline regressions, root causes with file:line pointers, ranked fixes with risk/touch columns, before/after numbers per fix).
