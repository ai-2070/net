...
*Named after the Rolling Stones' 1969 opener on Let It Bleed — the one Keith Richards wrote during a thunderstorm at his Robert Fraser flat, the one Merry Clayton recorded in a single overnight session at six months pregnant (she lost the baby days after; her voice cracks on the second "murder" and they kept it because nothing else would do). The song is a four-minute weather report from a culture that had run out of dry places. War, children, just a shot away; rape, murder; love, sister; the riff insisting one more time that shelter exists somewhere. v0.22 put a watchtower on every tier. v0.23 puts shelter on every binding. Three diverged cancellation stories collapse into one substrate primitive that the bindings pass through. Three different "callbacks must be cheap" footguns collapse into one bounded queue with a drop counter the operator can read. Three streaming-typed APIs (Rust, Node, Python) become four, then become the same four shapes everywhere with the same observer hook and the same metrics snapshot. The Python binding stops asking asyncio consumers to fall back to a thread pool. Same wire, same semantics, same surface — gimme shelter, or I'm gonna fade away.*
