...
- **A running mesh peer to join.** `net-mesh wrap` builds a node and *joins* the mesh via
...
- If invoke is refused unexpectedly, check that the serve side runs under the *same
  identity* as the wrap side, or widen with `net-mesh wrap --allow 0x<caller-origin>`.
