...
- The `net-mesh` binary built with the CLI feature:
  `cargo install net-cli` (or `cargo build --release -p net-cli`, which produces the `net-mesh` binary).
...
2. **Serve the mesh to a host** (demand side). On the node with your MCP host, run
   ...
   ```
   net-mesh mcp serve --node-addr <peer> --node-pubkey 0x<hex> --psk-hex <psk> --identity ./operator.toml
   ```
...
- [ ] The wrapped tool appears in `net_search_capabilities` output from the _other_
      node — discovery crossed the mesh.
- [ ] `net_invoke_capability` returns the tool's expected typed result rather than
      a permission denial.
- [ ] A node with a **different** root identity that runs `net-mesh mcp serve` can
      _search/describe_ the tool but its `net_invoke_capability` is refused —
...
- A **credentialed, external, or unknown-risk** capability is search/describe-only
  until pinned. Approve it out-of-band: `net-mesh mcp pin approve provider/tool`. The
  model cannot approve its own pin.
- If invoke is refused unexpectedly, check that the serve side runs under the _same
  identity_ as the wrap side, or widen with `net-mesh wrap --allow 0x<caller-origin>`.
