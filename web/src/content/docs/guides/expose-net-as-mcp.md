...
The shim exposes a fixed, small surface — the host discovers the _mesh_ through
...
credentials) can be invoked directly. A **sensitive** one — credentialed, external, or
unknown-status — is **search/describe-only until it's pinned**. This is the
confused-deputy defense: connecting an MCP host to the mesh grants it _no_ ambient
...
Without a pin or `--allow-capability`, `net_invoke_capability` on a sensitive
...
a pin is _local client consent_, not remote authorization — if the remote
