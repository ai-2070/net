...
impl LeafNode {
    ...
    fn handle_membership(&mut self, peer: NodeId, msg: MembershipMsg) {
        match msg {
            ...
            // The `Ack` answering a `0x0A00` request this leaf sent —
            // the awaiter the old fire-and-forget arm anticipated.
            // Correlated by the nonce `subscribe`/`unsubscribe`
            // returned, and surfaced distinctly: the Ack is the
            // protocol's own "admitted or refused" and until now was
            // discarded here, which made an anchor-side refusal
            // indistinguishable from success. An admission moves
            // `membership_admitted`, a refusal moves
            // [`DropReason::MembershipRefused`] **and** a
            // [`LeafEvent::Dropped`] — and a refused **Subscribe**
            // takes the claim back. A refusal that can no longer be
            // correlated is still surfaced (a refusal is always
            // news); an uncorrelated admission is stale good news and
            // is ignored.
            //
            ...
            MembershipMsg::Ack {
                ...
                if pending.kind == MembershipKind::Subscribe {
                    ...
                    let key = (peer, pending.stream_id);
                    if self.channel_claims.get(&key).copied() == Some(nonce) {
                        ...
                        if self.rpc_reply_carriers.remove(&key) {
                            self.reply_subscriptions.remove(&(peer, pending.canonical));
                        }
                    }
                }
            }
