...
impl LeafNode {
    ...
    fn handle_membership(&mut self, peer: NodeId, msg: MembershipMsg) {
        match msg {
            ...
            // The answer to a request this leaf sent: correlated by
            // nonce AND from-node (an Ack is only the publisher's to
            // give) in `handle_membership_ack`.
            MembershipMsg::Ack {
                nonce,
                accepted,
                reason: _,
    ...
    /// The membership plane's Ack side: the `Ack` answering a
