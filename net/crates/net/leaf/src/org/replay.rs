    ...
    pub fn admit(
        ...
        if let Some(inner) = st.by_caller.get_mut(caller) {
            if let Some(existing) = inner.get(&call_id) {
                ...
                // Expired overwrite REUSES the occupied key, so no
                // counter moves — the entry it replaces was already
                // charged and is charged to the same principal by
                // construction (the key is `(caller, call_id)`).
                inner.insert(
                    call_id,
                    ReplayEntry {
                        binding_digest,
