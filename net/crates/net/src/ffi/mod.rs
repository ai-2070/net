...
/// Declares [`NetError`] and, from the same list, the
/// `ALL_NET_ERRORS` table the C-header parity guard iterates.
///
/// The coupling is the point. `rust_values` used to be a hand-written
/// `&[i32]`, and it silently missed `InvalidArgument = -12` for the
/// whole of the scoped-capabilities work — so the guard reported green
/// while not covering the newest code. Replacing it with an exhaustive
/// `match` was not enough either: exhaustiveness forces a new variant to
/// get an arm, but the header asserts iterate the *list*, so a variant
/// added to the enum and to the match and omitted from the list still
/// passed.
///
/// Generating both from one declaration removes the possibility. A new
/// variant cannot exist without appearing in the table.
macro_rules! net_error_codes {
    ($( $(#[$meta:meta])* $variant:ident = $value:expr ),+ $(,)?) => {
        /// Error codes returned by FFI functions.
        ///
        /// `Copy` because it is a fieldless `#[repr(C)]` enum that is
        /// passed by value everywhere already; having it lets the
        /// header-parity guard read each variant's discriminant
        /// directly.
        #[repr(C)]
        #[derive(Clone, Copy)]
        pub enum NetError {
        ...
        /// Every [`NetError`] variant paired with the discriminant the
        /// C headers must mirror. Generated alongside the enum, so it
        /// cannot fall behind it.
        #[cfg(test)]
        const ALL_NET_ERRORS: &[(NetError, i32)] = &[ $( (NetError::$variant, $value), )+ ];
    };
}
...
mod tests {
    ...
    fn cr22_c_header_parity_with_rust_neterror() {
        ...
        // `ALL_NET_ERRORS` is generated by the same `net_error_codes!`
        // invocation that declares the enum, so a variant cannot exist
        // without appearing here. See that macro for why neither a
        // hand-written list nor an exhaustive match was sufficient.
        //
        // The `#[repr(C)]` discriminant is what the headers actually
        // mirror, so confirm the table's declared value matches it
        // rather than assuming the macro was fed correctly.
        for &(e, declared) in ALL_NET_ERRORS {
            assert_eq!(
                c_int::from(e),
                declared,
                "ALL_NET_ERRORS declares {declared} but the #[repr(C)] \
                 discriminant is {}",
                c_int::from(e)
            );
        }
        let rust_values: Vec<i32> = ALL_NET_ERRORS.iter().map(|&(_, v)| v).collect();
