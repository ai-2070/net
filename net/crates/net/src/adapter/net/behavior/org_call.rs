...
/// transcript (`ORG_SCOPED_STREAMING_PLAN.md` §1.2). A separate
/// signing domain from [`ORG_CALL_BINDING_CONTEXT`]: a signature
/// over the 11 unary fields can never verify as a streaming
/// binding or vice versa, whatever subset of fields the two
/// transcripts share.
...
/// call; the three streaming shapes map 1:1 onto the wire `kind` of an
/// [`OrgStreamCallProof`] ([`Self::stream_kind`]). `#[non_exhaustive]`:
/// a future shape (e.g. an announced fourth streaming form) must not
/// break downstream exhaustive matches the way `is_unary` silently
/// generalized would have.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum RpcCallShape {
    /// One request, one response — no streaming flags.
    Unary,
    /// One request, many responses (`FLAG_RPC_STREAMING_RESPONSE`).
    ServerStreaming,
    /// Many requests, one response (`FLAG_RPC_CLIENT_STREAMING_REQUEST`).
    ClientStreaming,
...
impl RpcCallShape {
    ...
    /// resolved to plain bools at the cortex layer — this module never
    /// imports the cortex constants).
    ...
    /// The proof `kind` this shape rides, or `None` for [`Self::Unary`]
    /// (which rides the unary [`OrgCallProof`] and emits no kind).
    ...
    /// The shape a wire `kind` names (`None` for 0 or any unknown kind).
...
impl CallBinding {
    ...
    fn transcript_hash(&self) -> [u8; 32] {
        // Exact 304 B (9 × 32-byte fields + two u64 LE) — the previous
        // `32 * 6 + 8 + 8 + 32 = 240` under-estimate forced one realloc
        // per sign/verify (plan §1.2).
...
/// Wall-clock expiry check with skew, shared by the unary and streaming
/// proofs (§2.3): the proof must not be expired, and its claimed expiry
/// must not exceed `now + MAX_ORG_PROOF_TTL_SECS`.
fn check_proof_expiry_at(
    proof_expires_at_unix_ns: u64,
    now_ns: u64,
    skew_secs: u64,
) -> Result<(), OrgError> {
...
/// The streaming four-party call binding
/// (`ORG_SCOPED_STREAMING_PLAN.md` §1.2): the 11 [`CallBinding`]
/// fields PLUS `kind` and `session_binding`, fixed width, 337 B,
/// hashed under [`ORG_STREAM_CALL_BINDING_CONTEXT`].
///
...
pub struct StreamCallBinding {
    /// Org A — whom the caller acts FOR (as [`CallBinding::acting_org`]).
    pub acting_org: OrgId,
    /// The caller entity S (the signing key).
    pub caller: EntityId,
    ...
    /// header removed.
    pub request_digest: [u8; 32],
    /// The streaming call kind (1 SS / 2 CS / 3 DX; 0 is never emitted
    /// and [`OrgStreamCallProof::decode`] rejects it).
    pub kind: u8,
    /// The full Noise handshake hash of the ONE session this opening
    /// rides (§1.3) — raw hash, no HKDF label (the derive_key context
    /// string is the domain separation).
    pub session_binding: [u8; 32],
}

impl StreamCallBinding {
    /// Domain-separated transcript hash. Fixed width (337 B: the 304-B
    /// unary field set + 1-byte kind + 32-byte session binding), so the
    ...
    /// Verify `signature` against the caller entity over this binding.
    pub fn verify(&self, signature: &[u8; 64]) -> Result<(), OrgError> {
        let sig = Signature::from_bytes(signature);
        self.caller
            .verify(&self.transcript_hash(), &sig)
...
/// The per-call admission proof for a STREAMING call
/// (`ORG_SCOPED_STREAMING_PLAN.md` §1.1). Wire-**prefix-compatible**
/// with [`OrgCallProof`]: the same five leading fields in the same
/// order (so the frozen old decoder consumes the prefix and ignores
/// the suffix — the §1.4 mixed-version argument), followed by
/// `kind: u8` and `session_binding: [u8; 32]`.
///
...
pub struct OrgStreamCallProof {
    ...
    /// The streaming call kind: 1 SS / 2 CS / 3 DX (0 is never emitted).
    pub kind: u8,
    /// The Noise handshake hash of the ONE session this opening rides.
    pub session_binding: [u8; 32],
}

impl OrgStreamCallProof {
    /// Assemble and sign a streaming proof for one call (the streaming
    /// counterpart of [`OrgCallProof::sign_for_call`]).
    #[allow(clippy::too_many_arguments)]
    ...
    /// `kind`/`session_binding`, which the provider separately checks
    /// against the call's flags and the RECEIVING session).
    #[allow(clippy::too_many_arguments)]
    ...
    /// (`OrgProviderPolicy` keeps its unary signature; `kind` and
    ...
    /// Wall-clock expiry check with skew (§2.3), as
    /// [`OrgCallProof::check_expiry`].
    pub fn check_expiry(&self, skew_secs: u64) -> Result<(), OrgError> {
        check_proof_expiry_at(
            self.proof_expires_at_unix_ns,
            current_timestamp().saturating_mul(1_000_000_000),
            skew_secs,
        )
    }

    /// Explicit-time variant (as [`OrgCallProof::check_expiry_at`]).
    pub fn check_expiry_at(&self, now_ns: u64, skew_secs: u64) -> Result<(), OrgError> {
        check_proof_expiry_at(self.proof_expires_at_unix_ns, now_ns, skew_secs)
    }
    ...
    /// STRICT streaming decode (§1.1): over-cap input is refused before
    /// parsing; truncation fails to decode; unknown `kind` (0 or > 3) is
    /// refused; and the decoder consumes the ENTIRE bounded value —
    /// trailing bytes after the suffix are refused, never ignored.
    pub fn decode(bytes: &[u8]) -> Result<Self, OrgError> {
        if bytes.len() > MAX_ORG_CALL_PROOF_BYTES {
            return Err(OrgError::InvalidFormat);
        }
...
impl std::fmt::Debug for OrgStreamCallProof {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
mod tests {
    ...
    /// §1.1 wire-prefix compatibility: an `OrgStreamCallProof`'s bytes
    /// BEGIN with exactly the `OrgCallProof` encoding of the same five
    /// fields. This is the property the §1.4 mixed-version argument and
    /// the frozen-old-provider witness rest on.
    #[test]
    fn stream_proof_bytes_prefix_match_the_unary_encoding() {
        let digest = [0x11u8; 32];
        let caller = caller();
        let membership =
            OrgMembershipCert::try_issue(&org_a(), caller.entity_id().clone(), 1, 3600)
        ...
        let unary_bytes = unary.encode().expect("encode unary");
        let stream_bytes = stream.encode().expect("encode stream");
        // The stream encoding is the unary field layout plus EXACTLY the
        // 33-byte suffix (1-byte kind + 32-byte session binding).
        assert_eq!(
            stream_bytes.len(),
            unary_bytes.len() + 33,
            "the streaming suffix is kind + session_binding",
        );
        // Structural prefix compatibility (§1.1): the frozen trailing-tolerant
        // unary decoder consumes an `OrgStreamCallProof`'s leading five fields
        // and recovers them EXACTLY — the property the §1.4 mixed-version
        // argument and the frozen-old-provider witness rest on. (The
        // `call_binding_sig` VALUE legitimately differs between the two
        // proofs — each signs its own transcript — so compatibility is
        // structural; every byte BEFORE the signature field encodes
        // identically for identical inputs.)
        let decoded =
            OrgCallProof::decode(&stream_bytes).expect("the unary decoder reads the stream prefix");
        assert_eq!(decoded.caller_membership, stream.caller_membership);
        assert_eq!(decoded.dispatcher_grant, stream.dispatcher_grant);
        assert_eq!(decoded.capability_grant, stream.capability_grant);
        assert_eq!(
            decoded.proof_expires_at_unix_ns,
            stream.proof_expires_at_unix_ns,
        );
        assert_eq!(decoded.call_binding_sig, stream.call_binding_sig);
        const SIG_WIRE: usize = 65; // postcard bytes-with-length prefix + 64
        assert_eq!(
            &stream_bytes[..stream_bytes.len() - 33 - SIG_WIRE],
            &unary_bytes[..unary_bytes.len() - SIG_WIRE],
            "everything before the signature field encodes identically",
        );
    }

    /// §1.1 full-consumption decode: truncation, trailing bytes and
    /// unknown kinds are all refused — the new decoder is strict even
    /// though the frozen old one is prefix-tolerant.
    #[test]
    fn stream_decode_rejects_truncation_trailing_bytes_and_unknown_kinds() {
        let (proof, _) = {
            let digest = [0x11u8; 32];
            let caller = caller();
            let membership =
        ...
        // Round-trip is exact.
        assert_eq!(OrgStreamCallProof::decode(&bytes).expect("decode"), proof);

        // Truncation at ANY suffix position is refused (the unary prefix
        // alone is NOT a valid streaming proof).
        for cut in [bytes.len() - 1, bytes.len() - 17, bytes.len() - 33] {
            assert!(
                matches!(
                    OrgStreamCallProof::decode(&bytes[..cut]),
                    Err(OrgError::InvalidFormat)
                ),
                "truncated streaming proof at {cut}/{} must be refused",
                bytes.len(),
            );
        }
        ...
        trailing.push(0);
        assert!(matches!(
            OrgStreamCallProof::decode(&trailing),
            Err(OrgError::InvalidFormat)
        ));

        // Unknown kinds (0 is never emitted; 4+ do not exist) are refused
        // even though the full value decodes structurally.
        for bad_kind in [0u8, 4, 0xFF] {
            let mut mutated = bytes.clone();
            let kind_idx = mutated.len() - 33;
            mutated[kind_idx] = bad_kind;
            assert!(
                matches!(
                    OrgStreamCallProof::decode(&mutated),
                    Err(OrgError::InvalidFormat)
                ),
                "kind {bad_kind} must be refused by the streaming decoder",
            );
        }
    }
}
