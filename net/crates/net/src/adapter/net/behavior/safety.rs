...
pub struct SafetyEnforcer {
    ...
    /// Compiled content patterns (for hot path). Gated behind the
    /// `regex` feature so consumers who don't ship pattern-matching
    /// safety predicates skip the regex crate.
    #[cfg(feature = "regex")]
    #[allow(dead_code)]
