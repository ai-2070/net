...
pub struct PlacementFilterRegistry {
    filters: DashMap<String, Arc<dyn PlacementFilter>>,
}
...
impl PlacementFilterRegistry {
    ...
    /// Register `filter` under `id`. Returns `true` on success,
    /// `false` if `id` is already registered (no overwrite — the
    /// SDK is responsible for generating unique IDs via its
    /// `pf-N` counter or explicit ID parameter).
    ///
    ...
    pub fn register(&self, id: String, filter: Arc<dyn PlacementFilter>) -> bool {
    ...
    pub fn get(&self, id: &str) -> Option<Arc<dyn PlacementFilter>> {
        self.filters.get(id).map(|r| r.clone())
    }
    ...
    /// Drop every registration. Mainly for test isolation —
    /// production callers should `unregister` deliberately.
    pub fn clear(&self) {
        self.filters.clear();
    }
}
...
mod tests {
    ...
    fn register_and_get_returns_same_filter() {
        ...
        assert!(reg.register("pf-1".into(), filter.clone()));
        assert_eq!(reg.len(), 1);
    ...
    fn register_refuses_to_overwrite_existing_id() {
        ...
        assert!(reg.register("pf-1".into(), original));
        assert!(
            !reg.register("pf-1".into(), challenger),
    ...
    fn unregister_returns_true_only_on_first_call() {
        let reg = PlacementFilterRegistry::new();
        reg.register("pf-1".into(), Arc::new(FixedFilter(0.3)));
    ...
    fn get_clone_outlives_unregister() {
        let reg = PlacementFilterRegistry::new();
        reg.register("pf-1".into(), Arc::new(FixedFilter(0.42)));
    ...
    fn clear_drops_every_registration() {
        let reg = PlacementFilterRegistry::new();
        reg.register("pf-1".into(), Arc::new(FixedFilter(0.1)));
        reg.register("pf-2".into(), Arc::new(FixedFilter(0.2)));
        reg.register("pf-3".into(), Arc::new(FixedFilter(0.3)));
        assert_eq!(reg.len(), 3);
        ...
        let handles: Vec<_> = (0..n)
            .map(|i| {
                ...
                std::thread::spawn(move || {
                    ...
                    assert!(reg.register(format!("pf-{i}"), f));
                })
            })
            .collect();
        for h in handles {
    ...
    fn global_singleton_is_shared_across_calls() {
        ...
        assert!(reg_a.register(id.into(), Arc::new(FixedFilter(0.6))));
