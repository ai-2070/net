...
//
// The default `store_dir`/`fetch_dir` path moves bytes over whichever
// transport the adapter is configured for. On a `with_replication`
// adapter, `store` auto-leaders each chunk channel and `fetch` auto-
// replicas + pulls via the replication runtime (one per-node replica
// set, no per-chunk `causal` tag) — so the SAME `store_dir`/`fetch_dir`
// scale past the ~15-20-chunks-per-node advertisement ceiling with no
// caller-visible role management. See
// `tests/cross_peer_blob.rs::replicated_directory_transfer_end_to_end`.
// (Earlier `become_dir_leader` / `fetch_dir_replicated` helpers were
// removed once the adapter made roles automatic.)
