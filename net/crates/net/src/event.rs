...
///
/// Defined in `net-mesh-wire` since Stage 2 (`NetSession`'s inbound
/// queue carries it); re-exported here so `crate::event::StoredEvent`
/// and every `net::event::StoredEvent` path still resolve. The JSON
/// conveniences and the `Serialize` impl ride the wire crate's `json`
/// feature, which the core enables.
pub use net_wire::event::StoredEvent;
