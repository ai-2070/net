...
#[tokio::test]
async fn review_pre_pin_stale_clock_cannot_reacquire_an_occupied_slot() {
    let dir = tempfile::tempdir().unwrap();
    let journal = journal_at(&dir.path().join("clock.json")).await;
    let mut terms = offer(true);
    terms.bounds.max_in_flight = 1;
    terms.reservation_ttl_secs = 1;
    let t = now_secs();
    let a = admit(&journal, candidate("clock-A", "adm-A", &terms, t), 1, t).await;
    let _b = admit(
        &journal,
        candidate("clock-B", "adm-B", &terms, t + 2),
        1,
        t + 2,
    )
    .await;
    let opened = journal.open_decision(&a, 1, t).await;
    eprintln!(
        "stale-time decision={opened:?}; occupied={}",
        journal.in_flight(SERVICE, t + 2).await.unwrap()
    );
    assert!(
        matches!(opened, Ok(DecisionOutcome::Busy)),
        "a sample captured before A expired reacquired capacity already owned by B"
    );
}
#[tokio::test]
async fn review_provider_resolution_refuses_ambiguous_incarnations() {
    let dir = tempfile::tempdir().unwrap();
    ...
    assert_eq!(
        journal
            .prune(t + terms.reservation_retention_secs + 1)
            .await
            .unwrap(),
        1
    );
    ...
    journal
        ...
        .await
        .unwrap();
    assert_eq!(journal.unresolved().await.unwrap().len(), 2);
    let resolved = journal
        .resolve(
            owner(),
            "same-key",
            TaskState::Failed {
                error: "refund specifically for quote-A".into(),
            },
            t,
        )
        .await;
    eprintln!(
        "result={resolved:?}; live={:?}; unresolved={:?}",
        journal.lookup(owner(), "same-key").await.unwrap(),
        journal.unresolved().await.unwrap()
    );
    assert!(
        resolved.is_err(),
        "an identity-less operator call must not silently select one of two financial records"
    );
}
struct ReviewSelectivePreflight {
    barrier: Arc<Barrier>,
}
#[async_trait::async_trait]
impl TaskPreflight for ReviewSelectivePreflight {
    async fn preflight(
        &self,
        _o: TaskOwner,
        _offer: &A2aOffer,
        ...
        if b.service.as_deref() == Some(SERVICE) {
            self.barrier.pass().await;
        }
        Ok(())
    }
...
struct ReviewRecorder {
    seen: Arc<parking_lot::Mutex<Vec<TaskBrief>>>,
}
#[async_trait::async_trait]
impl TaskExecutor for ReviewRecorder {
    async fn run(&self, b: TaskBrief, _cancel: CancelToken) -> Result<String, String> {
        self.seen.lock().push(b);
        Ok("artifact:done".into())
    }
}
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn review_operator_replacement_cannot_retarget_a_live_launch() {
    let provider = mesh().await;
    let caller = Arc::new(mesh().await);
    let who = TaskOwner::Peer(caller.node_id());
    let target = provider.node_id();
    let dir = tempfile::tempdir().unwrap();
    let journal = journal_at(&dir.path().join("launch.json")).await;
    let terms = offer(true);
    let t = now_secs();
    let a = brief("review-launch-replacement");
    let row = admit(
        &journal,
        AdmissionRecord::reserved(
            who,
    ...
    .await;
    journal
        .redeem(&row, "quote-A".into(), PAYER, t)
        .await
        .unwrap();
    let mut free = offer(false);
    free.service_id = "free-other".into();
    let barrier = Arc::new(Barrier::new(true));
    let seen = Arc::new(parking_lot::Mutex::new(Vec::new()));
    let serving = provider
        .serve_a2a_configured(
            TaskRegistry::new(),
            Arc::new(ReviewRecorder { seen: seen.clone() }),
            A2aServiceConfig::new(BTreeMap::from([
                (SERVICE.into(), A2aServicePolicy::Paid(terms.clone())),
                (
                    free.service_id.clone(),
                    A2aServicePolicy::Free(free.clone()),
                ),
            ]))
            .with_payment(HeldGate::new(false))
            .with_preflight(Arc::new(ReviewSelectivePreflight {
    ...
    let ready = PreparedTask {
        ...
        reservation: AdmissionReservation {
            ...
            admission_id: "adm-A".into(),
            commitment: row.commitment.clone(),
            purchase_hash: purchase_hash("adm-A", &row.commitment),
            capability: format!("{target}/net.a2a.task/{SERVICE}"),
            pricing_terms: terms.pricing_terms.clone(),
            expires_at: t + 300,
        },
    };
    let c = caller.clone();
    let pending = tokio::spawn(async move { c.submit_task_paid(&ready, &proof("quote-A")).await });
    barrier.await_entered(1).await;
    assert!(
        serving
            .store
            .lookup(who, &a.task_id)
            .await
            .unwrap()
            .unwrap()
            .deciding
    ...
    serving
        ...
        .resolve(
            ...
            &a.task_id,
            TaskState::Failed {
                error: "operator refunded A".into(),
            },
            now_secs(),
        )
        .await
        .unwrap();
    assert!(serving.store.forget(who, &a.task_id).await.unwrap());
    let b = TaskBrief::new("replacement B")
        .with_task_id(&a.task_id)
        .with_service(&free.service_id, "r1");
    assert!(matches!(
        caller.prepare_a2a(target, &b).await.unwrap(),
        PrepareReply::Reservation(_)
    ));
    barrier.release();
    ...
    let stored = serving
        ...
        .unwrap();
    eprintln!(
        "ack={ack:?}; executed={:?}; stored={:?}",
        seen.lock().as_slice(),
        stored
    );
    assert!(
        seen.lock().is_empty(),
        "A executed after its resolution while the journal describes replacement B"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn review_aborted_startup_writer_cannot_outlive_its_owner() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("startup.json");
    let terms = offer(true);
    let t = now_secs();
    let seed = journal_at(&path).await;
    let a = admit(&seed, candidate("old", "adm-A", &terms, t), 4, t).await;
    assert!(matches!(
        seed.open_decision(&a, 4, t).await.unwrap(),
        DecisionOutcome::Open(_)
    ));
    drop(seed);
    std::env::set_var("NET_A2A_REVIEW_OPEN_BARRIER", dir.path());
    let p = path.clone();
    let opening = tokio::spawn(async move { A2aAdmissionJournal::open(p).await });
    for _ in 0..500 {
        if dir.path().join("entered").exists() {
            break;
        }
        tokio::time::sleep(Duration::from_millis(5)).await;
    }
    std::env::remove_var("NET_A2A_REVIEW_OPEN_BARRIER");
    assert!(
        dir.path().join("entered").exists(),
        "startup reached publication barrier"
    );
    opening.abort();
    assert!(opening.await.unwrap_err().is_cancelled());
    let successor = A2aAdmissionJournal::open(&path).await;
    let exclusion_held = matches!(&successor, Err(A2aJournalError::OwnedElsewhere { .. }));
    if let Ok(next) = &successor {
        admit(next, candidate("new", "adm-B", &terms, t), 4, t).await;
        assert!(next.lookup(owner(), "new").await.unwrap().is_some());
    }
    std::fs::write(dir.path().join("release"), b"release").unwrap();
    for _ in 0..500 {
        if dir.path().join("finished").exists() {
            break;
        }
        tokio::time::sleep(Duration::from_millis(5)).await;
    }
    assert!(dir.path().join("finished").exists(), "old write finished");
    if let Ok(next) = &successor {
        eprintln!(
            "successor admitted before old publication; new record afterwards={:?}",
            next.lookup(owner(), "new").await.unwrap()
        );
    }
    assert!(
        exclusion_held,
        "cancelled startup released ownership while its writer was still live"
    );
}
