...
async fn review_reacquired_competitor(paid: bool) {
    let dir = tempfile::tempdir().unwrap();
    let store = journal_at(&dir.path().join("journal.json")).await;
    let mut terms = offer(paid);
    terms.reservation_ttl_secs = 1;
    let b = admit(&store, candidate("B", "adm-B", &terms, 100), 1, 100).await;
    let a = admit(&store, candidate("A", "adm-A", &terms, 102), 1, 102).await;
    assert!(matches!(
        store.open_decision(&b, 1, 104).await.unwrap(),
        DecisionOutcome::Open(_)
    ));
    assert!(
        matches!(
            store.open_decision(&a, 1, 104).await.unwrap(),
            DecisionOutcome::Busy
        ),
        "fresh-time positive control"
    );
    let stale = store.open_decision(&a, 1, 102).await.unwrap();
    let count = store.in_flight(SERVICE, 104).await.unwrap();
    eprintln!("paid={paid}, capacity={count}, stale verdict={stale:?}");
    assert!(
        matches!(stale, DecisionOutcome::Busy),
        "reacquiring old B did not advance the slot floor, so stale A also acquired"
    );
    assert_eq!(count, 1);
}
#[tokio::test]
async fn review_paid_capacity_survives_old_competitor_reacquisition() {
    review_reacquired_competitor(true).await;
}
#[tokio::test]
async fn review_free_capacity_survives_old_competitor_reacquisition() {
    review_reacquired_competitor(false).await;
}
