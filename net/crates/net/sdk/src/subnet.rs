...
// The subnet provisioning surface now lives in the CORE
// (`net::adapter::net::subnet::provision`) and is re-exported here
// verbatim — ONE definition, two reachable places (review-10 P1-7).
//
// It moved down because base `libnet`'s JSON constructor could not
// reach it: the core cannot depend on the SDK, so Go and C — which both
// receive their node from that constructor — had no way to declare
// subnet trust anchors, a security attachment, or a control channel at
// all. Nothing in it needed the SDK; it is pure conversion and
// validation over core authority types.
//
// This module keeps what genuinely belongs above the core: the `admin`
// verbs over a live node handle, the `Mesh::serve_subnet_exported`
// provider verb, and the cross-language fixture projection below.
pub use net::adapter::net::subnet::provision::{
    dto, fact_kind_wire, validate_subnet_authorities, NamedSubnetExport, NamedSubnetExports,
    SubnetExportAccess, SubnetProvisionError, LOCAL_PROVISION_KINDS,
};
