...
| Predicate `where:` header | `net_predicate_to_where_header` | Encode a predicate as the canonical `cyberdeck-where:` request-header pair (Phase 9b). Mirror of the Go SDK's `WhereHeader`; drops into a `request_headers`-shaped option list once a header-bearing call variant ships in `libnet_rpc`. Wire format pinned by `tests/cross_lang_capability/predicate_nrpc_envelope.json`. |
