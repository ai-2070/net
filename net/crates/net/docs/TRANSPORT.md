...
**Reroute.** When the failure detector marks a peer failed, `ReroutePolicy::on_failure` walks the affected destinations — identity-qualified: a *protected* candidate is affected iff it is bound to the failed peer (even at a drifted address), an *ordinary* candidate by address match — and resolves a new next-hop in this order. Every source excludes both the failed identity and the failed **transport** (address), since a routed peer records its relay's address and would otherwise be selected as an alternate onto the dead path:

1. `RoutingTable::lookup_alternate_excluding(dest, exclude_addr=failed_addr, exclude_identity=failed_node)` — the lowest-metric live candidate that is neither.
2. `ProximityGraph::path_to(dest)` — BFS over the topology graph. Returns the first hop of a path that isn't the failed node, isn't on its address, and is a direct peer of ours; prefers a hop whose forward/reverse indexes agree, so protected recovery lands on one that can carry an identity.
3. Any direct peer that is neither the failed identity nor on its address — last-resort fallback. Best-effort; if it can't reach the destination, the failure detector will catch it on the next cycle.

Both the reroute and the later restore are **compare-and-set** writes (`RoutingTable::observe` → `install_if_unchanged`): selection runs asynchronously from an event callback, so an unconditional write would let a stale decision clobber a route installed while the selection ran. Recovery additionally records the generation its own alternate produced and restores only while that exact state is current, and a peer-incarnation probe abandons any decision whose subject session has since been replaced. The original `next_hop` is preserved in `saved_routes` so `on_recovery` can restore the pre-failure route when the failed peer comes back.
