...
- [ ] Verdict mapping from the rippled `tx` method (lookup by hash) — tightened per review to a closed rule, so no unexpected result code falls through:
...
- [ ] Tests on the scripted-RPC fixture idiom — the review's row list, XRP-only set:
