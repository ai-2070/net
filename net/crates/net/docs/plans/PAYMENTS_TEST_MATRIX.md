...
**Implements:** the 2026-07-08 test-matrix brainstorm — five tiers proving *composition*, not every unit path. (The brainstorm was an internal design artifact, `docs/BRAINSTORMING.md`, deliberately kept local via `.gitignore`; its scope is inlined below in full, so this document stands alone.) Surveyed against the actual suites (~180 payments-relevant tests), the matrix is **~85% built**; this document is therefore two things at once: the **ledger** (invariant → test-name receipts, auditable cell by cell) and the **burn-down** of the genuine gaps (M1–M5).
...
| retry appends no duplicate; id bound to event scope (the 2026-07-06 review's M8 fix — distinct from this doc's M-numbered gaps) | `billing_stream::idempotent_retries_do_not_duplicate_log_records`; `core/billing_event::an_idempotency_key_from_a_foreign_scope_is_rejected` |
...
- [x] One test, impossible to regress, composing the company-level loop **across two real MeshNodes with the real engine gate** (no scripted gates): start provider → publish priced tool (`serve_tool_paid` + `EngineToolPaymentGate`, mock facilitator) → caller **discovers `pricing_terms` over the wire** (fetches the descriptor via `tool.metadata.fetch`, asserts it round-trips the announced terms, and pays *that* — so a discovery/publishing regression fails the test) → unpaid invoke → ERR_PAYMENT + `missing_quote` schematic → pay via `CallerPaymentFlow` → invoke serves → replay same quote → `already_redeemed` (+ `prior_payment=consumed`) → same quote on wrong tool → `wrong_tool_binding` → assert handler count == 1 AND billing count == 1 AND billing signature verifies caller-side.
