...
- **Gravity migration controller.** PR-5a ships `should_migrate_blob_to` as a primitive; PR-2b's `Artifact::Blob` gates placement scoring. PR-5i ships the `prefetch` action a per-node migration would call. But the gravity layer (Phase 4) only emits chain heat today — no blob-heat counter family, no migration state machine that consumes heat to decide "this blob should migrate to that target." Phase 4b. Sequencing: (a) add `BlobHeatRegistry` keyed on `[u8; 32]` with a `MeshBlobAdapter::fetch` bump; (b) emit `heat:blob:<hex>=<rate>` tags on the gravity tick; (c) controller that observes heat tags + runs `should_migrate_blob_to` + calls `adapter.prefetch` on the chosen target.
- **Cross-binding fixtures (T-5).** Python / Node / Go binding wrappers exist for the v0.15 `BlobAdapter` shape but not for the v0.2 `MeshBlobAdapter` + `publish_with_blob` + `BlobRefcountTable` + `BlobMetrics` + `prefetch` surfaces. Useful follow-up; not load-bearing for the substrate.
...
- **Multi-node prefetch e2e.** PR-5i's prefetch tests use a `RecorderAdapter` mock at the unit level. A 3-node DST proving A publishes + advertises → B admits via G-1 + calls `prefetch` → C is the actual chunk holder + replicates to B is a useful follow-up — but requires a 3-node harness the current 2-node `dataforts_greedy_e2e.rs` doesn't express.
