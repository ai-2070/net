...
**All six phases (A through F) shipped behind the `meshdb` Cargo feature.** AST, planner, local + federated executors, wire protocol envelopes + dispatch hookup, lineage walks, hash + sort-merge joins (row-intrinsic + payload-keyed, all four kinds), Count / Sum / Avg / Min / Max / DistinctCountExact / PercentileExact aggregates, Filter via synthetic-tag PredicateWire eval, tumbling-on-seq windowing, and the single-node LRU result cache with pull-based capability-version invalidation are all in code. Phase B is now end-to-end: `FederatedMeshQueryExecutor` rides `SUBPROTOCOL_MESHDB` over the mesh's existing encrypted Net session via `MeshDbWireDispatcher` + `MeshDbServer`, with a `MeshNodeMeshDbSender` bridging the wire-side `MeshDbWireSender` trait to `MeshNode::send_subprotocol`. A two-real-MeshNode integration test (`tests/meshdb_subprotocol_wire.rs`) drains a `Latest` query through the encrypted wire end-to-end. The `LoopbackTransport` ships alongside for in-process unit / integration tests. Remaining open items are bounded follow-ups: HLL p=14 / T-Digest c=100 sketch implementations behind `DistinctCountHll` / `PercentileTDigest`, the `Resume` continuation-token surface (the wire shape ships but the server returns a typed `ExecutorError` until a consumer drives it), and language bindings for the federated executor + transport beyond the Python / Node / Go / C shims already shipped against the local executor.
...
- **Phase B — Time-travel end-to-end.** Replica-aware routing via `CausalClaim` parsing of the three `causal:` tag forms (Presence / Tip / Range); proximity-ordered targets with most-specific-claim wins; `HistoricalRangeUnavailable` with per-replica available-range hints. `MeshQueryExecutor` async trait + `LocalMeshQueryExecutor<R: ChainReader>` walks atomic plans against a pluggable `ChainReader`. Wire protocol (`SUBPROTOCOL_MESHDB = 0x0F00`, `MeshDbRequest::{Execute, Resume, Cancel}`, `MeshDbResponse::{Batch, End, Error}`, `ResultBatch`, `ContinuationToken`, unified bidirectional `MeshDbFrame` tag) + dispatch hookup on `MeshNode` via `MeshDbWireDispatcher` (sync, non-blocking `MeshDbInboundRouter::try_route` called from the mesh's hot path) + `MeshDbServer` (per-call tokio task drives the executor + ships `MESHDB_SERVER_BATCH_ROWS`-sized batches back over `MeshNode::send_subprotocol`) + `MeshNodeMeshDbSender` bridging `Weak<MeshNode>` to the wire-side `MeshDbWireSender` trait. `FederatedMeshQueryExecutor<T: MeshDbTransport>` fans atomic operators out with NoRoute failover; `LoopbackTransport` drives 3-node in-process integration tests, `MeshDbWireTransport` drives the real-wire path. Cooperative cancellation via `QueryHandle::cancel` + wire-side `MeshDbRequest::Cancel { call_id }` flips a per-call `Notify` on the server. (`executor.rs`, `protocol.rs`, `federated.rs`, `transport.rs`.)
...
**Remaining work** is bounded follow-ups, all consumer-driven: HLL p=14 / T-Digest c=100 sketch implementations behind `DistinctCountHll` / `PercentileTDigest` (the exact variants ship today); `MeshDbRequest::Resume` continuation-token plumbing on the server side (the wire shape ships but the server returns a typed `ExecutorError` until a concrete consumer drives partial-result resume); host-language binding surface for the federated executor + wire transport (Python / Node / Go / C bindings ship today against the local executor; the federated + wire-transport pair is one slice away). Net-new surfaces (push-down fold-on-relay; streaming activation of the watermark on Window-joined sub-trees; sub-plan caching) come with their respective consumers.
