...
**Phases A + B + C + D-1 shipped behind the `meshdb` Cargo feature.** The AST, planner, executor, wire protocol, federated executor, `fork-of:` lineage walks, and inner hash-join (row-intrinsic keys) are all in code; Phase D-2 (watermarks, outer joins, sort-merge strategy, payload-keyed joins) and Phases E–F (aggregates / caching + bindings) park until a concrete consumer workload activates them.
