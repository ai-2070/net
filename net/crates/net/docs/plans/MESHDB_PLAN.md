...
**Phases A + B + C + D-1 + D-2 + E-1 shipped behind the `meshdb` Cargo feature.** The AST, planner, executor, wire protocol, federated executor, `fork-of:` lineage walks, inner + outer hash-joins on row-intrinsic keys, and Count aggregates on row-intrinsic group keys are all in code; D-2's remaining work (watermarks, sort-merge strategy, payload-keyed joins), the rest of Phase E (Sum/Avg/sketches, Filter executor, Window operator), and Phase F (caching + bindings) park until a concrete consumer workload activates them.
...
- **Phase D-2 — Outer hash-joins (Left / Right / Full).** Extends the executor to all four `JoinKind`s. `JoinedRowPayload.left` becomes `Option<ResultRow>` so unmatched right rows (RightOuter / FullOuter) can be emitted with `left=None`. RightOuter is implemented as a swapped LeftOuter (build on right, probe with left, swap labels at encode time); FullOuter scans both sides for unmatched. Watermark activation, sort-merge strategy, and payload-keyed joins still defer to a concrete consumer.
- **Phase E-1 — Count aggregate on row-intrinsic group keys.** `OperatorPlan::AggregateCount { input, group_by: Option<JoinKeyMode> }` — leaf operator over an arbitrary sub-plan. The planner accepts `group_by` clauses of `[]`, `[origin]`, `[seq]`, or `[origin, seq]`; surfaces `PlannerError` for non-Count aggregate functions or payload-keyed group fields. The executor groups rows via a `BTreeMap` keyed on the encoded row-intrinsic key (deterministic order), then emits one sentinel `ResultRow` per group carrying a postcard-encoded `AggregateRowPayload { group, value: AggregateValue::Count }`. The federated executor delegates the inner sub-plan via the transport, then aggregates locally. Sum/Avg/sketches + Filter + Window all defer to a concrete consumer driving row-schema decisions.
