...
**Phases A + B + C + D-1 + D-2 shipped behind the `meshdb` Cargo feature.** The AST, planner, executor, wire protocol, federated executor, `fork-of:` lineage walks, and inner + outer hash-joins on row-intrinsic keys are all in code; D-2's remaining work (watermarks, sort-merge strategy, payload-keyed joins) and Phases E–F (aggregates / caching + bindings) park until a concrete consumer workload activates them.
