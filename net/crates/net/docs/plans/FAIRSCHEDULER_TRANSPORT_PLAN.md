...
- **Concurrent large-file transfers — diagnosed + fixed.** A fan-out sweep (`tests/transfer_concurrency.rs`) exposed two distinct bugs the single-stream tests never hit:
  ...
  2. **Aggregate in-flight was unbounded (commit `0006e5d6e`).** The tx window is per-stream (≈ a whole chunk), so the file-count fan-out doesn't bound bytes on the wire; N concurrent large files overflow what the single recv loop can drain → recv-buffer drops. Fixed: `fetch_dir` holds an 8 MiB aggregate in-flight byte budget (each file reserves ≈ its current chunk), so large files self-limit to a couple concurrent while small files run wide. `directory_transfer_many_large_files` (6 × 4 MiB) now transfers cleanly at the default fan-out.
  - **Remaining (follow-up):** retransmit recovery from a *transient* drop is still limited by the early sender-side stream close — a FIN + ack-tracked close is the fix for genuinely lossy links; the budget avoids the sustained-overflow regime where recovery would otherwise be needed. Callers issuing many concurrent `transfer_fetch_chunk` directly (not via `fetch_dir`) must bound their own concurrency.
