...
- **Concurrent large-file transfers — diagnosed + fixed.** A fan-out sweep (`tests/transfer_concurrency.rs`) exposed two distinct bugs the single-stream tests never hit:
  ...
  - **Remaining (follow-up, lossy links only):** on localhost/LAN the corrected window keeps aggregate in-flight under the recv buffer so there are no drops to recover. On a genuinely lossy WAN, a dropped packet needs retransmit, but `serve_chunk` closes the reply stream right after sending — tearing down the retransmit buffer before a NACK round-trip. A FIN + ack-tracked close (close only after the receiver has acked all bytes) is the fix; also verify the retransmit driver actually fires for these streams. Callers issuing many concurrent `transfer_fetch_chunk` directly (not via `fetch_dir`) must bound their own concurrency.
