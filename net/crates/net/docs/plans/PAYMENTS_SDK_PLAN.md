...
net.billing.event@1
net.payment.dispute@1      # reserved: flag-only lifecycle extension, not P0 — named early because disputes touch billing/reorg/optimistic modes, but no dispute semantics ship before P5
```
...
**P2+ — additional rails:** demand-driven adapters (further chains, payment-intent references, Open Banking / bank-payment references through licensed providers where applicable, private ledgers). Each new rail is an adapter passing the same suite, never a new product worldview. Company-held prepaid credits only via licensed partners.
