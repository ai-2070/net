...
- **Verification confidence is a tier, not a boolean — and the tier vocabulary is a fixed protocol enum:** `observed | confirmed(n) | final`, canonical across all networks; adapters map their chain semantics *into* it (Solana commitment levels, EVM confirmations, XRPL validation) rather than exporting chain-specific states into policy. Facilitator receipt → `observed`, full stop — the v2 spec gives facilitators no way to report finality; `confirmed(n)` and `final` both come from the independent on-chain check of the tx hash (P1's chain checker). Policy picks per capability — the facilitator never has to be in anyone's trust root.
...
**P1 — real networks (config, not code):** `x402/base` (USDC, first real-money target), `x402/solana` (SPL-USDC, live facilitators), `x402/xrpl` (facilitator verified live at P1 start — t54; the conditional go/no-go is recorded in `PAYMENTS_P1_NETWORK_LADDER.md`). Facilitator trust is a named dependency: established defaults, self-hosted supported, facilitator identity/endpoint recorded in every verification result. Two-way door: x402-speaking agents pay Net capabilities; Net agents pay external x402 APIs with the same objects — zero translation. **"Config, not code" is an acceptance criterion:** enabling a P1 network means adapter/facilitator config + registry entries + conformance runs — no new envelope types, no core changes, no per-network branches outside `src/x402/`. If a network needs code, that's a design failure that goes to review, not a quiet exception.
...
- **Lifecycle hooks (the enterprise surface — Net ships zero dashboards):**
  ...
  - Delivery: in-process adapter interface only. Net calls registered adapters with signed lifecycle events from the local log. Built-in adapters are limited to test/dev examples; production sinks are customer/partner code. Net is not responsible for delivery semantics, retention, or downstream integrations. Predicate-filtered subscriptions (capability, counterparty, amount threshold, event kind) ride the existing predicate machinery.
  - **Adapter invocation contract (pinned now, not discovered in an incident):** every adapter call carries the signed event byte-preserved plus `event_id` + event-log sequence (the dedupe key), `hook_point`, and `subscription_id`. Everything downstream of the adapter — delivery ids, attempt counters, acks, retries, dead-letters — is the adapter author's domain, not Net's; at-least-once fan-out means duplicates *will* happen there, and sinks dedupe on event identity, never delivery identity. Every downstream failure recovery ends the same way: replay from the log.
  - **Subscriptions are versioned config objects** (`net.hook.subscription@1`): predicates, accepted event versions, registered adapter id. A subscription names an adapter, never a sink or a credential. Auditable, exportable, diffable — enterprises will ask "what changed in our hook config last month" and the answer is an object history, not a shrug.
  - **Hooks never block the payment path:** adapter dispatch is asynchronous off the local log; a slow or failing adapter stalls its own projection, never settlement, serving, or the log itself. Net invokes adapters in log order **per key (per quote/settlement chain), not globally** — consumers must not infer causality from arrival order across keys; the event-log sequence is the causal truth. There is no Net-side durable delivery queue and no dead-letter: an adapter that fell behind or died catches up by replaying from the stream.
  - **Adapter credentials follow the forwarding doctrine:** Net-side objects and logs carry no sink credentials — whatever secrets a production adapter needs (webhook auth, Kafka SASL, …) are the customer's own secret handling, outside Net. The shipped test/dev example adapters demonstrate the required pattern: `secret_ref` names in adapter config, never credential material in subscription objects, logs, or agent-visible surfaces.
