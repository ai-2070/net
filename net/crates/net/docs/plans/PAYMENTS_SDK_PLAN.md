...
2. **One payment policy engine, one implementation.** Rail config, wallet references, quotes, intents, verification, spend limits, billing emission, audit — implemented once in the Rust SDK, state in the shared locked per-user store (same regime as pins/consent; there is no daemon). Every embedded node on a machine enforces the same policy over the same state; CLI/Hermes/OpenClaw/shim are views. Enforcement is two-sided: the **caller's** node applies spend policy before anything leaves; the **provider's** node enforces payment requirements before its handler fires.
