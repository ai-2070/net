...
**Also resolved (Revision 2 — second-pass review):** every open question from the first Revision 2 pass has now been confirmed. v0.3 scope is locked.

- ~~CDC parameter presets vs raw knobs~~ → **Decision: raw `avg / min / max` only**. Presets age poorly because workload hints are subjective; CDC boundary choice is content-driven, not workload-driven. `ChunkingStrategy::cdc_for_workload(WorkloadHint::Logs)` is pure sugar that can land in v0.4 / v0.5 once real operator preferences surface; v0.3 ships raw knobs + docs.
- ~~Erasure-coded chunk channel naming~~ → **Decision: keep uniform at `dataforts/blob/<hex32>`** for data and parity alike. Splitting (`dataforts/blob-parity/<hex32>`) would break path-traversal uniformity, double the rule set, asymmetrize refcount logic, and complicate GC — for marginal value. Operators distinguish role via observability (metrics, `net blob tree`, future `net blob stripe`), not filesystem layout.
- ~~Per-channel default encoding~~ → **Decision: per-call only in v0.3, channel-level default deferred to v0.4**. RS encoding is a per-blob storage policy, not a channel-inheritance concern — blobs on one channel may legitimately differ in retention, durability tier, hot/cold pattern, and cost/value tradeoff. v0.4 adds the channel default once v0.3 deployments express a real preference; until then the `default_encoding` referenced in the cluster-fencing section is **not** a v0.3 surface (Phase C does NOT ship it).
- ~~Manifest cache shared with greedy LRU~~ → **Decision: separate caches in v0.3, never merge prematurely**. Chunk LRU value type is 4–64 MiB raw bytes; manifest LRU value type is 1–8 KiB `TreeNode`. Eviction patterns differ radically — manifest trees have strong locality (repeated walks of the same blob); chunk hotness is bursty and short-lived. Merging produces pathological churn. Merge candidate for v0.5+ only if operators hit memory ceilings AND the cache designs converge.
- ~~Staging-record TTL default~~ → **Decision: 7 days default, per-channel override via `RedexFileConfig::blob_staging_ttl`**. 7 days matches offline-node recovery windows, repair cycles, replication backfills, operator migration workflows, and disaster-recovery SLAs. 24 h is appropriate only for hyper-reliable clusters with no long-running backfill; channels for fast ephemeral pipelines can tighten to 1–2 days via the override.
- ~~Auto-repair on degraded-stripe detection~~ → **Decision: manual-only via `net blob repair` in v0.3, auto-repair deferred to v0.4** with full safety model. Auto-repair in v0.3 would be dangerous: interacts with replication budget, disk-pressure gates, GC; could trigger thundering-herd during outage recovery; needs multi-node coordination, a dedicated "repair budget" separate from "replication budget", rate-limiting, and fencing. v0.4 ships those primitives explicitly. v0.3's `blob_stripe_degraded_gauge` lets operators wire their own bounded auto-repair via the CLI in the meantime.

**No open questions remain for v0.3.** The plan is fully locked; implementation can proceed against the four-phase breakdown. Future questions will surface during Phase A's first integration test against real workloads.
