...
- **Source converters.** Two patterns:
  - **Push-via-observer.** `DaemonRegistry` → `MeshOsDaemonLifecycleSink` (Phase B's daemon supervision feed) and `ReplicationCoordinator` → `MeshOsReplicaTransitionSink` (Phase C's replica feed). Generic `DaemonLifecycleObserver` and `ReplicaTransitionObserver` traits sit on the substrate side; MeshOS-side sinks adapt to the event loop. Right fit when the signal is sparse (handful per day) and latency matters. (`sources.rs`.)
  - **Pull-via-tick.** `ProximityGraph` → `ProximityGraphLocalityProbe` (per-peer RTT) + `ProximityGraphHealthProbe` (per-peer Healthy/Degraded/Unreachable from edge staleness). Probes installed via `ProbeRegistry` (shareable cell) are polled by the loop on every Tick, BEFORE reconcile, so the freshest fold drives the diff. Right fit for hot-path-heavy signals + bounded coalescing. `[u8; 32] ↔ u64` id-bridge via `mesh::graph_id_to_node_id` (first 8 bytes little-endian). (`probes.rs`.)
- **Phase D-1 scheduler — continuous rebalancing.** `PlacementScorer` trait + `SchedulerRegistry` + reconcile arm. Leader-only by construction; per-chain it scores each holder, picks the worst, and emits `RequestEviction` when score < `score_floor` (default 0.5) AND best alternative ≥ score + `hysteresis_gap` (default 0.2) AND cooldown elapsed (default 5 min). Phase C's existing diff observes the holder-count drop and refills via `RequestPlacement` on the next tick — two-stage rebalance with no new action variant. Per-chain `last_rebalance` cooldown map on `MeshOsState` prevents flap. (`scheduler.rs`.)
- **Action-chain integration — Phase F's CortEX fold.** `ActionChainRecord` (serializable per-action wire form, postcard round-trip stable), `ActionChainAppender` trait the executor calls per dispatch / fail / gate outcome, `MeshOsSnapshotFold` (`impl RedexFold<MeshOsSnapshot>`) that decodes records and updates the snapshot's `recent_failures` ring buffer. `BufferingActionChainAppender` for tests; `NoOpActionChainAppender` for bootstrap. Executor wires via `with_chain_appender(...)` builder. (`chain.rs`.)
...
- **`MeshOsRuntime` stitching layer.** One-call entry point: `MeshOsRuntime::start(config, dispatcher) -> Self` spawns the loop + executor as tokio tasks and returns a live runtime with `handle()` / `handle_clone()`, `snapshot()` / `snapshot_reader()`, `executor_stats()`, and `shutdown()` / `shutdown_with_timeout()`. Source-converter helpers (`attach_to_daemon_registry`, `attach_to_replication_coordinator`) plug into the runtime's handle. (`runtime.rs`.)

**150 unit tests in the `meshos` module + 11 end-to-end pipeline integration tests + 13 daemon-registry + 9 daemon-trait + 15 replication-coordinator tests** (the latter three groups carry observer-hook regressions). `clippy --features meshos --lib --tests -- -D warnings` clean. `RUSTDOCFLAGS="-D warnings" cargo doc --features meshos --no-deps --lib` clean. Substrate builds clean both with and without `--features meshos`.

**Remaining work** is bounded follow-ups, all consumer-driven:

- **`PlacementFilter`-backed `PlacementScorer` impl.** The scheduler trait + reconcile arm are in code; production wires `PlacementFilter` behind the trait. Same shape as the proximity / health probe wiring — a constructor takes the scorer's substrate dependency, the trait impl converts to MeshOS scores.
- **Real `ActionChainAppender` against a substrate RedEX chain.** `chain.rs` ships the trait + the fold; production wires the appender to write records to a chain that the `MeshOsSnapshotFold` consumes on each node. Today's defaults (`NoOpActionChainAppender`) make this opt-in.
- **Migration orchestrator atomic-swap action variant.** Phase C's two-stage rebalance (RequestEviction + Phase C-driven refill) is acceptable as a first cut; an atomic-swap action variant would tighten the under-replication window if a workload demands it.
