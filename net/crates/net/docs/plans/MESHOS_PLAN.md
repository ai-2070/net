...
**121 unit tests in the `meshos` module + 11 end-to-end pipeline integration tests + 13 daemon-registry + 9 daemon-trait + 15 replication-coordinator tests** (the latter three groups carry observer-hook regressions). Substrate builds clean both with and without `--features meshos`.

**Remaining work** is bounded follow-ups, all consumer-driven:

- **Proximity / heartbeat source converters.** Both follow the same `Observer + sink` pattern as the daemon and replication sources, with a pinned `[u8; 32] ↔ u64` id-bridge convention (`mesh::node_id_to_graph_id` / `graph_id_to_node_id` — first 8 bytes little-endian). The design choice still in front: push-via-observer (low-latency, hot-path exposure) vs pull-via-tick-poll (coalesced sampling, no hot-path impact). Best driven by a real locality-aware consumer.
