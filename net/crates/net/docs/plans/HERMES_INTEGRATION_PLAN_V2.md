...
>
> **⚠ Security-model note (deviation to close):** `serve_enrollment` currently **auto-admits any valid single-use invite** (invite-as-authorization). The plan's stated property — *"a leaked invite lets someone only ask, visibly, never admits them"* — needs an **operator-approval hook**: route the arriving `JoinRequest` to the operator's surface (Telegram/desktop), await an approve/deny, and only then sign. That hook is the immediate next step (it's the SDK side of "approval routing") and turns the model from invite-as-grant into invite-as-permission-to-ask.
>
> **Remaining (frontends + approval):** the **operator-approval hook** on `serve_enrollment` (above); the **`net mesh …` CLI** / **PyO3** / **plugin** frontends (both-or-neither); **device-tier proxy**; and **PSK distribution** for a real cross-host join (the mesh PSK is an out-of-band build-time property today — the loopback test shares a constant). **Infra:** the `<90s` enroll + revoke-denies-on-next-invoke 2-machine acceptance, LAN discovery.
