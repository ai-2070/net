...
>
> **Remaining (frontends):** the **`net mesh …` CLI** / **PyO3** / **plugin** frontends (both-or-neither) — including wiring the `approver` to Hermes's Telegram/desktop approval surfaces; **device-tier proxy**; and **PSK distribution** for a real cross-host join (the mesh PSK is an out-of-band build-time property today — the loopback tests share a constant). **Infra:** the `<90s` enroll + revoke-denies-on-next-invoke 2-machine acceptance, LAN discovery.
