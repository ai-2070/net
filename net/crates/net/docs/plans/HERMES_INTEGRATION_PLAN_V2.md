...
>
> **Remaining (frontends):** wiring the **`approver`** to Hermes's Telegram/desktop surfaces (a Python-callback `serve_enrollment` binding — the async-callback follow-up to the auto path); the **`net mesh …` CLI**; the **Hermes plugin** meta-tools over the above; **device-tier proxy**; and **PSK distribution** for a real cross-host join (the mesh PSK is an out-of-band build-time property today — the loopback tests share a constant). **Infra:** the `<90s` enroll + revoke-denies-on-next-invoke 2-machine acceptance, LAN discovery.
