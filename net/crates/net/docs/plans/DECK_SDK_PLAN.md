...
**Phases 4–7 (Python / Node / Go / C) not started.** No binding has any Deck-SDK surface — no `bindings/python/src/deck.rs`, no `bindings/node/src/deck.rs`, no `bindings/go/deck-ffi/`, no `include/net_deck.h`. The MeshDB / compute precedents per binding give the FFI shapes the Deck-SDK binding rides on top of (last-error trio, discriminator parsing, channel-pumping goroutine, AsyncIterable shim). The §2–§5 sections are written against the actual Rust surface today, not the original sketch.
...
- **Phase 4 — Python SDK. NOT STARTED.** No `bindings/python/src/deck.rs` yet. pyo3 cdylib + `sdk-py/src/net_sdk/deck.py` wrapper. Slice 1: `DeckClient` + snapshot/status streams + `AdminCommands` (sync iterator protocol). Slice 2: `AuditQuery` + `subscribe_logs` + `subscribe_failures`. Slice 3: `IceCommands` with typestate enforced by hiding `commit` on `IceProposal` + `OperatorIdentity` / `OperatorRegistry` wrappers.
- **Phase 5 — Node / TypeScript SDK. NOT STARTED.** No `bindings/node/src/deck.rs` yet. napi-rs cdylib + `sdk-ts/src/deck.ts` AsyncIterable shim. Slice 1: client + admin + snapshot/status. Slice 2: audit + logs + failures. Slice 3: ICE typestate via two distinct classes.
- **Phase 6 — Go SDK. NOT STARTED.** No `bindings/go/deck-ffi/` cdylib yet. cdylib + `bindings/go/net/deck.go` wrapper. Slice 1: client + admin + snapshot. Slice 2: audit + logs + failures. Slice 3: ICE typestate via separate `IceProposal` / `SimulatedIceProposal` types.
- **Phase 7 — C SDK. NOT STARTED.** No `include/net_deck.h` yet. Vtable+callback for streams; opaque pointers for `DeckClient` / `IceProposal` / `SimulatedIceProposal` / `AuditQuery`; last-error trio mirroring `net_meshdb.h`. Runnable `examples/deck.c`. Smallest phase — mostly the cdylib's C-export surface that Phase 6 already requires.
