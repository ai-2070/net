...
>
> **Slice B — carry the chain on the invoke + provider audit (the headline, deferred; one design fork).** Attach the `TokenChain` to the nRPC invoke and have `WrapInvokeHandler` verify it (root-anchor to the owner, unrevoked, leaf == caller) and **audit the terminal subject** — this is what "every invocation carries the chain; remote wrapper audit shows which Hermes/subagent" delivers, and it's a cross-cutting wrap-protocol + identity-model change needing 2-machine validation. **Fork to decide:** where the chain rides (RPC header vs body envelope) and whether the gateway *operates under* the gateway identity (changing its `origin_hash` / owner-scope keying) or keeps the node identity with the chain as an attached attestation. Slice A does **not** change the node's operating identity or touch the wire — so it's additive and independent of this fork.
