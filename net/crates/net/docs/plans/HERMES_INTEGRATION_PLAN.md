...
> **Phase 2 status (2026-07-05) — the subscription blocker is closed and diff-based promotion works; NOT a full Phase-2 sign-off (phase-owned acceptance items remain — see below).** **Landed + tested:** the "no pin-change subscription" blocker is closed — `PinStore::watch` / `snapshot_and_watch` (SDK, feature `pin-watch`) is a real OS file watcher over the machine-shared store — it catches the atomic temp+rename AND a cross-process `net mcp pin approve`, debounced, emitting `PinChange{added, removed}` deltas (Rust test + Python `AsyncPinStore.snapshot_and_watch` / `AsyncPinWatcher`, tested). `integrations/hermes/pins.py` consumes it: promote the snapshot, then per delta describe the cap → register a real async `ToolEntry` (live schema, risk-tagged description, deterministic name) via `tools.registry` (as `mcp_tool.py` does), retire on removal — **diff-based, event-driven, no polling** (a cross-process approve promotes within ~1s). Consent still runs on every invoke, so a revoked pin fails closed. Promotion starts per-session (`on_session_start`). 3 pin tests + the full plugin suite green; the real-Hermes-loader check still passes. This satisfies the "pinned cap appears as a typed tool" and "pin approved cross-client is visible" acceptance clauses.
>
> **Phase-2 acceptance items still OUTSTANDING (why this isn't a full sign-off):** (1) **SDK-side canonical name allocation** — the Phase-2 checklist requires "pinned tool names are allocated by the shared pin store (SDK-side) and stable / collision-handled"; today names are deterministic *client-side*, so cross-frontend name agreement (Hermes ↔ shim ↔ OpenClaw) isn't SDK-guaranteed. (2) **`dynamic_schema_overrides` wired to the live descriptor** — this IS the acceptance criterion "descriptor update on the remote side propagates to the model-visible schema without restart"; the schema is captured at promote time, so a remote descriptor change is NOT yet reflected live. (3) **Audit refs** on structured results/logs (a Phase-2 checklist item; overlaps the Phase-3 delegation-chain-id work). (4) The **arg-accuracy** and **live cross-frontend descriptor-propagation** acceptance measurements need a **running-Hermes + live remote provider** (2-machine infra). Phase 2 is not "complete" until (1) and (2) land — they are phase-owned, not later-ring refinements.
