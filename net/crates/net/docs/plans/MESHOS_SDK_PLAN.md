...
**Phases 2–5 underway, partial parity.** All four bindings now exist and ship the slice-1 daemon-author surface (register / control-event receive / publish_log / graceful_shutdown), the slice-1b vtable / cgo-export daemon-callback bridge (Go + C), and the slice-2 audit-related extras (audit query / log stream / failure stream on the Deck side; capability conversion on the MeshOS Node side). The remaining gap to "v1 done" — the metadata + capability surface for Go/C, async wrappers for Python, `health`/`saturation`/`requirements` JS routing for Node, plus `MaintenanceStateView` decode + `context.Context` plumbing for Go — is enumerated under **Remaining work** below.
...
- **Phase 2 — Python SDK. SLICE 1 SHIPPED; SLICE 2 PARTIAL; SLICE 3 NOT STARTED.** `bindings/python/src/meshos.rs` ships `PyMeshOsDaemonSdk` + `PyMeshOsDaemonHandle` + `PyMeshOsDaemon` (the trait wrapper, callback-routed via `Python::with_gil`); `sdk-py/src/net_sdk/meshos.py` adds the context-manager + protocol class. Slice 1 (lifecycle + control + log + shutdown) **shipped**. Slice 2 — `MaintenanceStateView` decode + snapshot/restore **shipped**; `requirements()` / `CapabilityFilter` routing **stubbed** (the wrapper returns `CapabilityFilter::default()` rather than the daemon's advertised requirements). Slice 3 — async wrappers via pyo3-asyncio **not started**.
- **Phase 3 — Node / TypeScript SDK. SLICE 1+2 SHIPPED; SLICE 3 PARTIAL.** `bindings/node/src/meshos.rs` + `sdk-ts/src/meshos.ts` ship the cdylib + AsyncIterable wrapper. Slice 1 (lifecycle + control via `controlEvents()` AsyncIterable + log + shutdown) **shipped**. Slice 2 — capability conversion **shipped** (real `CapabilitySet` round-trip). Slice 3 — typed `MaintenanceStateView` matchers + error discriminator **shipped**; `health()` / `saturation()` JS callbacks **stubbed** (bridge returns `MeshDaemon::health/saturation` defaults rather than routing to the JS daemon); `requiredCapabilities` / `optionalCapabilities` on the JS daemon object **stubbed** (`CapabilitySet::default()` is hardcoded).
- **Phase 4 — Go SDK. SLICE 1+1b SHIPPED; SLICE 2+3 NOT STARTED.** `bindings/go/meshos-ffi/src/lib.rs` + `bindings/go/net/meshos.go`. Slice 1 (register / control / log / shutdown via lifecycle-only no-op daemon) **shipped**. Slice 1b — `MeshOsDaemon` interface + vtable-based cgo `//export` trampoline bridge (process / snapshot / restore / on_control / health / saturation) **shipped**. Slice 2 — `Metadata() *MetadataView`, `PublishCapabilities(CapabilitySet)`, `ControlEvents() <-chan DaemonControl` pumping goroutine **not started**. Slice 3 — `MaintenanceStateView` decode + `context.Context` plumbing on every blocking call **not started**.
- **Phase 5 — C SDK. SLICE 1+1b SHIPPED; SLICE 2+3 NOT STARTED.** `include/net_meshos.h` + `bindings/go/meshos-ffi/` cdylib. Slice 1 (`net_meshos_sdk_start` / `net_meshos_register_daemon` / `net_meshos_next_control` / `net_meshos_publish_log` / `net_meshos_graceful_shutdown` + the last-error trio) **shipped**. Slice 1b — `NetMeshOsDaemonVtable` (process / snapshot / restore / on_control / health / saturation) + `net_meshos_register_daemon_with_vtable` + `net_meshos_process_emit` / `net_meshos_snapshot_emit` helpers **shipped**. Slice 2 — `net_meshos_metadata` + `NetMetadataView` / `NetMaintenanceStateView` / `NetPeerSnapshot` + `net_meshos_publish_capabilities` + `NetCapabilitySet` **not started**. Slice 3 — folded into slice 2 for C (no separate ctx surface).
...
**Phase 2 (Python) — slice 2 finishing + slice 3:**
- Route `MeshOsDaemon.required_capabilities()` / `optional_capabilities()` through the bridge instead of `CapabilityFilter::default()` (`bindings/python/src/meshos.rs:~404` carries the slice-1 stub comment).
- Slice 3 — pyo3-asyncio `async def anext_control() -> DaemonControl | None` + `async for ev in handle:` against `tokio::sync::mpsc::Receiver`.

**Phase 3 (Node) — slice 3 finishing:**
- Route the JS `daemon.health()` / `daemon.saturation()` callbacks through the bridge (`bindings/node/src/meshos.rs:~514-572` currently returns substrate defaults).
- Route the JS `daemon.requiredCapabilities()` / `daemon.optionalCapabilities()` through the bridge (same slice-1 stub comment).

**Phase 4 (Go) — slice 2 + slice 3:**
- Slice 2: `MetadataView` Go struct + `Metadata()` accessor on `MeshOsDaemonHandle`; `PublishCapabilities(CapabilitySet) error` (stub-passthrough until substrate commits); `ControlEvents() <-chan DaemonControl` pumping goroutine that closes on shutdown/ctx-cancel.
- Slice 3: `MaintenanceStateView` Go decode (tagged enum); `context.Context` plumbing on `StartMeshOsDaemonSdk`/`Shutdown`/`NextControl`/`GracefulShutdown`.

**Phase 5 (C) — slice 2:**
- `NetMetadataView`, `NetMaintenanceStateView` (tagged union), `NetPeerSnapshot` declarations in `include/net_meshos.h`.
- `const NetMetadataView* net_meshos_metadata(NetMeshOsDaemonHandle*)` exported by the cdylib (pointer valid for handle's lifetime).
- `NetCapabilitySet` + `int net_meshos_publish_capabilities(NetMeshOsDaemonHandle*, const NetCapabilitySet*)` (stub-passthrough until substrate commits).

**Cross-cutting (all bindings):**
