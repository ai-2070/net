...
- **Phase 1 (partially landed):** secret store + audit (`net secret set`, policy parser, `net security audit` surface) — useful independent of forwarding, no forwarding yet.
  - **Landed:** the caller-side **policy** store + audit surface as `net_mcp::forward::ForwardingStore` / `ForwardingAudit` (`adapters/mcp/src/forward/store.rs`) — persistent (atomic, 0600, advisory-locked, corrupt≠reset), write-time-validated mutations, and a value-free audit view + render. It records destination bindings, never secret values.
  - **Remaining (needs a decision):** (1) the **secret value backend** — where the actual bytes live at rest (OS keychain vs encrypted file vs plaintext-0600); this is a security-posture choice, so it's deferred rather than defaulted. (2) The **CLI wiring** for `net secret set/list/rm` and `net security audit` (note an `audit.rs` / `cap.rs` command group already exists to reconcile with).
