...
- **Phase 2 (started):** sealed context + caller injection + destination accept-lists + conformance tests, for native HTTP-facing capabilities.
  ...
  - **Remaining (gated on the AEAD decision):** the *real* `ForwardedContextSealer`/`Opener` — encrypt sealed to the destination node's key. The SDK exposes no sealing primitive, so this is a fork: **(A)** grow a public SDK sealing primitive over the core's in-tree XChaCha20-Poly1305 (doctrine H6 — the principled path; reuses proven crypto but touches `net-mesh-sdk`/core and needs destination-key lookup via mesh identity), or **(B)** a direct crypto crate in the adapter (`chacha20poly1305` is already in the workspace lock, but a direct crypto dep bends the adapter's "SDK + serialization only" doctrine). Then caller-side context assembly + injection into native HTTP capabilities, and the full sentinel/replay/exfil/cross-caller conformance suite.
