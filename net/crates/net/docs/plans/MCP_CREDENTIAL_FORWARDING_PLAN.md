...
- **Phase 2 (started):** sealed context + caller injection + destination accept-lists + conformance tests, for native HTTP-facing capabilities.
  ...
  - **Remaining — mesh key distribution (a security-architecture fork, needs a decision):** the sealer/opener take X25519 keys as inputs today. The SDK already exposes the ed25519 identity (`Identity::to_bytes()` = secret seed, `entity_id()` = a peer's public key), and **core has no ed25519→x25519 conversion**, so wiring keys is a fork, each with real cost: **(1)** derive the forwarding X25519 keypair from the ed25519 identity via the libsodium-style ed25519→curve25519 conversion — no distribution needed (a caller derives the destination's sealing pubkey from its known `entity_id`), but it introduces new conversion crypto in the adapter and **reuses the identity key** for signing *and* sealing (a sanctioned pattern with cross-protocol caveats); or **(2)** a domain-separated forwarding-only X25519 key (KDF of the seed) — clean key separation, but the pubkey must be **published in the node's announcement**, an announce/discovery change. Then **injection** into native HTTP capabilities (a different subsystem) and the full sentinel/replay/exfil/cross-caller conformance suite.
