...
- **Phase 1 (partially landed):** secret store + audit (`net secret set`, policy parser, `net security audit` surface) — useful independent of forwarding, no forwarding yet.
  ...
  - **Remaining:** the `net secret set` value-entry verb (writes to the keychain backend behind a `net-cli` `keychain` feature) — thin glue now that the backend exists. That completes Phase 1; sealing/injection is Phase 2.
