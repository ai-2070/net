...
- **Phase 1 (partially landed):** secret store + audit (`net secret set`, policy parser, `net security audit` surface) — useful independent of forwarding, no forwarding yet.
  ...
  - **Remaining (gated on one decision):** a **persistent** value backend to slot behind `SecretBackend`. The options each cost something: **(A)** OS keychain via the `keyring` crate — best posture, but a new platform dependency (dbus/Secret Service on Linux) with CI/system-lib implications; recommend gating it behind a non-default `keychain` feature so the default build stays clean. **(B)** an encrypted-at-rest file — pure-Rust and CI-safe, but the SDK exposes no AEAD, so it needs either a new crypto crate or an SDK primitive (doctrine H6) plus a key-management story (likely daemon-identity-derived). Once a backend exists, the **CLI wiring** (`net secret set/list/rm`, `net security audit`) is thin glue on top (reconcile UX with the existing `audit.rs` / `cap.rs`).
