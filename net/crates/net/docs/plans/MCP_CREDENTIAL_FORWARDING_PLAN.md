...
- **Phase 1 (partially landed):** secret store + audit (`net secret set`, policy parser, `net security audit` surface) — useful independent of forwarding, no forwarding yet.
  ...
  - **Landed (value backend, option A):** `KeychainSecretBackend` (`forward/keychain.rs`) behind the non-default **`keychain`** cargo feature — a `SecretBackend` over the OS credential store (macOS Keychain, Windows Credential Manager, Linux kernel `keyutils`). Every platform is a pure-Rust store with **no C system library**: the Linux backend is `keyutils` rather than Secret Service specifically because keyring's Secret Service path pulls `libdbus-sys` (`libdbus-1-dev`), which headless CI / minimal images lack; the trade-off is that a keyutils keyring persists per login session, not across reboots (Secret Service can be swapped in later where that matters). Keychain calls run off the async runtime; a value surfaces only as a `ForwardedHeaderValue`. The default build / CI / `cargo install` never compile it; a CI step build-checks the feature on Linux so it can't rot, and the round-trip test is verified against the real Windows Credential Manager (it self-skips where no keychain is present).
...
- **Phase 2 (started):** sealed context + caller injection + destination accept-lists + conformance tests, for native HTTP-facing capabilities.
  ...
  - **Landed (caller assembly):** `ForwardedContextBuilder` (`forward/assemble.rs`) completes resolve → assemble → seal — collect resolved sends, stamp a fresh nonce + short expiry, validate; round-trips end-to-end through the real cipher.
  - **Landed (key derivation, option 2):** `ForwardingKeypair` (`forward/keys.rs`) derives a node's forwarding X25519 keypair from its ed25519 identity seed (`Identity::to_bytes()`) via a domain-separated BLAKE2s KDF — **distinct from the signing key**, no key reuse, no second secret to store, no SDK change (the SDK already exposes the identity). Chose option 2 over the ed25519→curve25519 conversion (which reuses the identity key for signing *and* sealing) for clean key separation. Tested through the full distribution shape (derive → publish pubkey → seal → open with derived opener).
  - **Blocked on an HTTP-facing subsystem (not more self-contained work):** the two remaining pieces — **distribution** (publish the forwarding pubkey in the node's announcement + read it on discovery) and **injection** (the destination using the opened headers on a downstream call) — both need an **HTTP-facing capability**. The MCP adapter today is **stdio-only**, and forwarding never applies to stdio (the never-for-stdio doctrine), so there is no injection target here yet. This lands with **Phase 3** (remote/HTTP MCP wrapping) or native HTTP capabilities in another subsystem — wiring distribution before an injection consumer exists would publish a key nothing reads. Plus the full sentinel/replay/exfil/cross-caller conformance suite.
