...
8. **Loopback stays loopback.** The Net extension must never require binding the gateway WS beyond `127.0.0.1`. Cross-machine traffic is the daemon's job — that's the whole point, and it's the security property: federation without widening the gateway's attack surface.
...
| Risk | Mitigation |
|---|---|
| Upstream plugin API churn (4.8M LOC, fast-moving) | Everything in one external extension; only stable plugin-sdk surfaces; `minHostVersion` pinned; fork ports as insurance |
