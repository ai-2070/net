...
- [x] Python surface: signer *references* only — `payment_signer_address` + `payment_signer` (a callable receiving the full EIP-712 typed-data JSON and returning the `0x…`-hex signature; runs on a blocking worker, never the event loop). Private key bytes remain unrepresentable in bindings — key-invariant negative tests extended (no kwarg accepts key material). The payment identity is now the **node's mesh identity** (SDK `Mesh::entity_keypair()` accessor; replaces the ephemeral per-gateway keypair). TS follows when the node binding grows a payment flow — it has none today (demand-driven)
- [x] Solana `exact` (SPL presign) follows base, same trait: `SchemeSigner::sign_svm_transfer` (defaulted structured refusal) + `ExternalSvmSigner` — intent-in/blob-out: Net derives the typed [`SvmTransferIntent`] from the quoted requirements (spec-required `extra.feePayer`, memo ≤ 256 bytes) and the **wallet** builds + partially signs the versioned transaction (it owns the key, the SPL machinery, and the blockhash RPC — none enter Net); the spec-pinned payload is `{"transaction": "<base64>"}`
- [ ] xrpl presigned Payment blobs after conformance against t54 — **no xrpl scheme spec exists at the pinned commit** (verified 2026-07-06: twelve chains have `scheme_exact_*.md` specs; xrpl does not, so its payload shape is t54-vendor-defined today). The seam instantiates the same intent-in/blob-out pattern once a pinned shape exists — see the ladder record
...
- Node-identity-bound payment caller in the Python gateway (needs the SDK to expose the entity keypair; replaces the ephemeral per-gateway identity) — **landed**: `MeshNode::entity_keypair()` + `Mesh::entity_keypair()`; the gateway's quotes, spend tracking, and invocation bindings now ride the node identity
