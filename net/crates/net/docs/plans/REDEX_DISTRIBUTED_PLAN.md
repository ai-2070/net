...
- 🔜 Tokio runtime loop driving `tick` on an interval. Dispatches inbound `SyncHeartbeat` to `tracker.record_heartbeat`, applies `coordinator.transition_to(...)` for each `PendingTransition`, ships `OutboundMessage::Heartbeat` through the mesh.
- 🔜 Spawn / register / withdraw lifecycle wired into `Redex::open_file` for replicated channels — when `RedexFileConfig::replication.is_some()`, construct + register a coordinator + spawn the runtime loop; cleanup on file close.
