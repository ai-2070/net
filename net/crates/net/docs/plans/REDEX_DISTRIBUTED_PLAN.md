...
- 🔜 Mesh dispatch wiring — substrate-side router that translates inbound `SUBPROTOCOL_REDEX` frames to `Inbound::*` and ships them via `ReplicationRuntimeHandle::dispatch`. Production `MeshNode` impl of `ReplicationDispatcher` that routes outbound through `send_subprotocol`.
- 🔜 Spawn / register / withdraw lifecycle wired into `Redex::open_file` for replicated channels — when `RedexFileConfig::replication.is_some()`, construct the coordinator + spawn the runtime + register the handle in a per-Redex registry; cleanup on file close drives `cancel()`.
- 🔜 `file_provider` closure wired into `Inbound::SyncRequest` / `Inbound::SyncResponse` so the runtime can call `handle_sync_request` / `apply_sync_response` against the live `RedexFile`. Currently the runtime logs but defers; the closure piece lands with the lifecycle integration.
