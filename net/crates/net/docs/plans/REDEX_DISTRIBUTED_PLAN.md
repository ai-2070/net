...
- 🔜 Heartbeat loop on `heartbeat_ms` — tokio task that drives `HeartbeatTracker::record_heartbeat` from inbound `SyncHeartbeat` and routes `is_leader_silent → transition_to(Candidate, MissedHeartbeats)`.
