...
| 112 | `CapabilityIndex::find_nodes_scoped` two-pass query+rescope → single-pass `filter+scope` walk over the candidate set | Pre-fix this method ran `self.query(filter)` (which itself walks the candidate set and does a `self.nodes.get(node_id)` per candidate to re-validate against `filter.matches`), then iterated the result `Vec<u64>` a SECOND time doing per-node `self.get(node_id)` — which clones the `CapabilitySet` — just to read `caps.tags` for scope resolution. For a 1000-node query with 50 survivors that's 1000 + 50 DashMap probes plus 50 full `CapabilitySet` clones (`String` channel-id + `HashSet` tags + `Vec` tools fields are non-trivial to clone). Post-fix folds scope resolution into the same shard-lock guard that re-validates `filter.matches`: one DashMap probe per candidate, zero `CapabilitySet` clones. `query_count.fetch_add(1)` is bumped inline so the metrics observation is preserved. Pinned by `find_nodes_scoped_single_pass_matches_legacy_two_pass` — covers `Any` (both same-subnet predicates), `GlobalOnly`, and `SameSubnet` against an inline replica of the legacy two-pass implementation, so semantic drift in any branch of `matches_scope` surfaces immediately. |
