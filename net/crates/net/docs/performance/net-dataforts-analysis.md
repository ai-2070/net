...
| 172 | `MeshBlobAdapter::fetch(Manifest)` sequential chunk fetch → `buffered(16)` | The manifest bulk-fetch path now drives chunk fetches through `futures::stream::iter(chunks).map(...).buffered(MANIFEST_FETCH_CONCURRENCY)` with `MANIFEST_FETCH_CONCURRENCY = 16`. `buffered` (not `buffer_unordered`) preserves chunk order so the assembled `Vec<u8>` stays byte-correct, while letting up to 16 fetches be in flight concurrently. For a 1024-chunk replicated blob at 1 ms/chunk, sequential = ~1 s vs buffered(16) ≈ ~64 ms — 15× speedup per the doc's worked example. Concurrency cap kept small to avoid overrunning the substrate's per-channel credit window on partial-availability manifests. Error handling preserved: break-on-first-error (in-flight futures get dropped at stream-end). All 106 existing `dataforts::blob::mesh` tests still pass. |
| 173B | `MeshBlobAdapter::store(Manifest)` sequential chunk stores → `buffer_unordered(16)` | Symmetric with #172 on the upload path. Verification prepass (hash + size mismatch checks) hoisted into a synchronous loop so chunk poison aborts BEFORE any store fires — preserves the legacy "no chunks stored on caller-poisoned manifest" contract. Then `futures::stream::iter` + `buffer_unordered(MANIFEST_STORE_CONCURRENCY = 16)` drives parallel stores; chunks are content-addressed so order doesn't matter and in-flight stores that have already landed are idempotent on retry. Each store future captures an owned `Bytes` sliced from a single `Bytes::copy_from_slice(bytes)` of the input (one extra memcpy on the upload path; required to sidestep the higher-ranked lifetime check that the `&[u8]`-borrowing closure form trips). |
| 177 | `verify_manifest_chunks` two-pass (sum + hash) → one-pass with `checked_add` | The chunk-list walk that pre-fix first summed `chunks.iter().sum()` to validate total length and then re-walked to BLAKE3 each region is now one loop. Per-iteration `offset.checked_add(chunk.size)` catches wrap on a hostile manifest; an `end > fetched.len()` check rejects buffer-overrun before slicing (no `&fetched[offset..end]` panic). The post-loop `offset != fetched.len()` catches the trailing-bytes case the old first-pass `total != fetched.len()` caught. Same correctness — one walk over the chunks vec. Pinned by the existing `resolve_rejects_chunked_manifest_with_tampered_chunk_bytes` (per-chunk hash mismatch) and a new `resolve_rejects_manifest_when_fetched_buffer_is_short` (per-chunk bounds-check path that pre-fix's first sum-check caught). |
| 174 | `BlobRef::encoded_len` full re-encode for Manifest/Tree → `postcard::experimental::serialized_size` walk | Pre-fix called `self.encode().len()` for the Manifest and Tree variants, which postcard-encodes the entire body into a heap `Vec<u8>` just to read its length and immediately drop it. Now `encoded_len` builds the same `Body` struct and asks `postcard::experimental::serialized_size(&body)` to walk the type and tally bytes without allocating an output buffer. Callers that pair `encoded_len` + `encode` no longer pay 2× the encode cost — the size walk is cheap, the bytes call is the only allocator pressure. (Small variant already uses the const-shaped header math `BLOB_REF_SMALL_HEADER_LEN`; unchanged.) Pinned by `encoded_len_matches_encode_len_without_allocating` which asserts byte-for-byte parity between `encoded_len()` and `encode().len()` for Small, three Manifest sizes (1, 8, 128 chunks), a Tree ref, and a Reed-Solomon Manifest. Drift between the size walker and the real encoder (e.g. a postcard schema change) would break the assertion. |
| 184 | `BlobAdapter::fetch` / `fetch_range` / `MeshBlobAdapter::fetch_chunk` return `Vec<u8>` → `Bytes` | Trait surface (`adapter.rs`), every implementor (`mesh.rs`, `fs.rs`, `noop.rs`, FFI `CallbackBlobAdapter`, Python `PyBlobAdapter` sync + async, Node `NodeBlobAdapter` sync + async, ~7 test stubs in `dispatch.rs`, `migration.rs`, and `greedy/runtime.rs`), plus the `resolve_payload` dispatch helper and the redex `file.rs` blob-ref fetch path all now traffic in `bytes::Bytes`. The headline win is inside `MeshBlobAdapter::fetch_chunk`: pre-fix the chunk's `Bytes` payload was eagerly `.to_vec()`'d on every read, paying N×payload_size bytes of memcpy across a manifest fetch. Post-fix the chunk's backing buffer flows through unchanged; the Small `fetch_range` path takes a zero-copy `Bytes::slice` instead of `bytes[..].to_vec()`. Manifest assembly still folds N chunks into one contiguous output (one memcpy per chunk, not two — see the inline comment) and the Tree-walk assembly buffer stays a `Vec<u8>` that wraps to `Bytes::from(out)` at return. The blob-tree node cache switched its value type to `Bytes` too — cache hits return an Arc-clone (refcount bump) instead of a `Vec::clone` (full memcpy). RS reconstruction and the FFI/JS callback boundaries are the two remaining `.to_vec()` calls — both intentional (RS encoder needs owned mutable shards; FFI free-buffer ownership semantics don't compose with `Bytes`'s Arc). Pinned by `fetch_range_small_is_zero_copy_slice_of_chunk_buffer` (Small `fetch_range` returns a `Bytes::slice` whose backing pointer survives re-slicing — a regression to `.to_vec()` would surface as distinct pointers), plus every prior `fetched.as_ref()` / `Bytes::from(...)` test update — 3946 lib tests + every integration test pass under the new surface. |
| 185 | `parse_blob_heat_tag` per-byte `u8::from_str_radix` → nibble lookup | Replaced the per-byte `from_str_radix(pair, 16)` (general-purpose radix parser: UTF-8 decode + range check + multiply-accumulate) with a 32-step nibble table over `hex.as_bytes()`. The lowercase-only gate above the loop continues to reject uppercase; the new `nibble_lowercase` returns `None` on any byte outside `b'0'..=b'9'` / `b'a'..=b'f'`, preserving the reject-bad-hex contract. Tick-cadence path (per heat tag received), so the savings per call are modest, but the new code is shorter and skips the `hex.get(i*2..i*2+2)` sub-slice + parser-dispatch per byte. Pinned by `parse_blob_heat_tag_decode_matches_from_str_radix_byte_for_byte` (four corner-case hashes plus a non-hex char that survives the lowercase gate but the nibble lookup rejects). |
| 176 | `GreedyCacheRegistry::contains_origin` O(N) `entries.values().any(...)` → O(1) reverse-index lookup | The colocation gate fires `contains_origin(target)` per admission that carries a `colocate-with[-strict]` hint; pre-fix this walked every cached entry. Now `GreedyCacheRegistry` carries an `origin_counts: HashMap<u64, usize>` reverse index — `set_origin_hash` swaps old/new contributions (no-op when `prev == new`), `evict` drops the entry's contribution, the sentinel `0` is never indexed (preserving the existing public contract that `contains_origin(0) == false`). `contains_origin` collapses to `origin_counts.contains_key(...)`. Same alignment in every direction so a regression that loses one bookkeeping call surfaces as a wrong answer, not a hidden leak. Pinned by `contains_origin_uses_reverse_index_and_stays_aligned_across_mutations` which walks the full mutation grid: insert-then-set, two-entries-same-origin (count == 2), evict one of two (still held), evict both (drops), re-set-to-zero (clears). |
| 175 | `GreedyRuntime::local_caps` `Mutex<Arc<CapabilitySet>>` → `ArcSwap<CapabilitySet>` | Per-event `dispatch_event` read is now one lock-free Acquire load + Arc clone via `local_caps.load_full()` instead of `local_caps.lock().clone()` (parking_lot acquire + Arc clone). `set_local_caps`, operator-cadence, performs an atomic Arc swap; the previous Arc is freed only when every outstanding read guard drops. Pinned by `set_local_caps_concurrent_with_dispatch_does_not_deadlock_and_swap_is_visible` which races 64 writes against 64 reads under a 1 s deadline and asserts the final snapshot is `Arc::ptr_eq` to the writer's last stored Arc — a property `ArcSwap` provides across concurrent writers. The existing `set_local_caps_updates_intent_evaluation` continues to cover the swap-visibility contract end-to-end through `should_admit`. (The other per-event mutexes the doc calls out — `cache` and `budget` — stay mutex-guarded; they wrap state with internal mutation patterns that don't map onto a snapshot type without a larger redesign.) |
| 180 | `MeshBlobAdapter::fetch` Manifest assembly Vec grew chunk-by-chunk → pre-allocate to `total_size.min(MAX_BULK_FETCH_BYTES)` | The over-cap check above the assembly loop already rejects `total_size > MAX_BULK_FETCH_BYTES` (256 MiB) — so once we reach the assembly path the declared total is bounded, `as usize` is safe even on 32-bit targets (256 MiB ≪ u32::MAX), and a hostile manifest cannot push the up-front allocation past the cap. Pre-fix `Vec::new()` + `extend_from_slice` per chunk did O(log N) reallocs on every legitimate fetch. The `.min(MAX_BULK_FETCH_BYTES)` is defensive: if the over-cap check ever drifts, the alloc stays bounded; at runtime it folds to a no-op. Pinned by `fetch_manifest_preallocates_vec_to_total_size` — fetches a non-power-of-two Manifest and asserts that the round-trip-through-Box<[u8]> Vec satisfies `capacity == len`, which would fail under the old grow-from-empty path (where capacity rounds up to the next power-of-two). |
| 178 | `MeshBlobAdapter::fetch` heat-bump Vec staging → streamed iterator into generic `bump_heat` | Pre-fix the post-fetch heat-bump path built a `Vec<[u8; 32]>` per call: `vec![*hash]` for `Small`, `chunks.iter().map(\|c\| c.hash).collect()` for `Manifest`, `Vec::new()` for `Tree`. `bump_heat(&hashes)` then iterated over the staged slice. Now `bump_heat<I: IntoIterator<Item = [u8; 32]>>` accepts any iterator and the caller streams directly — `std::iter::once(*hash)` for Small, `chunks.iter().map(\|c\| c.hash)` for Manifest, nothing at all for Tree. Eliminates one heap allocation per successful fetch on the hot path (the `Tree` walk path keeps its `Vec` because `walk_tree_range` builds it as its output buffer — that Vec just gets passed by value into `bump_heat` now). Pinned by the existing Small (`fetch_bumps_blob_heat_when_registry_wired`) and fetch-range (`fetch_range_bumps_blob_heat_for_touched_chunks_only`) tests plus a new `fetch_manifest_bumps_blob_heat_for_every_chunk_hash` which asserts a 3-chunk Manifest fetch lands a heat entry for each chunk hash — catches a regression that loses chunks from the streamed iterator or misroutes the Manifest arm. |
