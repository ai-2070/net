...
| 130 | `PacketReceiver::recv` zero-fills the recv buffer per packet (~1500 byte memset) → tokio `recv_buf_from(&mut BufMut)` | The legacy `resize(MAX_PACKET_SIZE, 0)` + `recv_from(&mut [u8])` shape memset ~1500 bytes per packet just for the kernel to overwrite them on the next syscall — pure wasted memory bandwidth at packet rate. `recv_buf_from` writes into the `BytesMut`'s spare capacity (via `BufMut::chunk_mut`) so the kernel's bytes are the first writers and no pre-init is needed. Added `tokio/io-util` feature to expose the API. `clear()` + `reserve(MAX_PACKET_SIZE)` returns the buffer to length 0 while keeping the allocation; once steady-state the `reserve` is a no-op. Pinned by `packet_receiver_recv_must_use_recv_buf_from_not_resize_zero` — source-level invariant assertion since the wasted memset is observable only as a microbenchmark regression at runtime. **Follow-up:** the three sibling `NetSocket` entry points in the same file (`recv_from`, `recv`, `try_recv_from`) had the same `resize(MAX_PACKET_SIZE, 0)` shape and were missed by the original fix; they now route through tokio's `recv_buf_from` / `recv_buf` / `try_recv_buf_from` respectively. `NetSocket` is held by `Arc<NetSocket>` in `mesh.rs` (392, 1221, 1646), so this is a live RX-path fix, not dead code. Pinned by `net_socket_recv_methods_must_use_recv_buf_family_not_resize_zero` using the same runtime-built-needle pattern as the linux `recvmmsg` pin to avoid self-match; runtime round-trip tests `net_socket_recv_from_round_trips_loopback_datagram`, `net_socket_recv_round_trips_loopback_datagram`, and `net_socket_try_recv_from_returns_none_when_empty_and_data_when_ready` cover the happy paths and the WouldBlock arm. |
...
| 132 | Two-step `is_valid_rx_counter` + `update_rx_counter` per packet → single-lock `try_admit_rx_counter` | Pre-fix every inbound packet took two `parking_lot::Mutex` acquisitions on the replay window: one pre-decrypt to short-circuit replays before paying AEAD, one post-decrypt to commit the novel counter. The `commit` path already rejects out-of-window / already-seen / `u64::MAX` counters internally so the pre-decrypt probe was redundant for safety — only the lock op count differed. New `PacketCipher::try_admit_rx_counter(received) -> bool` collapses the two-step into one lock acquisition; the production RX paths (`mesh.rs::process_local_packet`, `mod.rs::process_packet`, `session.rs::verify_and_touch_heartbeat`) call this instead. Behavioral change: replays now pay AEAD verify before being rejected at admit — the doc trade-off prices this in (replays are rare; the saved lock op on every non-replay dominates at 1 M pps). Legacy `is_valid_rx_counter` / `update_rx_counter` stay exposed for fuzz / regression tests that exercise the two-step boundary. Pinned by `try_admit_rx_counter_matches_update_rx_counter_semantics` — covers fresh-novel, in-window novel, out-of-window past, replay, u64::MAX refusal, and equal-to-rx_counter rejection scenarios against the legacy API as a reference. |
| 133 | `RetransmitDescriptor` cloned per send + per retransmit → `Arc<RetransmitDescriptor>` across the `ReliabilityMode` trait boundary | Pre-fix `on_send` moved a `RetransmitDescriptor` (carries `Vec<Bytes>` events) by value into the retransmit window, and `on_nack` / `get_timed_out` deep-cloned the descriptor per emission (Vec spine alloc + N `Bytes` refcount bumps). At `max_pending = 32` × ~10 events/packet that's ~320 `Bytes` refcount bumps + a Vec alloc per retransmit emission. Post-fix the trait exchanges `Arc<RetransmitDescriptor>` — `on_send` consumes the shared allocation, `on_nack` / `get_timed_out` emit `Arc::clone`s (one atomic refcount bump per emission regardless of the inner Vec's length). The `FireAndForget` no-op impl and the `mod.rs` reliable-send call sites wrap once at construction time. Pinned by `retransmits_share_descriptor_via_arc_refcount_not_deep_clone` (`Arc::as_ptr` identity check across both NACK and timeout paths — a regression that swaps back to `.clone()` on the inner descriptor would fail the pointer-equality assertion). |
