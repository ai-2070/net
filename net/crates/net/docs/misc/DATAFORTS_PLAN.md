...
| 3 — `BlobRef` + `BlobAdapter` hook | Rebel Yell | 🟡 partial on `dataforts-phase-3` branch — foundation + FileSystemAdapter + conformance suite shipped; S3 / IPFS reference adapters, mesh read-path dispatch, and bindings pending | `adapter::net::dataforts::blob::*` (`BlobRef`, `BlobAdapter` trait, `BlobAdapterRegistry`, `NoopAdapter`, `FileSystemAdapter`, `run_conformance_suite`) |
