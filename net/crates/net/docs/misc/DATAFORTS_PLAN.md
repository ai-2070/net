...
| 3 — `BlobRef` + `BlobAdapter` hook | Rebel Yell | 🟡 partial — foundation + FileSystemAdapter + conformance suite + substrate read-path (`RedexFile::{read_one, resolve_one}`, `RedexFileConfig::blob_adapter_id`) + write-path (`publish_blob`, `publish_blob_ref`) + Python / Node / C FFI bindings shipped. **Pending**: S3 / IPFS reference adapters; host-language-implemented BlobAdapters (Python class / JS class / cgo fn pointer wrapped as `dyn BlobAdapter` via callback bridge). | `adapter::net::dataforts::blob::*` + `RedexFile::{read_one, resolve_one}` + `bindings/python/src/blob.rs` + `bindings/node/src/blob.rs` + `src/ffi/blob.rs` |
