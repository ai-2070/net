...
The `register_rpc_inbound` hook is the load-bearing primitive the remaining `Mesh::serve_rpc` / `Mesh::call` glue builds on. With the wire-level `channel_hash` and the dispatch hook in place, RPC events route directly to per-channel handlers without going through the bus or the shard inbound queue.
