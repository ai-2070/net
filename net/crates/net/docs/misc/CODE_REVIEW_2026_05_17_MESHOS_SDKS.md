...
| Polish (remaining) | Name the magic poll cadences (item 14); rename Python `_caps` kwarg (item 15); prune `__all__` when verifier missing (item 17); document or align option drift (item 18); fix Go stream Close/Next race (item 19); document or align audit-stream timeout semantics (item 20); improve Go cgo idioms (item 22). |
