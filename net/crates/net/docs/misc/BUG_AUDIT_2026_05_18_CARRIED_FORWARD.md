...
#### S-4 — nRPC response delivery keyed on call_id alone; sequential counter lets any session peer spoof responses to in-flight calls — **Partially landed (random call_id)**
...
- **Fix (parts 2 + 3, deferred):**
  - **Carry `from_node` into `RpcClientFold::apply`.** The fold receives `&RedexEvent` which doesn't carry sender identity; adding it requires either extending `RedexEvent` or routing a side channel through the CortEX adapter. Architectural work; tracked separately.
  - **Reply-channel publish ACL = `{target_node_id}`.** Requires the reply-channel setup at `ensure_reply_subscription` to install a per-call ACL. Tracked separately.
  - Without these two parts, an attacker who has *observed* a victim's call_id (e.g., compromised middle-relay, intercepted wire packets) could still spoof a single response. The remaining attack surface is much narrower than the pre-fix "any session peer can spoof any in-flight call."
- **Regression test:** deferred to land with parts 2+3 — a useful test needs the from_node-binding to be the assertion target; with only random call_ids the assertion would be probabilistic.
