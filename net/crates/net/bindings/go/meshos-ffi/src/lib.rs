...
// These are coarse-grained handles touched by only one consumer
// thread in the common case. parking_lot::Mutex matches the rest of
// the workspace's lock convention and has no poison concept.
...
pub extern "C" fn net_meshos_sdk_shutdown(sdk: *mut NetMeshOsSdk) -> c_int {
    ffi_guard!(NET_MESHOS_ERR_CALL_FAILED, {
        ...
        clear_last_error_inner();
        let inner = match sdk_ref.inner.lock().take() {
            Some(s) => s,
            None => {
...
pub extern "C" fn net_meshos_try_next_control(
    ...
    ffi_guard!(NET_MESHOS_ERR_CALL_FAILED, {
        ...
        clear_last_error_inner();
        let mut guard = h_ref.inner.lock();
...
pub extern "C" fn net_meshos_graceful_shutdown(
    ...
    ffi_guard!(NET_MESHOS_ERR_CALL_FAILED, {
        ...
        let inner = match h_ref.inner.lock().take() {
            Some(h) => h,
            None => {
                set_last_error("already_shutdown", "daemon handle was already consumed");
                return NET_MESHOS_ERR_ALREADY_SHUTDOWN;
...
mod tests {
    ...
    unsafe extern "C" fn vt_process(
        ...
        if payload_len > 0 {
            ...
            *state.last_payload.lock() = slice.to_vec();
        }
    ...
    fn vtable_register_daemon_succeeds_and_bridge_calls_into_consumer() {
        ...
        assert_eq!(*state.last_payload.lock(), b"hello");
