...
// wrapper over the C ABI exported by `net::ffi::aggregator`
// (compiled into the main `libnet` cdylib).
//
// This file is a **reference implementation** documenting the
// Go-side surface for consumers of the `net_registry_client_*`
// + `net_fold_query_client_*` symbols. See the package README
// for build prerequisites; the canonical recipe is:
//
...
//
//   - `RegistryClient` — list / spawn / unregister the
//     aggregator groups registered on a remote node.
//   - `FoldQueryClient` — query latest cached summaries or force
//     a fresh `SummarizeNow` against a remote aggregator. Holds
//     a TTL cache; the binding exposes `InvalidateCache` /
//     `InvalidateTarget` for topology-change scenarios.
//
// # Memory model
//
// Both client types wrap an opaque `*C.net_*_handle_t` and
// install a `runtime.SetFinalizer` that releases the handle on
// GC. Callers wanting deterministic teardown call `.Close()`,
// which is idempotent and clears the finalizer.
//
// # Error model
//
// Every failing op surfaces a `*RegistryClientError`
// (or `*FoldQueryClientError`) carrying:
//
//   - `Kind` — stable kebab-case discriminator matching the C
//     `NET_REGISTRY_ERR_*` constants. Values:
//   - `"transport"`             — wire failure / no route / timeout
//   - `"codec"`                 — request encode / response decode
//   - `"unknown-template"`      — registry op only
//   - `"duplicate-group-name"`  — registry spawn only
//   - `"spawn-rejected"`        — registry spawn only
//   - `"spawn-not-supported"`   — registry op only
//   - `"unknown-kind"`          — fold-query op only
//   - `"invalid-args"`          — caller-side bad input
//   - `Detail` — operator-facing message from the substrate.
//
// Mirrors the `agg:<kind>` prefix in the Node + Python bindings.
package net
...
// Forward-declared opaque handles from `libnet` (the main net-mesh
// cdylib). The `mesh_handle` argument is a `void*` because the
// substrate's MeshNodeHandle type stays opaque at the C boundary.
...
// Error discriminants — values locked across SDKs.
...
// Shared free for strings returned by the *_list / *_spawn /
// *_query_* ops. The Rust side allocates via `CString::into_raw`;
// the matching free is `net_free_string`. Declared here rather
// than pulling in `redex.go`'s declaration because cgo files
// don't share C declarations across the package (each file is
...
// public C ABI; an SDK bump updates both sides in lockstep.
const (
...
// `*RegistryClientError` carries in its `Kind` field. Compare
...
// `errors.As(err, &reg)` retrieves it from a wrapped error.
...
// RegistryReplicaRow mirrors the per-replica row inside a
...
// RegistryGroupSummary mirrors the SDK's substrate-side
// `RegistryGroupSummary` 1:1. `GroupSeedHex` is a 64-char
// lowercase hex rendering of the 32-byte group seed.
...
// SummaryBucket is one (name, count) pair inside a
// SummaryAnnouncement.
...
// SummaryAnnouncement mirrors the SDK's substrate-side type.
// `FoldKind` is the numeric `FoldKind::KIND_ID`; `SourceSubnet`
// is dotted-notation ("3.7") or "global".
...
// Returns ErrAggregatorHandleClosed if `meshHandle` is nil.
...
// Close releases the underlying handle. Safe to call multiple
// times; only the first call frees.
...
// WithDeadline sets the per-call deadline for subsequent ops.
// Pass 0 to reset to the substrate default.
...
// List returns the groups registered on `targetNodeID`. The
// context's deadline takes precedence over the client's
// configured deadline if it is sooner.
...
// Spawn deploys a new group by referencing a daemon-side template.
...
// honorContextDeadline pushes ctx.Deadline() (if any) onto the
// substrate client's per-call deadline. Idempotent — calling
// repeatedly with the same context just re-sets the same value.
// Holding the RLock through this is safe; the C API permits
// concurrent ops on the same handle.
func (c *RegistryClient) honorContextDeadline(ctx context.Context) {
	if deadline, ok := ctx.Deadline(); ok {
		remaining := time.Until(deadline)
		if remaining <= 0 {
			// Already expired — leave the substrate deadline
			// alone so the op fails fast with the substrate's
			// own timeout machinery rather than us trying to
			// represent "elapsed=0" as a positive value.
			return
		}
...
// service. Caches recent `QueryLatest` responses by `(target, kind)`
...
// WithTTL sets the cache TTL. Pass 0 to disable caching entirely.
...
// Cache hit returns immediately; miss issues a wire RPC.
...
// InvalidateCache drops every cached entry. Use after a topology
// change so the next query re-resolves.
