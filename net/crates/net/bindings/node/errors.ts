 ...
 */
// ---------------------------------------------------------------------------
// Organization capability auth — four domains plus the parser fallback.
//
// Lives here rather than in `org.ts` so this module stays free of any native
// import: `abi_stability.test.ts` and the org vocabulary test build synthetic
// `Error` objects and run WITHOUT a compiled cdylib, which is what lets a
// vocabulary rename fail before the native module is rebuilt.
// ---------------------------------------------------------------------------
