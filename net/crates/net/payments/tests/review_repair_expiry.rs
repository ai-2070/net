...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn review_expired_in_flight_payment_is_not_proven_unpaid() {
    let w = world(true, false).await;
    w.flow
        .prepare_task(NODE, &w.offer, &brief("review-expiry-in-flight"))
        .await
        .expect("prepare");
    let flow = w.flow.clone();
    let winner =
        tokio::spawn(async move { flow.purchase_task(NODE, "review-expiry-in-flight").await });
    tokio::time::timeout(
        std::time::Duration::from_secs(5),
        w.settle_gate.await_entered(1),
    )
    .await
    .expect("settlement reached facilitator");
    assert_eq!(w.state("review-expiry-in-flight").await, StateTag::Paying);
    w.clock.advance(QUOTE_TTL_NS + 60_000_000_000);
    let duplicate = w.flow.purchase_task(NODE, "review-expiry-in-flight").await;
    let during = w.state("review-expiry-in-flight").await;
    w.settle_gate.release();
    let authoritative = tokio::time::timeout(std::time::Duration::from_secs(5), winner)
        .await
        .expect("settlement returns")
        .expect("no panic");
    eprintln!("duplicate={duplicate:?}; during={during:?}; original={authoritative:?}; final={:?}; billed={}",w.state("review-expiry-in-flight").await,w.billed().await);
    assert!(matches!(duplicate, A2aPurchase::Unknown { .. }), "an already-exposed settlement cannot become proven nonpayment merely because the quote expired: {duplicate:?}");
    assert!(
        matches!(authoritative, A2aPurchase::Paid { .. }),
        "successful original payment must reconcile: {authoritative:?}"
    );
}

fn retained_paid_state(a: &PurchaseAttempt) -> PurchaseState {
    let PurchaseState::Paid { proof, billing } = &a.state else {
        panic!("expected paid")
    };
    PurchaseState::PaidUnexecutable {
        ...
        refusal: net_payments::flow::a2a::RefusalRecord {
            ...
            reason: Some(SUPERSEDED_REFUSAL_REASON.into()),
            safe_to_retry: false,
...
#[tokio::test]
async fn review_superseded_key_cannot_overwrite_a_live_purchase() {
    let w = world(false, false).await;
    w.flow
        .prepare_task(NODE, &w.offer, &brief("collision"))
        .await
        .expect("prepare A");
    assert!(matches!(
        w.flow.purchase_task(NODE, "collision").await,
        A2aPurchase::Paid { .. }
    ));
    let a = w.attempt("collision").await;
    let other = format!(
        "collision#superseded/{}-{}",
        a.generation.seq, a.generation.incarnation
    );
    w.flow
        .prepare_task(NODE, &w.offer, &brief(&other))
        .await
        .expect("prepare valid different ID");
    assert!(matches!(
        w.flow.purchase_task(NODE, &other).await,
        A2aPurchase::Paid { .. }
    ));
    let before = w.attempt(&other).await;
    assert_ne!(before.quote_id, a.quote_id);
    w.store
        .retain_superseded(&a, retained_paid_state(&a), NOW + 1)
        .await
        .expect("retain A");
    let after = w.attempt(&other).await;
    eprintln!("live key before={:?}, after={:?}", before.key, after.key);
    assert_eq!(
        after, before,
        "retaining A overwrote a distinct valid live paid purchase"
    );
}
#[tokio::test]
async fn review_retained_success_cannot_be_downgraded_by_late_ambiguity() {
    let w = world(false, false).await;
    w.flow
        .prepare_task(NODE, &w.offer, &brief("monotone"))
        .await
        .expect("prepare");
    assert!(matches!(
        w.flow.purchase_task(NODE, "monotone").await,
        A2aPurchase::Paid { .. }
    ));
    let a = w.attempt("monotone").await;
    let known = retained_paid_state(&a);
    w.store
        .retain_superseded(&a, known.clone(), NOW + 1)
        .await
        .expect("retain success");
    w.store
        .retain_superseded(
            &a,
            PurchaseState::Unknown {
                last_error: "late sibling timeout".into(),
            },
            NOW + 2,
        )
        .await
    ...
    let after = w
        .store
        .superseded(&a.key, &a.generation)
        .await
        .expect("read")
        .expect("retained");
    assert_eq!(
        after.state, known,
        "late ambiguity erased proof and billing"
    );
}
