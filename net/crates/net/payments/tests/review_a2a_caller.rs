...
impl ScriptedChannel {
    fn new(inner: InProcessProvider) -> Self {
        Self {
            inner,
            review_wrong_hash: AtomicBool::new(false),
    ...
    async fn quote(
        ...
        self.inner
            .quote(
                ...
                capability,
                template,
                if self.review_wrong_hash.load(Ordering::SeqCst) {
                    None
                } else {
                    input_hash
                },
...
#[tokio::test]
async fn review_redeem_recovery_survives_engine_retention() {
    let w = world().await;
    let prepared = w
        .flow
        .prepare_task(NODE, &w.offer, &brief("review-redeem-retention"))
        .await
        .unwrap();
    let paid = w.flow.purchase_task(NODE, "review-redeem-retention").await;
    let A2aPurchase::Paid { proof, .. } = paid else {
        panic!("not paid");
    };
    let gate = EngineTaskAdmissionGate::new(w.engine.clone());
    let claim = || TaskPaymentClaim {
        tool_id: "net.a2a.task/summarize",
        quote_id: &proof.quote_id,
        binding: &proof.binding_sig,
        expected_input_hash: &prepared.reservation.purchase_hash,
    };
    gate.redeem(claim()).await.unwrap();
    // The handler can crash here, before the admission journal records Paid.
    w.clock.advance(7 * 3600 * 1_000_000_000);
    assert_eq!(
        w.engine
            .prune_terminal_records(w.clock.now_ns())
            .await
            .unwrap(),
        1
    );
    let replay = gate.redeem(claim()).await;
    assert!(
        replay.is_ok(),
        "original redeemed purchase lost before seven-day admission retention: {replay:?}"
    );
}

#[tokio::test]
async fn review_paying_survives_retention() {
    let w = world().await;
    seed(&w, "review-paying", sample_state(StateTag::Paying)).await;
    w.clock.advance(10_000_000_000);
    assert_eq!(
        w.store
            .prune(w.clock.now_ns(), 1_000_000_000)
            .await
            .unwrap(),
        0,
        "Paying can already be settled and must retain the exact purchase"
    );
}
#[tokio::test]
async fn review_lost_reply_recovers_after_quote_expiry() {
    let w = world().await;
    w.flow
        .prepare_task(NODE, &w.offer, &brief("review-expired"))
        .await
        .unwrap();
    w.channel.set_mode(PayMode::SwallowOnce);
    assert!(matches!(
        w.flow.purchase_task(NODE, "review-expired").await,
        A2aPurchase::Unknown { .. }
    ));
    assert_eq!(w.billed().await, 1);
    w.clock.advance(3_600_000_000_000);
    let second = w.flow.purchase_task(NODE, "review-expired").await;
    eprintln!(
        "retained billing={}, spend={}, resume={second:?}",
        w.billed().await,
        w.reserved_today().await
    );
    assert!(
        matches!(second, A2aPurchase::Paid { .. }),
        "settled replay must recover after quote expiry: {second:?}"
    );
}
#[tokio::test]
async fn review_wrong_input_hash_is_refused_before_payment() {
    let w = world().await;
    w.channel.review_wrong_hash.store(true, Ordering::SeqCst);
    let result = w
        .flow
        .prepare_task(NODE, &w.offer, &brief("review-misbound"))
        .await;
    let Ok(prepared) = result else {
        return;
    };
    let paid = w.flow.purchase_task(NODE, "review-misbound").await;
    if let A2aPurchase::Paid { proof, .. } = paid {
        let gate = EngineTaskAdmissionGate::new(w.engine.clone());
        let deny = gate
            .redeem(TaskPaymentClaim {
                tool_id: "net.a2a.task/summarize",
            ...
            .await
            .expect_err("mismatched input must fail provider redemption");
        eprintln!(
            "billed={}, provider refusal={}",
            w.billed().await,
            deny.schematic.reason
        );
    }
    assert_eq!(
        w.billed().await,
        0,
        "caller paid for evidence not bound to its reservation"
    );
}

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn review_stale_signer_cannot_pay_unapproved_replacement_quote() {
    let w = svm_world().await;
    w.spend
        .configure(|defaults, _| {
    ...
    let signer_gate = gate.clone();
    let signer_count = count.clone();
    let payments = Arc::new(
        net_payments::flow::CallerPaymentFlow::new(
            w.caller.clone(),
            SpendPolicyEngine::new(&w.spend_path, SpendProfile::Production),
            default_registry_v1(w.engine.provider_id().clone()),
            w.channel.clone(),
            w.clock.clone(),
        )
        .with_signer(
            "solana",
            Arc::new(ExternalSvmSigner::new(SVM_PAY_TO, move |_intent| {
                let g = signer_gate.clone();
                let c = signer_count.clone();
                Box::pin(async move {
                    if c.fetch_add(1, Ordering::SeqCst) == 0 {
                        g.pass().await;
                        Ok("cGFydGlhbGx5LXNpZ25lZC1zdm0=".to_string())
                    } else {
                        Err(net_payments::flow::signer::SignerError::new(
                            "second author failed before exposure",
                        ))
                    }
                })
            })),
    ...
    flow.prepare_task(NODE, &w.offer, &brief("review-quote-aba"))
        .await
        .unwrap();
    let q1 = match flow.purchase_task(NODE, "review-quote-aba").await {
        A2aPurchase::RequiresPaymentApproval { quote_id, .. } => quote_id,
        x => panic!("first hold {x:?}"),
    };
    assert!(w.spend.approve(&q1).await.unwrap());
    let fa = flow.clone();
    let a = tokio::spawn(async move { fa.purchase_task(NODE, "review-quote-aba").await });
    tokio::time::timeout(std::time::Duration::from_secs(5), gate.await_entered(1))
        .await
        .unwrap();
    let b = flow.purchase_task(NODE, "review-quote-aba").await;
    assert!(
        matches!(
            b,
            A2aPurchase::Denied {
    ...
    w.clock.advance(1_000_000_000);
    flow.prepare_task(NODE, &w.offer, &brief("review-quote-aba"))
        .await
        .unwrap();
    let q2 = match flow.purchase_task(NODE, "review-quote-aba").await {
        A2aPurchase::RequiresPaymentApproval { quote_id, .. } => quote_id,
        x => panic!("replacement hold {x:?}"),
    };
    assert_ne!(q1, q2);
    gate.release();
    let result = tokio::time::timeout(std::time::Duration::from_secs(5), a)
        .await
        .unwrap()
        .unwrap();
    eprintln!(
        "Q1 approved={q1}; Q2 never approved={q2}; stale author result={result:?}; bills={}",
        w.billed().await
    );
    assert!(
        !matches!(result, A2aPurchase::Paid { .. }),
        "stale signer purchased unapproved replacement quote"
    );
}
