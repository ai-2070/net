...
/// D2's symmetric order: archive publication wins the race with live resolution.
/// Actual first settlement creates the archive; the duplicate completion is
/// delivered through the production retention verb, not via file editing.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn review_existing_archive_inherits_live_disposition_on_late_success() {
    let w = world(true, false).await;
    let task = "existing-archive-live-resolution";
    w.flow
        .prepare_task(NODE, &w.offer, &brief(task))
        .await
        .unwrap();
    let flow = w.flow.clone();
    let winner = tokio::spawn(async move { flow.purchase_task(NODE, task).await });
    tokio::time::timeout(
        std::time::Duration::from_secs(5),
        w.settle_gate.await_entered(1),
    )
    .await
    .unwrap();
    let original = w.attempt(task).await;
    w.store
        .transition_exact(
            &original.key,
            &[StateTag::Paying],
            &original.identity(),
            PurchaseState::RefusedExposed {
                reason: "terminal sibling verdict".into(),
                reservation_kept: true,
            },
            w.clock.now_ns(),
        )
        ...
        .unwrap();
    w.settle_gate.release();
    let first = tokio::time::timeout(std::time::Duration::from_secs(5), winner)
        .await
        .unwrap()
        .unwrap();
    assert!(matches!(
    ...
    let archived = w
        .store
        .superseded(&original.key, &original.generation)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(archived.state.tag(), StateTag::PaidUnexecutable);
    let closed = w
        .flow
        .resolve_attempt(
            NODE,
            task,
            AttemptResolution::Closed {
                outcome: "accounted-for".into(),
                evidence: serde_json::json!({"ticket":"OPS-existing"}),
            },
        )
        .await
        .unwrap();
    assert_eq!(closed.generation, original.generation);
    assert_eq!(closed.state.tag(), StateTag::Resolved);
    let after = w
        .store
        .retain_superseded(&original, archived.state.clone(), NOW + 1)
        .await
        .unwrap();
    ...
    assert_eq!(
        ...
        closed.state,
        "live operator document is untouched"
    );
    eprintln!(
        "same generation: live={:?}, archive={:?}",
        closed.state.tag(),
        after.state.tag()
    );
    assert_eq!(
