...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn review_late_settlement_preserves_a_live_operator_disposition() {
    let w = world(true, false).await;
    let task = "resolved-live";
    w.flow
        .prepare_task(NODE, &w.offer, &brief(task))
        .await
        .unwrap();
    let flow = w.flow.clone();
    let paying = tokio::spawn(async move { flow.purchase_task(NODE, task).await });
    tokio::time::timeout(
        std::time::Duration::from_secs(5),
        w.settle_gate.await_entered(1),
    )
    .await
    .unwrap();
    let in_flight = w.attempt(task).await;
    w.store
        .transition_exact(
            &in_flight.key,
            &[StateTag::Paying],
            &in_flight.identity(),
            PurchaseState::RefusedExposed {
                reason: "terminal sibling transport failure".into(),
                reservation_kept: true,
            },
            w.clock.now_ns(),
        )
    ...
    let disposition = w
        ...
        .resolve_attempt(
            ...
            task,
            AttemptResolution::Closed {
                outcome: "written-off".into(),
                evidence: serde_json::json!({"ticket":"OPS-live"}),
            },
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        w.attempt(task).await.state,
        disposition.state,
        "original operator disposition remains"
    );
    let rows = w.flow.attempts().await.unwrap();
    let same: Vec<_> = rows
        .iter()
        .filter(|r| r.key == in_flight.key && r.generation == in_flight.generation)
        .collect();
    eprintln!(
        "late result={result:?}; same-incarnation states={:?}",
        same.iter().map(|r| r.state.tag()).collect::<Vec<_>>()
    );
    assert!(
        !same.iter().any(|r| r.is_unresolved_financial()),
        "the same resolved charge was silently reopened in a new archive record"
    );
}
fn review_contains_document(haystack: &serde_json::Value, needle: &serde_json::Value) -> bool {
    if haystack == needle {
        return true;
    }
    if let (Some(h), Some(n)) = (haystack.as_object(), needle.as_object()) {
        if n.iter().all(|(k, v)| h.get(k) == Some(v)) {
            return true;
        }
    }
    match haystack {
        serde_json::Value::Object(o) => o.values().any(|v| review_contains_document(v, needle)),
        serde_json::Value::Array(a) => a.iter().any(|v| review_contains_document(v, needle)),
        _ => false,
    }
}
#[tokio::test]
async fn review_late_settlement_preserves_arbitrary_operator_evidence() {
    let w = world(false, false).await;
    let task = "operator-evidence";
    w.flow
        .prepare_task(NODE, &w.offer, &brief(task))
        .await
        .unwrap();
    assert!(matches!(
        w.flow.purchase_task(NODE, task).await,
        A2aPurchase::Paid { .. }
    ));
    let a = w.attempt(task).await;
    w.store
        .retain_superseded(
            &a,
            PurchaseState::RefusedExposed {
                reason: "exposed refusal".into(),
                reservation_kept: true,
            },
            NOW + 1,
        )
        .await
        .unwrap();
    let original = serde_json::json!({"late_settlement":{"ticket":"OPS-HISTORY"},"operator_evidence":"also operator-owned"});
    w.flow
        .resolve_superseded_attempt(
            NODE,
            task,
    ...
    let after = w
        .store
        .retain_superseded(&a, paid_evidence(&a), NOW + 2)
        .await
        .unwrap();
    let PurchaseState::Resolved { outcome, evidence } = after.state else {
        panic!("resolution lost")
    };
    assert_eq!(outcome, "closed");
    assert!(
        review_contains_document(&evidence, &original),
        "late settlement overwrote arbitrary operator evidence instead of preserving it alongside"
    );
}
#[tokio::test]
async fn review_prior_format_archive_is_still_exactly_addressable() {
    let w = world(false, false).await;
    let task = "prior-format";
    w.flow
        .prepare_task(NODE, &w.offer, &brief(task))
        .await
        .unwrap();
    ...
    let a = w.attempt(task).await;
    w.store
        .retain_superseded(&a, paid_evidence(&a), NOW + 1)
        .await
        .unwrap();
    // Persist exactly the previous writer's namespace shape, with real evidence.
    net_payments::policy::store::mutate_json_if_changed::<serde_json::Value, _, _>(
        w.store.path(),
        |v| {
            let legacy = v.as_object_mut().unwrap().remove("superseded").unwrap();
            let live = v.get_mut("attempts").unwrap().as_object_mut().unwrap();
            for (k, a) in legacy.as_object().unwrap() {
                live.insert(k.clone(), a.clone());
            }
            ((), true)
        },
    )
    .await
    .unwrap();
    let reopened = A2aPurchaseStore::new(w.store.path());
    assert_eq!(
        reopened.attempts().await.unwrap().len(),
        2,
        "old document is accepted and lists both rows"
    );
    let retained = reopened.superseded(&a.key, &a.generation).await.unwrap();
    assert!(
        retained.is_some(),
        "accepted old archive is no longer reachable through its exact historical identity"
    );
}
