...
> **N-5 follow-up (second cubic round on the review commits):** four more items on the fix commits themselves, all addressed. **(P1)** `redeem_via_engine`'s fail-closed arm interpolated the raw `EngineError` (wrapping `StoreError`/`EnvelopeError`/`X402Error` — file paths, I/O detail, facilitator responses) into the caller-facing string → info leak to SDK/MCP callers; now logs server-side via `tracing::error!` and returns only a generic verdict, with regression `a_store_failure_fails_closed_without_leaking_internal_detail` (corrupt the store → assert the message is exactly generic, no path/serde detail). **(P2)** the same fn's rustdoc used intra-doc links to `mesh::`/`mcp_gate::` items that dangle under single-feature docs (same class as N-10) → plain spans. **(P2)** the SVM transaction-level-bind comment still called per-transfer attribution "deferred" though N3b enforces it below → reworded. **(P3)** a `xhigh-effort` typo in this plan's addendum → `extra-high-effort`.
