...
| 121 | Stale floors: `MIN=196` (wire) and 31 (deck) | High | ci | Closed |
...
| 121 | Stale floors: `MIN=196` (wire) and 31 (deck) | **Closed** — wire 196→275 and deck 31→35 re-measured in the repair tree (`78880312c`); the last two floors are closed by measurement, not by the attribute scan this finding used. Leaf native: `cargo test --features mock-control-plane` runs **345** (264 lib + 81 across the ten native integration binaries; the four `wasm_*` binaries compile to 0 tests natively and doctests are 0) → floor raised 308→345. `sensing_org_lease_wire`: `cargo nextest run --features "cortex tool fixtures"` — exact command in the step — runs **9**, all passing, so `--min 9` was already exact; this finding's "11" was the attribute scan over-counting, which is why the two were deferred rather than raised. No test in either surface is `#[cfg(unix)]`/`#[cfg(windows)]`, so both counts are platform-independent. | `ci.yml` leaf "Native tests" step (hash lands with the commit) |
...
**The tally: 129 closed + 1 noted = 130.** *Closed* = the required closure is established at the finding level; six of those (#8, #17, #47, #50, #94, #107) carry a named deferred sub-part and two (#10, #122) carry a recorded note — neither makes the finding open. (#121 was the one *Deferred* row; its last two floors are closed by measurement at the row, so the four floor values it names now equal their suites' executed inventories.) *Noted* (#126) = no closure landed and none is claimed; the gap is named in the source and at the row. Every finding is named exactly once with its commit (or, at #126, with the statement that no commit closed it); rows group by repair commit because each row's findings share a file set and one verification run.
