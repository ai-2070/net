...
### X-1 — `StandbyGroup::promote` has no fencing; partition heal yields split-brain — **Local epoch scaffolding landed; cross-node wire enforcement deferred**
...
- **Partial fix landed: local epoch scaffolding.** Added a `term: u64` field to `StandbyGroup`, `ForkGroup`, and `ReplicaGroup`. Spawn-time members start at `term = 1`; every successful `promote` / `promote_with_placement` (StandbyGroup) and every successful `try_recover` (ForkGroup, ReplicaGroup) calls `self.term = self.term.saturating_add(1)`. A `pub fn term(&self) -> u64` accessor is exposed on each group so a future wire-fencing layer can stamp routed events with the issuing active's term. Regression test `promote_bumps_term_counter` on StandbyGroup pins the local bump semantic across two consecutive promotes.
- **Why cross-node enforcement is still deferred:** The wire integration ("daemon host rejects at lower term, demote-to-standby on observing a higher term") requires extending every routed event envelope with the issuer term, having every receiver track `current_term` per origin, and re-syncing terms in the membership protocol on reconnect. That's a fleet-wide wire-format change — out of scope for this audit's bounded sweep. The alternative ("you have been demoted" control message through `MeshDaemon::on_control`) requires the cross-node mesh path to be reachable; during the partition that's exactly what's broken, so the message would also be unable to fence the rogue active until reconnect — at which point an event-carried term would already have done the job.
- **Operator mitigation today:** require `leader_pinned` placement (avoids auto-promote entirely) or deploy a fencing-token issuer at the daemon host layer. The local `term()` counter is the building block both of those layers can read against.
...
#### X-13 — Failed placements don't retry on different-node recovery — **Landed**
...
- **Fix:** Locked decision Option C (periodic tick from the meshos loop) — fully landed.
  - **Trait + impls:** `UnhealthySlotRecovery` trait in `compute/mod.rs` with `has_unhealthy_slots()` + `try_recover(&scheduler, &registry, &daemon_factory) -> Vec<u8>`. Impls on StandbyGroup, ForkGroup, ReplicaGroup; each capped at `MAX_RECOVERIES_PER_TICK = 4` so pathological "every slot unhealthy" states make progress without wedging the caller.
  - **Loop integration:** added `RecoveryRegistry` type in `compute/mod.rs` — Arc-wrapped `Vec<RecoveryHandler>` where `RecoveryHandler = Box<dyn FnMut() -> Vec<u8> + Send>`. The handler captures `(group, scheduler, registry, daemon_factory)` so the loop runs heterogeneous groups uniformly. `MeshOsLoop` gained a `recovery_registry` field; the tick handler calls `try_run_all` between `poll_probes` and `run_reconcile`, logging recovered slot indices at debug level. Panicking handlers are caught with `catch_unwind` and evicted (same posture as the probe poll).
  - **SDK surface:** `MeshOsRuntime::recovery_registry()` returns the shared `RecoveryRegistry`; callers `register` a handler post-`start`. Empty registry → tick handler is a no-op, so existing deployments see no behavior change.
- **Regression test:** `try_run_all_collects_recovered_slots_and_drops_panicking_handlers` in `compute/mod.rs` covers (a) successful handlers collect their recovered slot indices into the aggregate vec across ticks, and (b) a panicking handler is evicted on first call so subsequent ticks see only the survivors.
