...
| Done (TS) | Snapshot docstrings now read `AsyncIterable<unknown> /* parsed MeshOsSnapshot JSON */` to match reality (item 12); `MeshOsDaemonSdk.__rawNapiSdk()` `@internal` accessor replaces the `(sdk as unknown as { raw: never }).raw` cast hack (item 13); `type bool = boolean` alias dropped in favor of `boolean` (item 16). |
| Done (Py) | Control-poll cadence extracted as `_CONTROL_POLL_INTERVAL_MS` and used in both `anext_control` and `__anext__` (item 14); `publish_capabilities(_caps=…)` kwarg leak fixed (item 15 — and the existing wrapper was broken: the pyo3 param is `caps`, not `_caps`, so the prior call would have raised TypeError); `__all__` now adds `OperatorRegistry` / `AdminVerifier` only when `_HAS_VERIFIER` (item 17). |
| Done (Go) | `DeckSnapshotStream` doc spells out the no-concurrent-Next+Close contract (item 19); `net_deck.h` audit-stream `timeout_ms == 0` semantics documented (item 20); `unsafe.Pointer(uintptr(cgoHandle))` annotated with rationale + manual `**C.char` arithmetic replaced with `unsafe.Slice` (item 22). |
| Wontfix | Option drift between TS / Py / Go (item 18) — `callbackTimeoutMs` is napi-specific because JS callbacks can stall the supervisor; Python's pyo3 borrow lock and Go's interface-method dispatch handle the same problem differently and don't need the knob. Leave as-is. |
