...
| BROWSER-2, VEC-8, VEC-9, §11 drift | FIXED | `a9178e580` | every `sink_error` closed-refusal text re-types into `OrgStreamError`; `parseOrgError` classifies the full frozen kind set; fixtures regenerated at their source (`samples()`), 130 rows; generator comment corrected; layout metadata derived/checked [§23 correction: `error_vectors.json` holds 29 vocabulary vectors; 130 is the `gen_org_error_fixtures --check` conformance row count over `streaming_opening_vectors.json`] |
