...
| 6 | **Valid, in part.** | `ObserverNotQualified` recovery now names a readable, stable authority view as well as valid membership (`sensing.rs`, `consumer.rs`); the impossible "close after drop" wording is gone; the CI comment names the variable that exists (`listed`). The legitimate `NoOrganizationAuthority` retention-stage rotation wording is PRESERVED. The branch-name replacement is **not** justified — both refs exist. On the roster bound: the comparison stays `-ge`, so adding a witness never trips it, but the threshold IS kept at the current witness count (27 as of `547ce0865`) and is raised whenever one is added — so `$listed` equals it. That is deliberate: a threshold below the count would let a silently dropped witness pass, which is the failure this gate exists to catch. §6's observation that the numbers coincide is correct, and the earlier claim that this is "never an equality" was wrong and is withdrawn. |
