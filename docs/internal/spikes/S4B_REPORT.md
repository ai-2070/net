...
(default build), `anchor ls` (feature `rtc-bootstrap`), `anchor serve`
...
| --- | --- | --- | --- |
| **R1** the trickle socket is unauthenticated | `GET /rtc/trickle?node_id=&dialog=` needed only those two query values, both guessable; anyone who could guess or observe one could drive the exchange, and the credential was checked only at `POST /rtc/offer` | a 32-byte CSPRNG **attempt token** minted per accepted offer, returned in the offer body, presented as the WebSocket subprotocol `net-bootstrap-attempt.<hex>` and checked in a tower **layer** — not an extractor, which would answer before the check. Retirement is token-owned. | `the_attempt_token_holder_can_trickle_and_abandon_its_own_attempt` and `kyra_uncredentialed_websocket_cannot_retire_another_offer` (403 no token / 404 wrong token or wrong dialog / 426 the holder), plus the Chromium harness |
| **R2** the listener's ingress runs no bounds | native ingress ran `validate_size` + the per-peer signal budget; the bootstrap path called `accept_bootstrap_offer` directly, which ran neither — it DID charge the claimed node id's dialog allowance, which is the attribution half of the same defect | `RtcSignalMsg::validate_size` + `MeshNode::admit_signal_frame` are now the one shared gate; `accept_bootstrap_offer_keyed` / `apply_bootstrap_candidate_checked` charge the **token's** random id — an identity the caller cannot choose — and `open_signal_dialogs` counts bootstrap dialogs too | `kyra_bootstrap_candidates_must_keep_the_native_size_bound`, `kyra_bootstrap_candidates_must_keep_the_native_frame_budget`, `an_http_caller_cannot_spend_the_claimed_nodes_budget` |
| **R3** the credential's expiry is unverified | the credential was **unsigned** — a serialized struct whose deadlines any recipient could edit; nothing bound it to an issuer, so a listener could not tell a minted credential from a forged one | format version 2 carries an **Ed25519 issuer signature** over `signing_bytes()`; `BootstrapConfig::credential_issuer` is required; `anchor credential mint --issuer-identity` signs | `kyra_expired_credential_holder_cannot_extend_its_own_deadlines`, `a_credential_from_another_issuer_is_refused` |
| **R5** the offer body prints the credential | `#[derive(Debug)]` on `OfferRequest` put the bearer credential in any error/trace log; the ACME account key was written then chmod'd | a redacting `Debug`; the key is created `0o600` **at inception** with a post-write verification, and a `#[cfg(unix)]` fault-injection test proves the verification fires | `kyra_offer_debug_must_not_expose_the_encoded_credential`, `the_private_key_is_created_0600_and_a_wrong_mode_is_fatal` |
...
## 12. The second repair round (Kyra's second HOLD)

Kyra held again at `3d08a2df0`, crediting R3 outright and several
subclaims elsewhere. Seven of her follow-up probes reproduced real
defects; the CI and NAT evidence at that head was red. This section
is the disposition of that amended ledger. **Receipts in this
section are for THIS candidate; §10's 52/52 belongs to `871e0138d`
and is not re-used as evidence.**

| Finding (round 2) | What was actually wrong | Repair | Witness / receipt |
| --- | --- | --- | --- |
| **R1** exact attempt ownership | the middleware matched the token's `(node, dialog)` and upgraded; a token whose CORE attempt had expired, been rejected or completed still authorized a socket (she observed the candidate path reporting no dialog behind a token that reached 426) | the anchor records `(claimed node, dialog) -> accounting identity` at acceptance; the layer requires that exact row via `bootstrap_attempt_is_live` and retires the token when it is gone | `an_attempt_that_ended_on_the_anchor_stops_authorizing_its_token`; RED with the liveness check removed |
| **R2** the actual reservation leaks | ingress is charged to the attempt's random identity; every terminal path released `(claimed node, dialog)`, so an owner close retired the token and left the random reservation admitting frames | the owner record is consumed by `release_signal_budget` — the single point owner close, expiry, our Reject, the peer's Reject and completion all already go through — and the expiry sweep takes the same route | `ending_an_attempt_releases_the_identity_its_ingress_was_charged_to`, whose observable is the budget refusing a frame under that identity; RED when only the claim is released |
| **R2** the observer | `open_signal_dialogs` returned `max(budget, side counter)`; the counter was incremented after the completion spawn and decremented on one teardown, so it reported an open attempt after the row was gone and zero while a reservation was live | the side counter is deleted: the value is the peer's budget reservations plus its live attempt rows, and both are consumed by the same release | asserted inside the two witnesses above |
| **R2** serialized-size parity | a candidate whose `candidate + mid` landed exactly on the limit passed the field sum and was refused by the native decoder, which bounds the encoded bytes | `validate_size` measures the encoding (`serialized_size`, no allocation) | `a_frame_at_the_field_limit_is_refused_by_both_ingresses` |
| **R4** future `not_before` accepted | the cache checked `not_after` only | the whole window is enforced | `a_certificate_that_is_not_valid_yet_is_not_served` |
| **R4** CN overrode a wrong SAN | CN and SAN were OR'd | SAN is authoritative; CN only when there is no SAN extension | `a_matching_common_name_cannot_override_a_wrong_san` |
| **R4** a failed startup kept its port | the challenge task's handle was dropped, which DETACHES it | startup failure, TLS-bind failure and shutdown all `abort_and_join` | `a_failed_acme_startup_releases_its_challenge_port` — the observable is the port binding again; RED without the join |
| **R4** unbounded successful renewals | only errors backed off, so a lifetime inside the horizon re-ordered immediately for ever | a floor between successful orders; the horizon still governs when it is further out | source; the pacing constant is `MIN_RENEWAL_INTERVAL` |
| **R4** publication could destroy a usable pair | certificate written over the live one, then the key deleted to recreate it | both halves staged as siblings and renamed only once both exist | `a_failed_publication_leaves_the_previous_pair_intact` |
| **R4** challenges outlived failures | `set_ready`/`poll_ready` returns left key authorizations installed | a Drop guard retires this order's tokens on every exit | source |
| **R5** the response printed the token | `OfferResponse`'s derived `Debug` carried the full bearer | redacting `Debug` | `kyra_r_response_debug…`'s property, asserted in the redaction test |
| **R5** the protection check had no failing case | nothing ever made a created key fail it, so deleting the check kept every assertion green | the observation is injectable and a permissive reading is exercised | `a_key_that_fails_its_protection_check_is_an_error_and_is_removed` |
| **R5** the library witnesses never ran | the listener step selects integration tests and the SDK suite's feature list has no `rtc-bootstrap` | a new CI step runs `--lib --features rtc-bootstrap` on Linux and pins six names | CI step "SDK library witnesses under rtc-bootstrap (Stage 4b)" |
| **R6** Deck was still unwired | the comment claimed a mesh; both constructors passed none | `runtime.rs` boots the deck's own node and attaches it, `demo/spawn.rs` attaches node[0]'s, and the column distinguishes "no anchors" from "not in this build" | `the_rollup_the_anchor_column_renders_carries_an_ingested_anchor` (RED without `.with_mesh`), plus a painted-table test |
| **R6** the pin guard | `-p net-cli --test anchor_ls_live` named a crate the guard does not inventory | the step runs the crate with the feature and greps both test names — no `--test` form | CI job "Integration test pin guard" |
| **R8** provenance | the client transmitted to the announced address but `learned=peer-reflexive`: `trickle_local_candidate` signalled `driver.local_addr()`, the BOUND address, so behind a NAT the announcement contributed nothing to gathering | the native trickle frame signals `rtc.public_addr` when configured, as `bootstrap_host_candidate` already did; the scenario's anchor also turns `serve_stun` on, which was off and over-determined the probe's refusal | `natsim_natted_anchor_publishes_a_reachable_rtc_addr` asserts `selected_remote` **and** `selected_learned == "signalled"` |

### Two things this round does NOT claim

- **The unsolicited STUN probe is evidence, not a verdict.** The
  natsim gateway is address-restricted by construction
  (`iifname gw?-wan udp dport <rtc> ct state new drop`), so a
  stranger's binding request is dropped there whatever the anchor
  does. What the scenario now asserts is the provenance of the pair
  the client uses.
- **The reporting-order deviation in R3 stands**, as Kyra accepted:
  acceptance always requires the issuer signature; the refusal
  message still names expiry first.
