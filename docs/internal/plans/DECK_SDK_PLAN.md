...
**Phases 4–7 shipped.** All four bindings ship slices 1+2+3 (admin / audit-log-failure streams / ICE break-glass with typestate-enforced simulate→commit) **plus** the operator-policy verifier surface: `OperatorRegistry` + `AdminVerifier` + `VerifyError` discriminator + `OperatorIdentity.sign_proposal()` / `sign_payload()` + `SimulatedIceProposal.signing_payload()` (the deterministic envelope bytes for offline / cross-deck signing flows). Remaining items are cross-cutting only: runnable examples + parity test (enumerated below).
...
- **Phase 4 — Python SDK. SHIPPED.** `bindings/python/src/deck.rs` + `sdk-py/src/net_sdk/deck.py`. Slices 1+2+3 + operator-verifier surface: `DeckClient` + `snapshots()` iterator + `status_summary_stream` + `AdminCommands` (9 methods), `AuditQuery` fluent builder + `subscribe_logs` + `subscribe_failures`, `IceCommands` (7 factories) + typestate via separate `IceProposal` / `SimulatedIceProposal` classes, `OperatorIdentity` (generate / from_seed / from_identity / operator_id / public_key / sign_proposal / sign_payload), `OperatorRegistry` (insert / register / contains / verify / verify_bundle), `AdminVerifier` (new / with_freshness / with_full_policy + policy-knob getters), `VerifyError` mapped through `DeckSdkError.kind` (`not_authorized` / `signature_invalid` / `insufficient_signatures` / etc.), `SimulatedIceProposal.signing_payload()` (deterministic envelope bytes).
- **Phase 5 — Node / TypeScript SDK. SHIPPED.** `bindings/node/src/deck.rs` + `sdk-ts/src/deck.ts`. Same surface as Python via napi-rs + AsyncIterable shim: `DeckClient`, all admin methods (async, returning typed `ChainCommit`), `AuditQuery` fluent builder + `subscribeLogs` + `subscribeFailures` AsyncIterables, `IceCommands` + napi typestate (separate `IceProposal` / `SimulatedIceProposal` classes), `OperatorIdentity` (`generate` / `fromSeed` / `fromIdentity` / `operatorId` / `publicKey` / `signProposal` / `signPayload`), `OperatorRegistry`, `AdminVerifier`, `signingPayload()` getter. TS wrapper exposes `DeckSdkError extends Error` with `.kind`.
- **Phase 6 — Go SDK. SHIPPED.** `bindings/go/deck-ffi/src/lib.rs` + `bindings/go/net/deck.go`. Same surface via cgo: `DeckClient` + `Snapshots(ctx)` channel + admin methods, `AuditQuery` fluent builder + `SubscribeLogs(ctx, filter)` + `SubscribeFailures(ctx, sinceSeq)`, `IceCommands` (7 factories) + separate `IceProposal` / `SimulatedIceProposal` opaque types + `Commit([]DeckOperatorSignature)`, `DeckOperatorIdentity` (`Generate` / `FromSeed` / `OperatorID` / `PublicKey` / `SignProposal` / `SignPayload`), `DeckOperatorRegistry` (`Insert` / `Register` / `Contains` / `Len` / `Verify` / `VerifyBundle`), `DeckAdminVerifier` + constructors + getters, `SimulatedIceProposal.SigningPayload()` (allocates JSON on the cdylib side; wrapper releases after copy). Finalizers hooked on every opaque handle.
- **Phase 7 — C SDK. SHIPPED.** `include/net_deck.h` + the shared `deck-ffi` cdylib. Same surface as Go since they share the cdylib: client lifecycle + admin commits + audit query builder + log/failure stream callbacks + ICE typestate (two distinct opaque handles) + `net_deck_operator_identity_*` + `net_deck_operator_registry_*` + `net_deck_admin_verifier_*` + `net_deck_simulated_signing_payload` / `net_deck_signing_payload_free`. Last-error trio mirrors `net_meshdb.h` / `net_meshos.h`.

All four language bindings now reach "v1 done" for the Deck operator-command surface.
...
The per-binding operator-command surface is feature-complete. What remains is cross-cutting polish that doesn't gate "v1 done":
...
- **Cross-deck co-signing workflow** (see **Deferred work** section) — `IceProposalBundle` (Serialize + Deserialize) + bundled re-exports of `blast_radius_hash` / `ice_proposal_signing_payload` for paste/file exchange between operator decks. **Status: indefinitely deferred.** The offline-signing primitives every binding already exposes (`SimulatedIceProposal.signing_payload()` + `OperatorIdentity.sign_payload(payload)`) are sufficient — adding the `IceProposalBundle` wrapper waits on a consumer that drives multi-op coordination. The default `ice_signature_threshold = 1` makes this unnecessary for current cluster operations.
