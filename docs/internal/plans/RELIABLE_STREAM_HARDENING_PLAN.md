...
- **H-8 ✅, then SUPERSEDED** — originally closed as "no live bug": the substrate delivered arrival order plus `seq`, transfer reordered itself, nRPC framed its own order, and a general in-order buffer was deferred because no consumer needed one. **Stage 5's browser leaf is that consumer.** The deferred buffer now exists: the native receive path holds out-of-order arrivals per reliable stream in a window-bounded buffer, drops duplicates and releases in sequence order, and the leaf's `RxStream` does the same on its side. `TRANSPORT.md`'s ordering contract ("FIFO delivery within the stream; the receive side reorders into sequence", with "no ordering across streams" as the stated exception) is therefore the live contract, and this bullet's original wording — plus the `Reliability::Reliable` docstring it "corrected" for claiming in-order — describes the transport as it was before that work, not as it is. Kept rather than deleted because the reasoning was sound at the time and the reversal is the interesting part: the deferral was contingent on having no consumer, and it stopped being true.
