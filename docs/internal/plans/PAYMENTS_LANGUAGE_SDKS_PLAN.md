...
| Outbound HTTP-402 (`fetch_paid`) | ✅ | ✅ (opt-in `payments-http`) | WS-T3 | deferred | deferred |
| Signer seam: eip155 typed-data callback | ✅ | ✅ | WS-T2 | WS-G | WS-G |
| Signer seams: svm / xrpl intents | ✅ | ✅ | deferred | deferred | deferred |
...
- [x] **P2 — HTTP-402 client** (subsumes deferred N4a): `PaymentHttpClient` (+ `AsyncPaymentHttpClient`) over `X402HttpFlow::fetch_paid` — same payment kwargs as the gateway (policy path **required** — the caller's spend policy is the entire gate), plus an optional payer `identity` handle (ephemeral if omitted). `fetch_paid(url)` returns a `(status_json, body)` tuple: the `X402HttpOutcome` projection (`fetched` / `paid` / `requires_payment_approval` / `denied` / `provider_refused` / `transport_error`; `paid` carries the byte-preserved settlement as base64) and the raw body as `bytes`. Behind a NEW opt-in `payments-http` feature (pulls `net-payments/http-facilitator` = reqwest/rustls — a real size add kept OUT of the default wheel; built in the CI python-tests job so the rows run). Rust projection + driven `transport_error` tests; pytest rows (both duals) + stub + `__init__.py` re-export.
- [x] **P3 — svm/xrpl signer seams**: `payment_signer_svm(_address)` / `payment_signer_xrpl(_address)` kwargs on both gateway classes, mirroring the eip155 contract (typed intent JSON in → signed artifact string out — base64 SVM tx / hex XRPL blob), bridged to `ExternalSvmSigner`/`ExternalXrplSigner` (registered under `solana`/`xrpl`) under the same `spawn_blocking` + `Python::attach` pattern. Each pair is both-or-neither (shared `signer_pair` validator); an absent pair means that namespace is simply skipped at selection (existing Rust semantics — no new policy). All three schemes coexist on one gateway.
- [x] Tests: Rust `collect` arity/both-or-neither test extended to all three schemes; pytest rows for svm/xrpl construction + both-or-neither + all-three-coexist, and the no-key-material negative test extended to the new signer namespaces (still no key kwarg anywhere); stub + docstring updated.
