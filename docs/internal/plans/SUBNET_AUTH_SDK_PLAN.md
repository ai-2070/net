...
- [x] Rust, TypeScript, Python, and Go lead with only named-export serve plus exported call; C mirrors the same application boundary. *(C takes the concrete `net_subnet_ref_t` at the ABI seam; name resolution is the language layer's, per §6.4.)*
- [x] No public caller surface is named `call_subnet` / `callSubnet` or accepts a subnet reference, credential, or export binding.
- [x] Public candidate ownership is verified and sampled coherently. *(S1.)*
...
- [ ] Every binding can configure roots, attachment, and named exports; install/declare/apply operations are clearly separated as advanced administration. **DELIBERATE SCOPE REDUCTION** — Rust/Node/Python meet this in full. Go configures named exports (`MeshConfig.SubnetExports`) and C takes the concrete binding, and both carry the full install/declare/apply admin surface — but neither can configure roots/attachment/control-channel (construction-time `MeshNodeConfig` state; base libnet cannot reach the SDK's single DTO conversion module). Decided 2026-08-04: ship now, defer; documented in `go/subnet.go`, `include/net_subnet.h`, and the coverage record (`partial`). Closing it means relocating the conversion module into the core crate.
...
- [x] The required `net + cortex + fixtures` E2E gate is green at exact head. *(Locally at `b6335d133`; the pinned CI family must confirm on its runners.)*
...
- [ ] Every language consumes the Rust-generated stable-kind fixture without drift. **FOUR OF FIVE** — Rust, Node, Python, and Go each carry a fixture-consuming suite. C has no independent fixture consumer: its vocabulary is the compiled-in Rust single source crossing `out_err`, and the org-ffi tests pin layout/codes but do not read `stable_kinds.json`. Review point: accept as-is (C has no runtime classifier to drift) or add an org-ffi fixture-reading test.
- [x] Rust, Node, Python, Go, and C package/compile tests are green. *(On this host: full battery in the S6 commit message. Go's cgo test binary and the C executable examples are CI-owed — symbol-level verified locally; environmental limits documented there.)*
- [x] Skills, public docs, examples, coverage anchors, generated declarations, and release notes match the shipped symbols. *(No NEW runnable example was shipped — docs and skill carry snippets only; the coverage anchors are CI-checked symbols. Review point if a runnable example is wanted for release.)*
...
1. **Go/C construction-time trust anchors** — deferred by decision (see the unchecked §11 item). Serve/call/admin are complete in both; a gateway node is constructed from Rust/Node/Python.
2. **X2 cross-language live cell** + `gen_subnet_scenario` — CI-owed, not shipped; the Node/Python live suites deliberately do not fabricate it.
3. **Go cgo test binary + C executable examples** — CI-owed (need all sibling cdylibs and a single-toolchain link; the MSVC/mingw duplicate-`net::ffi`-symbol constraint is documented in `org-ffi/Cargo.toml`). Locally verified to the symbol level: `go build`/`go vet` green, all five new exports present in the built DLL, zero undefined references in the test link.
4. **No C fixture consumer** for `stable_kinds.json` (§11 review point).
5. **No new runnable example** (§11 review point).

Cross-binding invariants a reviewer should hold the branch to: the two-verb surface with no `call_subnet` anywhere; one Rust conversion module (no binding re-derives DTOs); wholesale-replace admin semantics with decode-all-before-install; the wrapped-envelope classification rule (`subnet:<kind>` scanned, not prefix-matched, in every binding); and `applied: false` as an authenticated no-op, never an error.
