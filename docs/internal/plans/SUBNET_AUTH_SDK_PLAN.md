...
**Status:** IMPLEMENTED (S0–S6, 2026-08-04) on `subnet-sdk`, head `b6335d133` — awaiting review; see §12 for the per-phase record and the recorded deviations
...
**Status: HOLD.** The 2026-08-04 review (`docs/internal/misc/CODE_REVIEW_2026_08_04_SUBNET_SDK.md`)
returned eight blocking findings. The repair chain has landed on `subnet-sdk`
(P1-1 … P1-7, P2-1 … P2-3, P3-1 … P3-7, plus PERF §1/§2), and the remaining
open item is the per-language live matrix (P1-8). Release acceptance stays
UNCHECKED until that matrix exists and full CI is green at the exact repaired
head.
...
- [x] External callers present organization authority only and receive no subnet context.
- [x] Exact gateway, boundary, `EXPORT`, topology epoch, and provider-local admission are revalidated per call.
- [x] Authority movement yields one uncharged `AuthorityChanged` and no SDK retry. *(`live_exported_service_darkens_on_authority_movement_and_recovers`.)*
- [x] Every binding can configure roots, attachment, and named exports; install/declare/apply operations are clearly separated as advanced administration. *(Closed by P1-7/P1-6. The conversion module moved into the core (`net::adapter::net::subnet::provision`), so base libnet's JSON constructor reaches it: `net_mesh_new` now takes `subnet_authorities`, `subnet_attachment`, `subnet_control_channel`, and `subnet_exports`, and Go surfaces them on `MeshConfig`. `net_sdk::subnet` re-exports the moved types, so there is still one definition. The earlier "ship now, defer" decision is superseded — it left Go and C with an advertised provider verb that could never produce an authorized service.)*
- [x] Signed artifacts are generated by the CLI and consumed as canonical opaque bytes.
- [x] Private root material crosses only through checked file paths.
- [ ] The required `net + cortex + fixtures` E2E gate is green at exact head. *(Was green at `b6335d133`; the repair chain has moved the head substantially. Must be re-run and confirmed on CI runners at the repaired head before this is checked again.)*
- [x] Existing `subnet_route_hop_alloc` remains green and independently pinned.
- [x] The new production-path `subnet_relay_alloc_e2e` target exists, is green, and is pinned exactly once in its specified CI family. *(Pin landed in S6; run green under the exact pinned invocation.)*
- [x] Focused inverse tests prove unrelated placement, wrong crossing, stale control, and unowned public discovery fail closed.
- [x] Every language consumes the Rust-generated stable-kind fixture without drift. *(Closed by P3-2: org-ffi now carries `c_surface_consumes_the_shared_stable_kind_fixture`, which puts every pinned kind through the exact scan a C caller performs over an `out_err` wire, in both the bare and wrapped envelope shapes. P3-1 additionally made the fixture gate a BYTE comparison against the single renderer rather than a field-by-field check.)*
- [ ] Rust, Node, Python, Go, and C package/compile tests are green. *(Green per-commit through the repair chain on this host — core/SDK/org-ffi cargo suites, Node vitest, `go build ./...`. Owed at the repaired head: a full battery on CI runners, Go's cgo test binary, and the C executable examples. `go vet` fails on a pre-existing unrelated `MeshOsDefaultDaemon` symbol, confirmed identical on a clean stash.)*
- [x] Skills, public docs, examples, coverage anchors, generated declarations, and release notes match the shipped symbols. *(Re-reconciled after the repair chain: the frozen provider verbs (`mesh.serveSubnetExported` / `mesh.serve_subnet_exported`), the scanned-envelope classification rule, the Go/C trust-anchor examples, and the offline-issuance validation boundary. No NEW runnable example was shipped — docs and skill carry snippets only. Review point if a runnable example is wanted for release.)*
- [x] Organization load balancing remains dark in API, docs, examples, and release notes.
- [x] `git diff --check` is clean and no generated dependencies or temporary validation artifacts are committed.

---

## 12. Implementation record (2026-08-04)

One commit per phase on `subnet-sdk`; each commit message carries its own verification record. Verified on the implementation host (Windows, `cargo -j 4`; Node 24/npm 11; Python 3.11/maturin; Go 1.26 + mingw gcc 15.2).

| Phase | Commit | Landed |
|---|---|---|
| S0 | `90bdadd6d` | RED contracts + `subnet_relay_alloc_e2e` production-relay witness |
| S1 | `c279c1ab7` | Public owner query + `OrgClient::call_exported` |
| S2 | `b6ce6f267` | `net_sdk::subnet` facade, `NamedSubnetExports`, admin namespace, stable-kind fixture |
| S3 | `1a8782603` (+`be825f33e` lockfile) | `net-mesh subnet` offline issuance (`keygen`/`issue-*`/`inspect`) |
| S4a | `2ad97aebb` | Node/TS adapter (napi build + 8/8 vitest) |
| S4b | `e43311bf2` | Python adapter (maturin build + 74/74 pytest) |
| S4c | `457e966f7` | C ABI (11/11 org-ffi tests; header TU-compiles in C and C++) |
| S4d | `e86af2a46` | Go adapter (gofmt/build/vet; symbol-level cgo verification) |
| S5 | `4ba52e257` | Skill chapter + 11 public docs pages + 3 coverage rows (102/102 anchors) |
| S6 | `b6335d133` | `subnet_relay_alloc_e2e` CI pin + full local gate battery |

**Placement decision recorded in S4c:** all subnet C ABI ships in the existing `libnet_org` cdylib — the plan's base-libnet default is defeated by the link analysis §6.4 invites (`net::ffi` lives inside the `net` crate and cannot depend on `net-mesh-sdk`; org-ffi already does). `net_subnet.h` includes `net_org.h`; `NET_ORG_ERR_SUBNET` (−13) lives in the org numeric-mirror contract. No new cdylib.
...
Cross-binding invariants a reviewer should hold the branch to: the two-verb surface with no `call_subnet` anywhere; one conversion module in the CORE that no binding re-derives; wholesale-replace admin semantics with decode-all-before-install; the wrapped-envelope classification rule (`subnet:<kind>` SCANNED, not prefix-matched, in every binding); an export NAME resolved by Rust-owned node state at every boundary; a discovery candidate whose verified publisher must equal its live session pin; and `applied: false` as an authenticated no-op, never an error.

---

## 13. Review-10 repair record (2026-08-05)

The 2026-08-04 review returned HOLD with eight blocking findings. Repairs landed
one commit per finding-group on `subnet-sdk`; each commit message carries its own
verification record, including mutation checks where a witness could otherwise
pass vacuously.

| Finding | Commit | Repair |
|---|---|---|
| P1-1, P1-2 | `ee368c813` | `VerifiedOwner` carries the verified publisher; the discovery seam requires the live pin to equal it; conflict exclusion spans the publisher's whole live footprint |
| P1-3, P1-4 | `8aaeb610b` | Every C array count bounded before its slice exists; `mesh_arc` consumed on every refusal path |
| P2-1 | `55325b3f6` | Node and Python scan for the embedded `subnet:` envelope; Python natives carry `.kind` |
| P2-2, P3-1 | `1ba87262a` | One core `wire_kind()`; the fixture gate is a byte comparison against the single renderer |
| PERF §1, §2 | `9c01882f1` | Global publish verdict hoisted out of the per-subscriber loop; `SubnetPolicy::assign` borrows instead of rendering per tag |
| P3-2 … P3-7 | `4a5eb1493` | C fixture consumer, allocation-witness sensitivity control, `NET_SUBNET_*` drift guard, rustdoc placement, narrowed TS cast, issuance-boundary docs |
| P1-5 | `e937d0194` | `mesh.serveSubnetExported` / `mesh.serve_subnet_exported` on the high-level SDKs; `Typed` demoted to a deprecated alias |
| P2-3 | `941fda5cc` | Both subnet facades refuse at IMPORT on a feature-off build |
...
`net_subnet_ref_t` is gone), and `@net-mesh/core/subnet` / `net.subnet` now
throw at import on a feature-off build.
