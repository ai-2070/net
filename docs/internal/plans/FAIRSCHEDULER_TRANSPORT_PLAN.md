...
- **Concurrent large-file transfers — diagnosed + fixed.** A fan-out sweep (`tests/transfer_concurrency.rs`) exposed two distinct bugs the single-stream tests never hit:
  ...
  - **Lossy-link loss recovery — DONE** (commit `78bfc4ee2`, `STREAM_RETRANSMIT_PLAN.md`). The reliable-stream retransmit was entirely unwired (machinery present, but `on_send` not called on the mesh path, no NACK emitted, no `spawn_retransmit_loop`); a lost packet was a permanent gap → 30 s timeout. Now wired end-to-end: register-on-send, receiver NACK-on-gap (`SUBPROTOCOL_STREAM_NACK`), sender resend-on-NACK, and a timeout backstop for tail loss — plus the receiver-reliability fixes (create-reliable on the RELIABLE flag; requester opens its receive stream reliable; `serve_chunk` waits for ack before close). Verified: mid-stream + tail loss recover, and the concurrent-4 MiB sweep now passes k=2..8 (retransmit even recovers recv-buffer-overflow drops, making the `fetch_dir` in-flight budget belt-and-suspenders rather than load-bearing). Benefits every reliable stream, not just transfer. Callers issuing many concurrent `transfer_fetch_chunk` directly (not via `fetch_dir`) should still bound their own concurrency for throughput.
