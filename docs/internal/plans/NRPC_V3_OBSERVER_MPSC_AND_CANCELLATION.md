...
6. **Coordinated mpsc bound across bindings.** Hard-coded `1024` per binding is fine for v3; a future tunable (via env var or per-mesh config) lets ops staff size the queue to their observer's actual cost. Wait until a user actually files a "1024 is too small" issue. (v3-final note: bound is now centralized at `cortex::OBSERVER_BUFFER_CAPACITY` via N4, so a future tunable is a one-place change.)
7. **`CancelToken(NonZeroU64)` newtype.** Today `cancel_token: u64` flows raw through `MeshNode::reserve_cancel_token`, `CallOptions::cancel_token`, and the FFI surface. A newtype with a `NonZero` constraint would express the "0 is the no-token sentinel" rule at the type level instead of inline checks. Substantive surface change touching all three bindings — defer until a real consumer bug attributes to passing 0 or another mesh's token by mistake (current orphan-TTL GC makes those safe no-ops anyway). N9 in `CODE_REVIEW_2026_05_24_NRPC_V3.md`.
8. **Code-generator for per-binding POD mirrors.** `RpcCallEventJs` (napi), `PyRpcCallEvent` (pyo3), and `RpcCallEventC` (rpc-ffi) each carry the same 8-field shape + a parallel `From<&InnerRpcCallEvent>` converter. Inevitable for FFI but mechanical — a small codegen step from the substrate's `RpcCallEvent` definition would prevent drift when the substrate's event shape gains fields. N12 in `CODE_REVIEW_2026_05_24_NRPC_V3.md`.
