...
**Next authorized unit:** Finish the large-response acceptance slice: exercise partial-transfer deadline and session teardown through live calls, test queue saturation/stalled credit and buffer cleanup, and review sender transfer lifetime. The current response drainer can still drop pieces under load, and the one-second credit wait is per packet rather than a shared server transfer deadline. Local 22 KB success is now real, but does not close these lifecycle/fault-injection gates or the brief. Then resume the two-node CI journey and final V2 acceptance. V3 remains unstarted.
