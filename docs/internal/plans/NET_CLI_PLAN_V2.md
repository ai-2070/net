...
**Status:** Implementation authorized in the working session. CLI-1 preflight/durable restoration, DOC-0, and CLI-2A are implemented and locally verified. CLI-2B is partially implemented: aggregator list resolution/inspection and strict profile-loader selection are verified; CLI-wide inspection and bind work remain. Platform CI and the requested two-node CI journey remain outstanding; off-host readiness is not claimed.
...
**Remaining CLI-2B review units:** Extend the common inspection/resolution approach to other mesh clients, hosted services, offline artifacts and persistent stores; enforce explicit selections on currently bypassing commands without interpreting unrelated profile defaults as remote intent; cover unsupported target flags. Then expose/validate the selected journey's supported bind and add non-loopback evidence. `--inspect-target` is not global yet, and the short-lived client still binds to the shared `127.0.0.1:0` default. These boundaries are stated in public docs; CLI-2B is not closed.
