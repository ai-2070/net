...
**Next review unit:** Provider-bound metadata hydration and fail-closed schema usability before publication, followed by offline regeneration/downstream acceptance. Source inspection found existing public provenance in `Mesh::inner().capability_fold()` (node ID plus per-entry tags/metadata), while `list_tools` aggregates away provider identity. Use that existing surface to select exact `(tool ID, version, provider)` tuples and define conflict behavior before fetching. Real oversized-schema, NotFound/mismatch/remaining-budget failure tests and deterministic regeneration after provider shutdown are still required. CLI-3 is not complete; the two-node journey and exact-head V2 acceptance still precede V3.
