...
**Remaining CLI-2B review units:** Extend inspection to the remaining local command families (other offline issuance/artifacts, MCP pins, forwarding policy, staged transfers and temporary supervisors) and standalone anchor serving; finish unsupported-target coverage. `--inspect-target` is deliberately not global yet. Exact-head platform CI and the full two-node publisher/consumer journey remain outstanding; CLI-2B is not closed.
