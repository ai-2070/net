...
**Status:** Implementation authorized in the working session. CLI-1 preflight/durable restoration, DOC-0, and CLI-2A are implemented and locally verified. CLI-2B is partially implemented: remote-client/hosted-shim inspection, strict profile-loader selection, and explicit/profile bind resolution are implemented. Offline/persistent-wide inspection, bypassing-command selection validation, platform CI and the requested two-node CI journey remain outstanding; off-host readiness is not claimed.
...
**Remaining CLI-2B review units:** Extend inspection to offline artifacts, persistent stores and standalone anchor serving; enforce explicit config/profile selections on currently bypassing commands without interpreting unrelated defaults as remote intent; finish unsupported-target coverage. `--inspect-target` is deliberately not global yet. Exact-head platform CI and the full two-node publisher/consumer journey remain outstanding; CLI-2B is not closed.
