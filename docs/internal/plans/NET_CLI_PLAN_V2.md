...
**Status:** Implementation authorized in the working session. CLI-1 preflight/durable restoration, DOC-0, and CLI-2A are implemented and locally verified. CLI-2B is partially implemented: remote-client/hosted-shim inspection, NetDB/saved-typegen inspection, explicit selector validation before dispatch, and explicit/profile bind resolution are implemented. Remaining local/standalone-service inspection, platform CI and the requested two-node CI journey remain outstanding; off-host readiness is not claimed.
...
**Remaining CLI-2B review units:** Extend inspection to the remaining temporary supervisors (snapshot, audit/log/failures, peer/daemon/channel, aggregator inspect, admin/ICE) and the keychain value target; finish unsupported-target coverage. `--inspect-target` is deliberately not global yet. Exact-head platform CI and the full two-node publisher/consumer journey remain outstanding; CLI-2B is not closed. V3 remains a proposed follow-on and is not interleaved with this work.
