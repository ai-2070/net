...
**Remaining CLI-2B review units:** Extend inspection to organization/subnet/bootstrap credential issuance and adoption, temporary supervisors and the keychain value target; finish unsupported-target coverage. `--inspect-target` is deliberately not global yet. Exact-head platform CI and the full two-node publisher/consumer journey remain outstanding; CLI-2B is not closed. V3 remains a proposed follow-on and is not interleaved with this work.
