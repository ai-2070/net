...
**Next authorized unit:** Negotiate and implement bounded multi-packet unary response delivery, restoring successful 22 KB typegen capture. Pin opt-in/envelope compatibility, per-call/aggregate memory bounds and cancellation/session cleanup before implementation, with the rejection behavior above retained for unsupported peers and over-limit payloads. The original timed-out 22 KB response is now an explicit error, not a solved large-response workflow. Only after that transport work should the two-node journey and final V2 acceptance resume; V3 remains unstarted.
...
**Transport-size finding / remaining acceptance:** A roughly 22 KB metadata response failed on the existing unary RPC receive path (`WSAEMSGSIZE`; `MAX_PACKET_SIZE = 8192`) and the caller timed out without publication. Reducing the fixture to roughly 6 KB exercises successful fetch, not a fix for large-response transport. Do not claim arbitrary oversized-schema support or close that evidence gap. A transport fix requires its own bounded review; this slice changes neither the core RPC protocol nor SDK APIs. Next local workflow work is the documented two-node MCP/native CI journey using a contract within the verified transport range, followed by explicit resolution of this size limitation and exact-head V2 acceptance. V3 remains unstarted.
