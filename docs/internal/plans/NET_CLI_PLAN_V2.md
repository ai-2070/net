...
**Remaining CLI-2B review units:** Extend inspection to other offline issuance/artifacts, temporary supervisors, standalone anchor serving and the keychain value target; finish unsupported-target coverage. `--inspect-target` is deliberately not global yet. Exact-head platform CI and the full two-node publisher/consumer journey remain outstanding; CLI-2B is not closed.
