...
**Eighth review unit — implemented 2026-09-20 (`34c37b7f9`):** Added native pseudoterminal subprocess tests using test-only `portable-pty` for interactive YES acceptance, NO refusal, `--yes` prompt bypass and dry-run prompt bypass. PTYs merge terminal output, so these establish terminal behavior; separate stdout/stderr framing remains the pipe-driven automation tests' responsibility. Extracted the existing confirmation/sign/commit sequence into a shared helper without changing policy or output behavior. A real SDK two-signature quorum rejects the helper's `--yes` path with policy-rejected for both TTY/non-TTY inputs, leaves the local freeze absent, and accepts a second operator signature as a positive control. This is SDK quorum-policy evidence, not a deployed-authority subprocess test; the temporary CLI supervisor does not expose such a configurable authority.

Evidence: restoring the old TTY-specific `--yes` behavior made the real terminal witness fail immediately on its unexpected prompt; the inverse was removed and all **14 focused confirmation/automation cases passed**. Full Windows suites: **302 default passed**, **315 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target compilation, strict default binary clippy, formatting, diff hygiene and index refresh passed. Windows native PTY behavior is verified locally; Unix terminal behavior still requires exact-head CI. CLI-DX's enumerated local implementation/evidence backlog is now closed at these stated boundaries; this is not final platform or V2 acceptance. Continue CLI-3 live typegen, then cumulative two-node journey and exact-head CI. V3 remains unstarted.

**Seventh review unit — implemented 2026-09-20 (`a846359d0`):** Added `rpc_deadline` real-mesh/CLI subprocess evidence for the uncertain-effect/no-retry contract. The handler decodes the actual transfer-cancel request and increments a synthetic non-idempotent effect counter before withholding its response. The CLI exits 7 with empty stdout and the uncertain-effect/no-retry warning; the handler is then allowed to finish after caller exit. Its counter stays at one until a second explicit fast-reply invocation succeeds and advances it to two. This establishes handler-side CLI retry behavior, not a new transfer-engine cancellation or remote rollback guarantee. No production code changed.

Evidence: deliberately bypassing the CLI's outer deadline made the witness fail (SDK timeout exit 3 instead of CLI deadline exit 7); restoring it returned GREEN, and no inverse edit remains. Unsupported-deadline coverage now explicitly includes temporary-local log/failure streams, alongside audit streaming; these keep pre-effect rejection instead of receiving artificial lifetime deadlines. The selected capability-journey service/stream startup boundaries are covered by the preceding MCP/wrap slices; universal deadline support is not required. Fixture configs use Unix mode 0600 (including the existing wrap fixture); actual Unix execution still belongs to CI. Full Windows suites: **297 default passed**, **310 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries; the final expanded unsupported-mode witness also passed separately. Default/optional all-target compilation, strict default binary clippy, formatting, diff hygiene and index refresh passed. Remaining CLI-DX acceptance: actual TTY confirmation/policy-denial evidence and exact-head CI. Live typegen and the documented same-runner two-node journey still follow; V3 is not started.

**Sixth review unit — implemented 2026-09-20 (`4ee3ca740`):** `wrap --timeout` now shares an absolute startup budget across configuration, identity loading, attachment and child MCP initialization/discovery/publication. A successful initial publication ends the deadline's scope: provider lifetime and later refreshes are not timed out. Startup cancellation drops the managed direct child; arbitrary descendants are not covered by a process-tree supervision guarantee. Inspection still rejects explicit deadlines. This uses existing client/serve-handle ownership rather than adding a supervisor framework.

Evidence: **11 focused automation/wrap tests passed**. The new `wrap_startup` CLI integration binary compiles a dependency-free Rust MCP fixture with the installed Rust compiler. A real mesh and wrapped-child control socket prove a stalled initialization returns exit 7, emits no `wrapped` result and closes the child's live socket. A successful publication advertises one tool, remains alive beyond its startup budget, then emits exactly the expected `wrapped`/`server_exited` NDJSON lifecycle records when the fixture exits. Other controls prove zero-budget/no-packet refusal, unavailable-peer expiry before trying the deliberately nonexistent child, inspection rejection and secret-free diagnostics. Final Windows suites: **296 default passed**, **309 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target checks, strict default binary clippy, formatting (including fixture source), web checks, diff hygiene and index refresh passed. The new CLI test is auto-discovered by the existing CLI CI jobs. Remaining: selected stream startup, real handler-side no-retry/uncertain-effect witnesses, actual TTY/policy-denial coverage, live typegen, the documented two-node journey and exact-head CI. V2 acceptance and V3 entry remain open.

**Fifth review unit — implemented 2026-09-20 (`c0db3157d`):** `mcp serve --timeout` now shares one startup deadline across configuration, operator identity loading, mesh attachment and shim construction. The deadline ends before protocol input: neither a delayed client initialization nor the running MCP session gets a lifetime limit. Inspection and local pin mutations remain unsupported with explicit deadlines; ordinary MCP stdout remains JSON-RPC even with `--output json`.

Evidence: eight focused automation tests passed. A real loopback mesh plus CLI subprocess initializes MCP, waits beyond its two-second startup budget, successfully requests `tools/list`, and shuts down via EOF; every stdout line is checked as a protocol response, with no trailing status envelope. Zero-budget/no-packet and unavailable-peer startup controls exit 7 without protocol output or PSK disclosure. Final Windows suites: **293 default passed**, **306 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target compilation, strict default binary clippy, formatting, web checks, diff hygiene and index refresh passed. This establishes the MCP startup/lifetime boundary, not all service/stream startup coverage. Remaining CLI-DX work includes wrap/selected stream startup, real handler-side no-retry/uncertain-effect controls, actual TTY/policy-denial and broader NDJSON coverage; live typegen, the two-node journey and exact-head CI remain ahead of V2 acceptance.

**Accepted scope clarification (2026-09-20):** The user agreed to finish the bounded V2 prerequisites without requiring directory-receive deadline support. `recv-dir` continues rejecting explicit `--timeout` before effects, as permitted by the unsupported-deadline contract below. Cancellation-safe directory reconstruction is a deferred enhancement, not a V2/V3 entry gate; earlier review-unit notes listing it as remaining implementation do not expand accepted scope. Finish CLI-DX startup/automation evidence, live typegen and the documented same-runner two-node journey, then record exact-head CI and V2 acceptance before requesting V3 authorization.

**Fourth review unit — implemented 2026-09-20 (`71d2db59d`):** `transfer recv-blob` now carries one absolute deadline through configuration, attachment and every network stream wait. Writes consume elapsed time but are not cancelled; final flush/rename proceeds outside cancellation after the complete stream is acquired. Timeout returns exit 7 without success output and leaves the destination unchanged; the existing `<out>.partial` staging file may remain. This is not resumable transfer or a strict total wall-clock limit. Directory receive remains explicitly unsupported: substrate `fetch_dir` launches reconstruction tasks and a blocking installation step, so wrapping the whole future would not safely cancel those effects. Its cancellation boundary needs a separate design/change; do not claim all receive deadlines are done.

Evidence: **17 focused tests passed**, including a real attached loopback peer that never serves the blob, preservation of an existing destination, zero-budget refusal before partial creation, and a successful budgeted byte-for-byte receive. A synthetic stream witness stalls after one chunk and proves expiry preserves both the original destination and staged chunk without publication. Inspection/staging/directory-receive timeout refusal remains covered. Final Windows suites: **291 default passed**, **304 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target checks, strict default binary clippy, CLI formatting, web checks, diff hygiene and index refresh passed. Next: service/stream startup budgets, then the remaining CLI-DX evidence; directory cancellation-safe integration, real handler-side no-retry controls, actual TTY/protocol coverage and exact-head CI remain open. V3 remains unstarted.

**Third review unit — implemented 2026-09-20 (`74480771f`):** Extended explicit `--timeout` to transfer ls/status/cancel and live typegen generate/snapshot acquisition. Typegen reuses one absolute deadline across explicit configuration validation, attachment and discovery. Rendering and output writes begin only after acquisition succeeds and are outside cancellation; total wall-clock completion may exceed the acquisition budget. Offline typegen, inspection and transfer receive/staging still reject explicit deadlines before effects. Existing five-second, inline-only discovery semantics are unchanged; CLI-3 remains outstanding. Boxed dispatcher/typegen futures after the existing man/completion witness exposed Windows stack overflows in both default and optional-feature builds.

Evidence: all 11 focused automation/transfer-admin tests passed. New subprocess cases cover zero-budget/no-packet behavior and unresponsive peers for each newly supported verb, exit 7 with empty stdout and secret-free diagnostics, and no generated destination on expiry. A real loopback peer with no tools exercises discovery timeout while preserving an existing snapshot, then successful snapshot publication when the budget permits the existing five-second observation. A real transfer-list RPC succeeds with an explicit budget. Unsupported-mode tests cover receive/staging, offline generation and inspection before destination creation. Final Windows suites: **289 default passed**, **302 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target compilation, strict default binary clippy, CLI formatting, web checks and diff hygiene passed. Remaining: cancellation-safe transfer receive budgets, service/stream startup budgets, handler-side no-retry/uncertain-effect controls, actual TTY integration, broader protocol/stream coverage and exact-head platform CI. CLI-DX and V2 acceptance remain open; V3 has not started.
...
**Authorized transport prerequisite — first review unit implemented 2026-09-20 (`0180e9b66`):** The user accepted fixing the native large-response limitation before continuing the two-node journey. [Native large RPC response brief](NET_RPC_LARGE_RESPONSE_BRIEF.md) owns the bounded sequence. This first unit fixes silent failure, **not large-response delivery**: direct publish refuses over-cap framed payloads before opening/charging its stream; an oversized RPC RESPONSE is replaced by a small existing-wire-status Internal response with the same call identity, the size/limit, and an uncertain-effect/no-automatic-retry warning. Application bytes are not echoed. No packet constant, wire status, SDK signature or fragmentation capability changed.

Evidence: `integration_nrpc_mesh` verifies the exact largest accepted response, one byte over and 22 KB as explicit server errors instead of timeouts; an oversized request never invokes its handler, and a following small request on the same service succeeds. Handler counts prove no retry. The CLI metadata witness restores a 22 KB failure case for snapshot and generation and checks explicit size diagnostics, unchanged existing snapshot/no generated directory and one invocation per command. The full existing CortEX/nRPC CI-family selection passed **281 tests across 30 binaries**, using the local `cargo tf` feature alias and zero retries (routing/hijack, streaming/duplex, protected calls, tools and cross-language regressions included). CLI suites passed **312 default / 325 rtc-bootstrap**, each with **1 existing ignored** aggregator query and zero retries. Core default `cargo check --lib`, strict default lib/bins clippy, web checks and diff hygiene passed. Workspace-wide `cargo fmt --all -- --check` hit Windows command-length error 206; explicit rustfmt checks of the three changed core/test files and CLI formatting passed. CodeGraph refresh passed; ccc refresh was requested but remains in progress alongside another workspace's indexing. Broad feature/rustdoc/pre-push matrix and exact-head platform CI remain outstanding; this is not a ready-branch claim.

**Authorized transport prerequisite — initial delivery implementation 2026-09-20 (`5ead88e8a`):** Native unary calls now opt into bounded fragments using request bit 6 before proof signing. Existing response envelopes preserve status/headers/body, with a 1 MiB encoded-response cap, 256 fragments and an 8 MiB incomplete-payload budget per caller. Packet size remains 8 KiB. Reassembly validates lengths before allocation, binds peer/call/session, tolerates identical duplicate/reordered pieces and rejects contradictory or malformed pieces. Pending-call cancellation/deadline cleanup owns partial buffers; a session-turnover watcher also retires calls without deadlines. Fragment sends wait for packet credit for at most one second, remain bound to the receiving session and never use roster fallback. Legacy callers retain explicit single-packet refusal. Request-body fragmentation remains out of scope.

**Delivery evidence:** The two-node UDP witness succeeds at the old packet boundary, boundary+1, 22 KB and the 1 MiB encoded maximum; maximum+1 fails explicitly, with five calls and exactly five handler invocations. Live typegen restores roughly 22 KB metadata and successful capture/offline TS/Python regeneration after provider shutdown; over-limit metadata still preserves outputs and is not retried. **281 integration tests across 30 CortEX/nRPC binaries**, **93 focused RPC/compatibility units**, and **312 default / 325 rtc-bootstrap CLI tests** passed with zero retries (each CLI run retains **1 existing ignored** aggregator query). `TYPEGEN_TSC` was set for real downstream TS checks. Shared Rust/TS/Python/Go fragment-layout fixtures pass; Go was run as a standalone test file, not the cgo binding suite, and none of these layout checks claim cross-language network interoperability. Core default check and strict lib/bins clippy, targeted rustfmt/CLI formatting, web checks, diff hygiene and both code indexes passed. Default-feature strict rustdoc was attempted and failed on the pre-existing `behavior/deck.rs:353` link to the webrtc-gated `DeckClient::rtc_anchors` (confirmed in the parent revision); this is not a passed documentation gate. The full feature/workspace/rustdoc matrix and exact-head Unix/platform CI remain outstanding.

**Partial-transfer lifecycle correction 2026-09-20 (`11fcb4a5d`):** Ten new lifecycle/credit witnesses use connected native nodes, real unary calls and first-fragment UDP delivery, then inspect the actual pending registry and reserved bytes. Deadline, dropped future, cancel token, session-table eviction, receive-lifetime retirement and node shutdown release the incomplete response without retry. The retirement and shutdown tests first failed: an unchanged session ID remained in the table, leaving a no-deadline call pending. Caller watching and fragment send admission now check shutdown and receive-lifetime retirement. Advisory inactivity is deliberately still permitted. Credit witnesses cover bounded stall, subsequent credit reuse, blocked-send resumption after refund, and refusal to send after retirement even when credit is refunded. Session eviction/retirement are deterministic state injections, while shutdown invokes the real shutdown path; heartbeat expiry and cross-computer teardown are not claimed.

Correction validation: **103 focused RPC units**, **281 integration tests across 30 binaries**, and **5 live-metadata CLI tests** passed with zero retries; `TYPEGEN_TSC` was set. Default core check and strict production lib/bins clippy, targeted rustfmt, diff hygiene and code-index refresh passed. The earlier full CLI default/rtc-bootstrap counts are historical evidence, not reruns of this correction. Broad feature/workspace/rustdoc gates, the previously recorded default-rustdoc link failure, and exact-head platform CI remain outstanding. No push.

**Next authorized unit:** Finish the sender half of large-response acceptance: test queue saturation, provide/review a shared server transfer deadline and cancellation ownership, and ensure failed delivery does not leave queued fragments draining after the call ends. The current response drainer can still drop pieces under load, and the one-second credit wait is per packet rather than a shared server transfer deadline. The new caller cleanup and credit witnesses do not close these remaining gates or the brief. Then resume the two-node CI journey and final V2 acceptance. V3 remains unstarted.

**Second review unit — implemented 2026-09-20 (`5b8be6f0e`):** Added provider-attributed acquisition in `cli/src/commands/typegen/live.rs`, using the existing public capability fold in a single observation (no SDK widening). Conflicting selected advertisements, including differing versions, fail; identical replicas select the lowest node ID and count unique advertisers. Missing input triggers exact-provider JSON `tool.metadata.fetch`; missing output also triggers it when that advertisement exposes the metadata service. A legacy inline-input advertisement without that service may retain absent output. Returned ID/version/tags and already-inline schemas must match; missing/unusable input, unsupported present output, NotFound and RPC failure refuse before final publication. Live generation also refuses renderer skips. Optional output and snapshot v1 remain intact; offline skip behavior is unchanged. All fetches share the remaining CLI deadline, or one 30-second post-attachment acquisition limit when omitted. Windows stack growth from the RPC future was caught by subprocess tests and fixed with boxed futures.

Evidence: real SDK metadata service plus separate CLI captures a roughly 6 KB input contract omitted from the announcement, then shuts down the provider and regenerates every TS/Python artifact byte-identically twice. The fixture deliberately publishes a schema-free projection: it proves metadata acquisition, **not automatic SDK schema truncation**. Real subprocess controls cover NotFound, wrong ID/version, missing input, malformed input and timeout for both snapshot and generation, with empty stdout, preservation of the old snapshot/no generated directory, and one handler call per invocation. Three sequential delayed fetches exhaust a single three-second budget without partial publication or retry. Unit controls cover deterministic replica selection, provider/version/schema conflicts, tag/inline-schema mismatch and optional output. Temporarily removing ID/version checks made the subprocess witness fail on false-success snapshot publication; restored checks are green. Existing downstream fixture checks ran successfully: Python compile/mypy and TS strict compilation (the initial TS invocation skipped; rerunning with `TYPEGEN_TSC` pointing at the web package's installed compiler executed it).

Final Windows suites: **312 default passed**, **325 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target compilation, strict default binary clippy, formatting, web checks, diff hygiene and ccc/CodeGraph index refresh passed. Public CLI/typegen documentation and changelog now describe hydration, conservative selection, timeout behavior and transport limitations. Exact-head Unix/platform CI remains outstanding.

**Historical transport-size finding:** The original roughly 22 KB metadata response failed with `WSAEMSGSIZE` at `MAX_PACKET_SIZE = 8192`; reducing the fixture to roughly 6 KB did not resolve that finding. The later authorized transport implementation and current remaining acceptance are recorded immediately above. Do not interpret the original smaller-fixture success, or the new bounded large-response success, as arbitrary-size schema support or completed V2 acceptance. V3 remains unstarted.

**First review unit — implemented 2026-09-20 (`6ff853421`):** Live discovery now waits for every explicit tool ID after tag/ID intersection, instead of stopping at unrelated descriptors. Missing IDs exit 7 before snapshot/generated publication; tag-only/unfiltered queries observe the full five-second window. The existing absolute CLI deadline can shorten that window, not extend it. Offline subset filtering and snapshot v1 are unchanged. Public typegen/reference docs and the CLI changelog describe this bounded contract and retain the missing-schema limitations.

Evidence: **9 focused cases passed**, covering selection/tag intersection, missing IDs, broad-window observation, real SDK-host/CLI delayed arrival, preservation of an existing snapshot/no generated directory on missing input, and existing target/bind/deadline controls. Restoring first-nonempty behavior failed both the selection unit witness and, after strengthening its fixture, the delayed-arrival subprocess witness (`unrelated tools ended discovery`); all inverse edits were removed. The first live fixture had allowed announcements to coalesce and did not distinguish the regression, so it was replaced with a test-only 50ms announcement cadence through existing public `MeshNodeConfig`/`Mesh::from_node_arc`. The existing non-loopback bind fixture exposed the same default ten-second coalescing race and now uses that cadence too. Production SDK configuration is unchanged; these are controlled arrival witnesses, not evidence that default announcement cadence always fits the observation window.

Final Windows suites: **306 default passed**, **319 rtc-bootstrap passed**, each with **1 existing ignored** aggregator query and no retries. Default/optional all-target compilation, strict default binary clippy, formatting, web checks, diff hygiene and code indexes passed. Unix/exact-head CI remains outstanding. The ccc workflow kept both code indexes current; no SDK API was widened.

**First-unit handoff (superseded by the second review above):** Provider-bound metadata hydration and fail-closed schema usability before publication, followed by offline regeneration/downstream acceptance. Source inspection found existing public provenance in `Mesh::inner().capability_fold()` (node ID plus per-entry tags/metadata), while `list_tools` aggregates away provider identity. Use that existing surface to select exact `(tool ID, version, provider)` tuples and define conflict behavior before fetching. Real oversized-schema, NotFound/mismatch/remaining-budget failure tests and deterministic regeneration after provider shutdown were still required at this checkpoint. The two-node journey and exact-head V2 acceptance still precede V3.
