...
| **2B.3** | `ArcSwap`-published generation-stamped `OrgRouteSet` + publish-if-current + warmed-call consumption. **2B.3a SIGNED** at `fd05a89ba`; **2B.3c-pre SIGNED** at `2aa6431ed`; **2B.3b SIGNED** at `5524bbc25`; **2B.3c** step 1 accepted and **step 2 SIGNED** at `d90493a5d`; **2B.3d-pre step 1 SIGNED** at `596e32190` — it was HELD by two independent reviews and then by a specification review, repaired additively in both rounds, and the repairs are inside the signed head. `2B.3d` itself remains unauthorized. See [`OLB_2B3B_WARMED_CALL_BOUNDARY_DESIGN.md`](OLB_2B3B_WARMED_CALL_BOUNDARY_DESIGN.md) §19. Design at revision 6 in that document. |
...
| **2B.5** | `sensed_candidates` join + §8 classification + granted-candidates-Unknown, inside the actor, never on the request path. **SUPERSEDED FOR ORGANIZATION EXACT SENSING — NOT AN IMPLEMENTATION CONTRACT for that lane:** the organization-audience exact-sensing path performs **one bounded PER-CALL observation snapshot on the request path** (`S <= 32` rows), then releases the lock before route/budget/ordering work — see [`ORG_EXACT_SENSING_ACQUISITION_PROJECTION_DESIGN.md`](ORG_EXACT_SENSING_ACQUISITION_PROJECTION_DESIGN.md) D6.4/D6.5 and [`ORG_CAPABILITY_LOAD_BALANCING_PLAN.md`](ORG_CAPABILITY_LOAD_BALANCING_PLAN.md) §2A. This row governs only the unsensed/warmed-pool 2B lane and cannot govern OA exact sensing. |
