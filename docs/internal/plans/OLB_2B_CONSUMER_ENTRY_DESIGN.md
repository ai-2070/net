...
| **2B.3** | `ArcSwap`-published generation-stamped `OrgRouteSet` + publish-if-current + warmed-call consumption. **2B.3a SIGNED** at `fd05a89ba`; **2B.3c-pre SIGNED** at `2aa6431ed`; **2B.3b SIGNED** at `5524bbc25`; **2B.3c** step 1 accepted, step 2 landed and unsigned; **2B.3d-pre step 1 implemented at a candidate, HELD by two independent reviews and then by a specification review, repaired additively in both rounds, still unsigned** — see [`OLB_2B3B_WARMED_CALL_BOUNDARY_DESIGN.md`](OLB_2B3B_WARMED_CALL_BOUNDARY_DESIGN.md) §19. Design at revision 6 in that document. |
