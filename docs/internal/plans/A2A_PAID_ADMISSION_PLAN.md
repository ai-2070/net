...
**Two upstream gaps found while building, left unfixed as out of slice:** a paid `purchase_task` against a provider whose payments RPC is unregistered blocks indefinitely instead of answering an ambiguity verdict (the pay RPC carries no deadline), and a prepare whose brief exceeds the nRPC request-body limit blocks instead of being refused — so a `max_prompt_bytes` set above that limit is unenforceable in practice. Both belong to `mesh_rpc`, and both would change transport semantics for every service.
