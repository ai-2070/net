...
| `http402.md` | When a Net agent **pays an external x402 HTTP API** — the outbound `X402HttpFlow` + the Python `PaymentHttpClient`, the header-only v2 transport, why it's the same objects (the two-way door). |
...
| `bindings.md` | When the language matters — the per-language table (only **Rust + Python** have a native flow; Node/Go carry only a golden-vector verifier), the full Python `CapabilityGateway` surface (approval verbs, HTTP-402 client, svm/xrpl signers). |
...
1. **Identify the language.** Only **Rust** and **Python** have a native
   payment flow (Python's is now the full demand surface — approval verbs,
   HTTP-402 client, svm/xrpl signers). **Node** exposes read-only
   `ToolDescriptor.pricingTerms` at discovery + a golden-vector verifier;
   **Go** has a golden-vector verifier but no flow. No native TS/Go/C payment
   flow exists. Check `bindings.md` before promising a flow.
