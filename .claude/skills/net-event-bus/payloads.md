...
7. **Pydantic vs dataclass on the Python side.** Both work with the typed channel API. `net_sdk.channel._to_dict` picks, in order: `model_dump()` (Pydantic v2), a `dict` copy, `dataclasses.asdict` (which also recurses into nested dataclasses, and is why `@dataclass(slots=True)` works — a slotted instance has no `__dict__`), a `__dict__` copy, a `{"_value": ...}` wrapper for JSON scalars and lists, then declared `__slots__`. Anything else raises `TypeError` at publish rather than producing a payload that dies inside `json.dumps`. **Deserialize** is the asymmetric side: `TypedChannel.subscribe` uses `model(**payload)` — fine for dataclasses and plain `__init__`-accepting types. **For Pydantic v2 you usually want `model_validate_json(...)`, not `model(**...)`**, since the latter skips coercion and validators: pass `parse=` to `node.channel('name', parse=lambda raw: MyModel.model_validate_json(raw))`. `parse` receives payload-only JSON (the `_channel` tag is stripped first), so a model with `extra="forbid"` still accepts channel events.
