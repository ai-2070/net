...
- **A capability re-announcement is dropped, not queued, when it lands inside the broadcast window.** This note previously claimed only a node's *first* announcement ever reaches peers. That was wrong: it generalised from a measurement that was really the broadcast window. Re-measured at `058a31409` — three announces ~300 ms apart, and only the first reaches peers; the same three spaced 12 s apart propagate in **both** directions, adding a tag and removing one. The window is `min_announce_interval`, 10 s by default; an announce inside it returns `Ok(())` and ships nothing. On a node started with the plain `start()` these examples use, the code's deferral arm is a silent drop (the deferred-flush arm needs the `start_arc` variant), so peers keep the previous set until the next out-of-window announce or the keep-alive. The announcer's **own** fold updates immediately either way — a node's view of itself is correct while its peers' view is stale, which is what makes this look like propagation failing. Space re-announcements past the window, or expect the change to ride the keep-alive, and do not read `Ok(())` as "peers have this". `registry.rs` keeps demonstrating membership growing for that reason — retirement is demonstrable too, just not inside 10 s. Verify the window against your own version before designing a withdrawal-based scheme on it.
