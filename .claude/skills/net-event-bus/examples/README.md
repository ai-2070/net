...
| `objectstore.rs` / `objectstore.ts` / `objectstore.py` / `objectstore.c` | Rust ✓ · TS ✓ · Py ✓ · C ✓ | S3 / MinIO | store bytes · mint a content address · fetch them from another node · store the same bytes again for the same address | `RESULT ok dedup=1 readback=1 bytes=64` |
...
- **`object-store` could not be written in C or Go, because neither could mint a blob address — found, and the C half fixed.** The ABI could `store`/`fetch` a blob given an *encoded* ref and had no way to create one: `net_blob_publish` is declared in no shipped header and targets the external-hook adapter registry, not the substrate `MeshBlobAdapter` that `net_mesh_blob_adapter_*` uses. Two functions close it — `net_mesh_blob_adapter_publish` (BLAKE3 + store + encoded ref out) and `net_blob_ref_hash` (the 32-byte hash out of an encoded ref, which the transport fetch needs because `net_fetch_blob` addresses by hash, not by ref) — both declared in `net.go.h`, mirrored to `go/net.h`, with `-D warnings`-clean stubs for builds without `dataforts`. `objectstore.c` runs on them.
- **Go still has no `object-store`, and the reason narrowed.** The mint now exists as an ABI symbol, but the Go binding has no mesh blob-*transfer* methods at all: `MeshNode` exposes neither `ServeBlobTransfer` nor `FetchBlob`, and `go/net.h` carries none of `net_transport.h`'s prototypes. Go can `Publish`/`Store`/`Fetch` locally; it cannot fetch from another node, which is the route. That is the remaining binding work.
