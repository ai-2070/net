...
- **`object-store` has no Go or C example, for the same structural reason.** Both bindings can `store`/`fetch` a blob given an *encoded* ref, and neither can mint one: Go exports no `BlobRef` constructor, and C's only mint (`net_blob_publish`) is declared in no shipped header and targets the external-hook adapter registry rather than the substrate `MeshBlobAdapter`. A producer in those languages cannot create the address the route is about. Rust, TypeScript and Python carry it.
- **`net_rpc.h` does not parse on its own — found while writing `jobqueue.c`.** `RpcResponseSinkHandleC` is used inside the `net_rpc_streaming_handler_fn` typedef (~line 501) before its own typedef (~line 902), and that function-pointer typedef precedes its own definition (~line 508), so a translation unit that includes only `net_rpc.h` fails with unknown-type errors. `jobqueue.c` works around it with an identical forward typedef, which C11 permits; the header should declare these before first use.
