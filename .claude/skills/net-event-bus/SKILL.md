...
| `dataforts.md` | When the user is on v0.15+ and asks about **greedy caching**, **data gravity** (chains drift toward readers), **blob refs** (substrate carries content-addressed pointers; bytes in S3 / Ceph / IPFS / FS), or **read-your-writes** (`WriteToken` + `wait_for_token` so a producer reads its own write deterministically). Compositional data plane on top of RedEX + CortEX. |
