...
- **Permission tokens → root-anchored token chains (v0.27):** a channel registered with `token_roots` enforces auth. A presented credential is a `TokenChain` — honored only if it **roots** at one of the channel's `token_roots`, **binds** at its leaf to the subscribing peer's `EntityId`, and **authorizes** the action (`SUBSCRIBE` / `PUBLISH`) at every link. Bare single-token credentials were replaced wholesale; the chain travels the wire on subscribe (`subscribe_channel_with_chain`, surfaced as `SubscribeOptions.token` on the SDK) and the publisher re-checks expiry + revocation while the subscription lives. A **delegated** publisher (granted via `owner → org → this_node`) must install its full chain locally with `MeshNode::set_publish_chain(channel, chain)` at startup, or its publishes silently fail the root-anchor check; **direct-issued** publishers (the common case) need no change. Revocation is immediate (a root dropped from `token_roots` revokes inline). **This is a breaking wire change** — v0.26 and v0.27 peers do not interoperate on auth-gated channels; roll the substrate atomically across peers. (v0.27.2 separately binds a capability announcement's wire `node_id` to the verified signer — a forged `node_id` is now rejected with `WireError::NodeIdMismatch` instead of trusted.)
