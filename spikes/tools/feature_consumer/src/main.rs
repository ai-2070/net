...
fn main() {
    ...
    let udp: net_mesh::adapter::net::PeerAddr =
        net_mesh::adapter::net::PeerAddr::Udp("127.0.0.1:1".parse().expect("addr"));
    println!("peer addr: {udp:?}");
    #[cfg(feature = "wire-only")]
    {
        let rtc = net_mesh_wire::peer_addr::PeerAddr::Rtc(net_mesh_wire::peer_addr::RtcPeerId {
            slot: 1,
            generation: 0,
        });
