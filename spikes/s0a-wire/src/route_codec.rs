//! Routing-envelope codec extracted from
//! `net/crates/net/src/adapter/net/route.rs` (lines 1-319) for the S0a
//! wire-boundary spike. Only the codec travels: `ROUTING_MAGIC`,
//! `ROUTING_HEADER_SIZE`, `RouteFlags`, `RoutingHeader` and
//! `to_bytes` / `from_bytes` / `write_to` / `write_at` / `read_from` /
//! `forward`. The route *table* (`RouteEntry`, metrics, `next_hop`,
//! `SchedulerStreamStats`) is deliberately left behind.
