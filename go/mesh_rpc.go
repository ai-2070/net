...
// `NewMeshRpc(node)` to wrap it in a `*MeshRpc` and `Call` /
// `CallService` / `Serve` against the mesh.
//
// # Build prerequisites
//
...
//
//   - Build a `*MeshNode` upstream via `NewMeshNode(cfg)`.
//   - Call `NewMeshRpc(node)` to take an `Arc<MeshNode>` clone and
//     wrap it in a `*MeshRpc`. The wrapper installs a runtime
//     finalizer so dropped values release the C handle, but call
//     `Close()` for deterministic cleanup.
//   - Call `Serve(service, handler)` to register a handler. The
//     returned `*ServeHandle` MUST be closed when the service
...
/*
#cgo LDFLAGS: -L${SRCDIR}/../net/crates/net/target/release -lnet_rpc -lnet
#include <stdint.h>
#include <stdlib.h>
#include <string.h>
...
// ABI 0x0002 — client-streaming caller-side.
...
// ABI 0x0002 — server-side dispatcher hooks.
...
// `NewMeshRpc(node)` where `node` is a live `*MeshNode` from
// `NewMeshNode(cfg)`.
//
...
// NewMeshRpc wraps a live `*MeshNode` in an nRPC consumer. The
// MeshNode keeps owning its handle; this constructor clones the
// inner `Arc<MeshNode>` once and hands it to `net_rpc_new`, which
// takes ownership of the clone.
//
// Installs a runtime finalizer to release the C handle on GC, but
// callers SHOULD `defer rpc.Close()` for deterministic cleanup.
//
// Call chain: NewMeshRpc → node.arcClonePtr() →
// `net_mesh_arc_clone(handle)` → returned `*mut Arc<MeshNode>` →
// `net_rpc_new(node_arc)` → `*MeshRpcHandle`.
func NewMeshRpc(node *MeshNode) (*MeshRpc, error) {
	if node == nil {
		return nil, errors.New("net.NewMeshRpc: node must be non-nil")
	}
	registerDispatcher()
	arcPtr := node.arcClonePtr()
	if arcPtr == nil {
		return nil, errors.New("net.NewMeshRpc: node is shutting down or freed")
	}
	h := C.net_rpc_new(arcPtr)
	if h == nil {
		// `net_rpc_new` only returns NULL on NULL input. Our nil
		// gate above should make this unreachable, but be defensive.
		return nil, errors.New("net_rpc_new returned NULL")
	}
	r := &MeshRpc{handle: h}
...
// lock. Returns ErrRpcClosed if Close has already taken the handle
// down.
...
// on this MeshRpc return ErrRpcClosed. Blocks briefly on any
...
// ErrRpcClosed is returned by operations on a closed MeshRpc. Named
// distinctly from any `ErrClosed` to avoid collisions with sibling
// modules in this package.
var ErrRpcClosed = errors.New("net.MeshRpc: handle is closed")
...
//     (consumed by the typed wrapper's SetObserver / MetricsSnapshot
//     once it lands in G-2).
...
func installCancelWatcher(ctx context.Context, r *MeshRpc) (uint64, func()) {
	...
	go func() {
		...
		case <-ctx.Done():
			...
			// us. ErrRpcClosed is a quiet drop — substrate's cancel
			// is best-effort.
			_ = r.withHandle(func(h *C.MeshRpcHandle) {
...
// =====================================================================
// ABI 0x0002 — Server-side handlers
// =====================================================================
