 ...
 *
 * Moved into the dedicated `net_cortex.h` header so a Zig / Swift / C
 * consumer who wants only this slice of `libnet` can include it
 * standalone. Compiled when the cdylib is built with `--features "netdb
 * redex-disk"`; symbols remain unresolved otherwise.
 * ========================================================================= */
 ...
 * Resource-agnostic (a GPU NVLink domain is the motivating instance);
...
int      net_mesh_publish_island_topology(net_meshnode_t* handle,
                                          ...
                                          size_t* out_count);
int      net_mesh_match_islands(net_meshnode_t* handle,
...
int      net_mesh_release_island(net_meshnode_t* handle, uint64_t island,
                                 int* out_outcome);
int      net_mesh_claim_island(net_meshnode_t* handle,
                               const char* criteria_json,
 ...
 * persistent: 0 = in-memory chunks; 1 = disk-backed (requires the redex
 *             to have been opened with a `persistent_dir`).
 * overflow_json: NULL or null-terminated JSON for the v0.3 overflow config
 *                (fields: enabled, high_water_ratio, low_water_ratio,
 *                max_pushes_per_tick, scope, tick_interval_ms). Empty
 *                string / NULL = overflow off (the v0.2 default).
 *
 * Returns a non-null handle on success. On error returns NULL.
 * Free with net_mesh_blob_adapter_free.
 */
...
/* Caller frees the returned string via net_free_string. */
...
/* Returns a JSON-encoded OverflowConfig (caller frees via net_free_string). */
...
/* Returns a JSON `[{"bucket": <str>, "value": <u64>}, ...]`
 * sorted lex by bucket. `matcher_json` may be NULL (walks every
 * entry). Caller frees the returned string with
...
/* Returns a JSON list of capacity rows sorted by `available`
 * descending (ties broken on bucket ascending), truncated to
 * `query.limit`. Each row carries
 * `{"bucket": <str>, "idle": <u64>, "busy": <u64>,
 *   "reserved": <u64>, "available": <u64>,
 *   "summed_capacity": <u64?> }`.
 *
 * `rtt_map_json` is the materialized RTT map, encoded as a JSON
 * `[{"node_id": <u64>, "rtt_ms": <u32>}, ...]` array. May be
 * NULL or `"[]"`; either disables the RTT filter regardless of
 * `query.max_rtt_ms`. The plan-noted callback variant for
 * pure-Rust / TS / Python is intentionally not exposed at this
 * C boundary — every C-ABI consumer materializes the map.
 *
